...
class ResultMerger:
    ...
    def merge_results(
        self, results_by_modality: dict[str, list[SearchResult]]
    ) -> list[MergedResult]:
        ...
        try:
            ...
            total_results = sum(
                len(results) for results in results_by_modality.values()
            )
            logger.debug(
                f"Merging {total_results} results from {len(results_by_modality)} modalities"
            )
            ...
            for location, modality_results in result_groups.items():
                try:
                    ...
                    logger.warning(
                        f"Failed to merge results for location {location}: {e}",
                        extra={
            ...
            logger.debug(
                f"Successfully merged into {len(merged_results)} unique results"
            )
            return merged_results

        except Exception as e:
            logger.error(
                f"Failed to merge results: {e}",
                exc_info=True,
                extra={
                    "operation": "merge_results",
                    "error_type": type(e).__name__,
                    "modality_count": len(results_by_modality)
                    if results_by_modality
                    else 0,
                    "total_input_results": sum(
                        len(results) for results in results_by_modality.values()
                    )
                    if results_by_modality
                    else 0,
    ...
    def _group_by_location(
        self, results_by_modality: dict[str, list[SearchResult]]
    ) -> dict[tuple[str, int], dict[str, SearchResult]]:
    ...
    def _merge_location_group(
        self, location: tuple[str, int], modality_results: dict[str, SearchResult]
    ) -> MergedResult:
        ...
        try:
            ...
            content = max(
                (r.content for r in modality_results.values()), key=len, default=""
            )
            ...
            for modality, result in modality_results.items():
                if not isinstance(result.score, int | float) or result.score < 0:
                    logger.warning(
                        f"Invalid score {result.score} for modality {modality}, using 0.0"
                    )
                    score = 0.0
                else:
            ...
            logger.error(
                f"Failed to merge location group {location}: {e}",
                exc_info=True,
                extra={
                    "operation": "merge_location_group",
                    "error_type": type(e).__name__,
    ...
    def boost_relevant_results(
        self, results: list[MergedResult], query: str
    ) -> list[MergedResult]:
    ...
    def filter_by_confidence(
        self, results: list[MergedResult], min_confidence: float = 0.3
    ) -> list[MergedResult]:
        """Filter results below minimum confidence threshold."""

        return [r for r in results if r.combined_score >= min_confidence]

    def get_diversity_subset(
        self, results: list[MergedResult], max_results: int = 20
    ) -> list[MergedResult]:
    ...
    def generate_search_summary(
        ...
        if not results:
            return {"query": query, "total_results": 0, "summary": "No results found"}
        ...
        return {
            ...
            "multi_modality_results": len(
                [r for r in results if len(r.source_modalities) > 1]
            ),
...
if __name__ == "__main__":
    ...
    text_results = [
        SearchResult(
            "def calculate_delta", "options.py", 42, 1.0, "text", {}, time.time()
        ),
        SearchResult(
            "delta calculation", "pricing.py", 15, 0.9, "text", {}, time.time()
        ),
    ]

    structural_results = [
        SearchResult(
            "calculate_delta function",
            "options.py",
            42,
            0.95,
            "structural",
        ...
        SearchResult(
            "Delta class", "greeks.py", 8, 0.85, "structural", {}, time.time()
        ),
    ]

    results_by_modality = {"text": text_results, "structural": structural_results}
    ...
    for result in merged:
        print(
            f"  {result.file_path}:{result.line_number} - Score: {result.combined_score:.2f}"
        )
