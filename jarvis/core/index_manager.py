...
class IndexManager:
    ...
    def __init__(self, workspace_root: str = ".", index_path: str = ".jarvis/index.duckdb"):
    ...
    async def build_index(self, force_rebuild: bool = False) -> Dict[str, Any]:
        ...
        py_files = [f for f in py_files if not any(p in str(f) for p in ignore_patterns)]
        ...
        return {
            ...
            "files_per_second": total_indexed / (duration / 1000) if duration > 0 else 0
        }
        ...
        for file_path in files:
            try:
                ...
                index = FileIndex(
                    ...
                    exports=[c["name"] for c in analysis.classes] + [f["name"] for f in analysis.functions],
                    keywords=keywords,
    ...
    def _extract_keywords(self, content: str) -> List[str]:
        ...
        python_keywords = {'self', 'def', 'class', 'if', 'else', 'for', 'while', 'return', 'import', 'from'}
    ...
    def _detect_trading_patterns(self, content: str) -> Tuple[bool, List[str]]:
        ...
        for category, keywords in self.trading_patterns.items():
    ...
    async def _get_last_indexed_time(self) -> Optional[float]:
    ...
    async def search_by_pattern(self, pattern: str, pattern_type: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    async def get_file_metrics(self, file_path: str) -> Optional[Dict[str, Any]]:
        ...
        if not result.empty:
            ...
            # Parse JSON fields
            for field in ['imports', 'exports', 'keywords', 'doc_strings', 'trading_keywords']:
