...
class Jarvis:
    ...
    def _init_tools(self):
        ...
        if self.config.verbose:
            print(f"🤖 Jarvis initialized with hardware mode: {self.config.hardware_mode}")
            print(f"   • CPU cores: 12 (8 performance + 4 efficiency)")
            print(f"   • GPU: Metal (20 cores)")
            print(f"   • RAM: 24GB (19.2GB available)")
    ...
    async def assist(self, query: str) -> Dict[str, Any]:
        ...
        for phase in phases:
            ...
            if self.tools.tracer:
                async with self.tools.tracer.trace_span(f"jarvis.{phase.value}") as span:
    ...
    async def explain_approach(self, query: str) -> str:
        ...
        discover_result = await self.phase_executor.execute_phase(Phase.DISCOVER, context)
