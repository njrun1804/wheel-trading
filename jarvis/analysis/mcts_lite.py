...
class MCTSLite:
    ...
    async def search(
        self, initial_state: dict[str, Any], time_limit_ms: float = 5000
    ) -> dict[str, Any]:
    ...
    def _select(self, node: MCTSNode) -> MCTSNode:
        ...
        if not self._is_terminal(current) and len(current.children) < self._get_actions(
            current.state
        ):
    ...
    def _apply_action(self, state: dict[str, Any], action: str) -> dict[str, Any]:
        ...
        if action == "parallelize_loops":
            ...
            new_state["complexity_increase"] = (
                new_state.get("complexity_increase", 0) + 5
            )
