...
def volume_regime_prediction_test():
    ...
    data["vol_change_5d"] = (data["future_vol_5d"] - data["volatility_20d"]) / data[
        "volatility_20d"
    ]
    data["vol_change_10d"] = (data["future_vol_10d"] - data["volatility_20d"]) / data[
        "volatility_20d"
    ]
    ...
    print(
        f"  Volume spike days: {data_clean['volume_spike'].sum()} ({data_clean['volume_spike'].mean():.1%})"
    )
...
def risk_parity_allocation():
    ...
    for regime, params in regime_risks.items():
        ...
        print(
            f"{regime:<7} | {params['frequency']:>9.0%} | {params['volatility']:>7.0%} | "
    ...
    for regime, params in regime_risks.items():
        ...
        print(
            f"{regime:<7} | {params['frequency']:>9.0%} | {position_size:>13.1%} | "
            f"{risk_contrib:>16.1%}"
        )
