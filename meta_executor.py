...
class MetaExecutor:
    ...
    def __init__(self, meta_db_path: str = None):
        ...
        print(
            f"🛡️  Safety mode: {'DRY RUN' if self.dry_run_mode else 'LIVE EXECUTION'}"
        )
    ...
    def safety_check(self, modification: CodeModification) -> tuple[bool, list[str]]:
        ...
        size_change_ratio = abs(modified_lines - original_lines) / max(
            original_lines, 1
        )
        ...
        if size_change_ratio > 2.0:  # More than 2x size change
            size_valid = False
            size_msg = (
                f"Excessive size change: {original_lines} → {modified_lines} lines"
            )
    ...
    def apply_modification(self, modification: CodeModification) -> bool:
        ...
        if self.dry_run_mode:
            ...
            try:
                ...
                expected_hash = hashlib.sha256(
                    modification.modified_content.encode()
                ).hexdigest()
                ...
                if new_hash == expected_hash:
                    ...
                    print(
                        "⚠️  Verification warning: File content differs from expected"
                    )
        ...
        """,
            (
                ...
                modification.validation_passed,
                modification.applied,
                modification.rollback_available,
                outcome,
            ),
        )
    ...
    def execute_code_generation(self, generation: CodeGeneration) -> bool:
        ...
        if generation.generation_type == "method_addition":
            modified_content = self._add_method_to_file(
                original_content, generation.generated_code
            )
        elif generation.generation_type == "optimization":
            modified_content = self._apply_optimization(
                original_content, generation.generated_code
            )
        else:
            # Generic addition at end of class
            modified_content = self._generic_code_addition(
                original_content, generation.generated_code
            )
    ...
    def _generic_code_addition(self, original_content: str, generated_code: str) -> str:
        ...
        try:
            ...
            has_classes = any(
                isinstance(node, ast.ClassDef) for node in generated_tree.body
            )
            has_functions = any(
                isinstance(node, ast.FunctionDef) for node in generated_tree.body
            )
        ...
        try:
            ...
            """,
                (
                    ...
                    "rollback",
                    True,
                    f"Rolled back modification {modification_id}",
                ),
            )
    ...
    def get_executor_status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "success_rate": self.successful_modifications
            / max(len(self.modifications), 1),
...
"""
        ...
        for mod_id, target_file, change_type, applied, _outcome in cursor.fetchall():
