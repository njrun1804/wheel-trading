...
from claude_cli_reasoning_capture import ClaudeCLIReasoningCapture, CLIReasoningEvent
...
class MetaClaudeCLITrainer:
    ...
    def ingest_cli_session(self, session_file: str) -> Dict[str, Any]:
        ...
        self.meta_prime.observe("meta_cli_training_session", {
        ...
        print(f"📚 Ingested session: {len(session_data['events'])} events → {len(insights)} insights → {len(improvements)} improvements")
    ...
    def _extract_meta_training_insights(self, session_data: Dict[str, Any]) -> List[MetaTrainingInsight]:
        ...
        for i, event in enumerate(events):
            if event["event_type"] == "claude_analysis":
                ...
                insight = self._analyze_code_generation_for_meta_learning(event, session_data)
                if insight:
                    insights.append(insight)
            
            elif event["event_type"] == "error_recovery":
                # Analyze error recovery for meta_executor safety improvements
                insight = self._analyze_error_recovery_for_meta_learning(event, session_data)
                if insight:
                    insights.append(insight)
    ...
    def _analyze_reasoning_for_meta_learning(self, event: Dict[str, Any], session_data: Dict[str, Any]) -> Optional[MetaTrainingInsight]:
    ...
    def _analyze_code_generation_for_meta_learning(self, event: Dict[str, Any], session_data: Dict[str, Any]) -> Optional[MetaTrainingInsight]:
        """Analyze code generation patterns for meta_generator enhancement"""
    ...
    def _analyze_error_recovery_for_meta_learning(self, event: Dict[str, Any], session_data: Dict[str, Any]) -> Optional[MetaTrainingInsight]:
    ...
    def _analyze_event_sequences(self, events: List[Dict[str, Any]]) -> List[MetaTrainingInsight]:
        ...
        for i in range(len(events) - 2):
            if (events[i]["event_type"] == "user_request" and
                events[i+1]["event_type"] == "claude_analysis" and
                events[i+2]["event_type"] == "code_generation"):
    ...
    def _apply_insights_to_meta_system(self, insights: List[MetaTrainingInsight]) -> List[Dict[str, Any]]:
    ...
    def _create_meta_improvement(self, insight: MetaTrainingInsight) -> Optional[Dict[str, Any]]:
    ...
    def _extract_reasoning_chain(self, content: str) -> List[str]:
        ...
        for sentence in sentences:
            if any(word in sentence.lower() for word in ["first", "then", "next", "because", "therefore"]):
    ...
    def _extract_safety_considerations(self, content: str) -> List[str]:
        ...
        safety_items = []
        for sentence in sentences:
            if any(word in sentence.lower() for word in ["safe", "careful", "validate", "check", "ensure"]):
    ...
    def _extract_solution_approach(self, content: str) -> List[str]:
        ...
        solution_steps = []
        for sentence in sentences:
            if any(word in sentence.lower() for word in ["solution", "approach", "strategy", "implement"]):
    ...
    def get_training_summary(self) -> Dict[str, Any]:
        ...
        total_insights = sum(len(session.get("insights", [])) for session in self.training_sessions)
        ...
        for improvement in self.meta_improvements:
            ...
            component_enhancements[component] = component_enhancements.get(component, 0) + 1
...
def demo_meta_cli_integration():
    ...
    if session_files:
        ...
        print("⚠️ No CLI session files found - run claude_cli_reasoning_capture.py first")
