...
class TestBlackScholesPrice:
    ...
    def test_call_option_itm(self) -> None:
        ...
        result = black_scholes_price_validated(S=110, K=100, T=1, r=0.05, sigma=0.2, option_type="call")
        assert abs(result.value - 17.6630) < 0.0001

    def test_put_option_itm(self) -> None:
        """Test in-the-money put option."""
        result = black_scholes_price_validated(S=90, K=100, T=1, r=0.05, sigma=0.2, option_type="put")
        assert abs(result.value - 10.2142) < 0.0001

    def test_deep_otm_call(self) -> None:
        """Test deep out-of-the-money call."""
        result = black_scholes_price_validated(S=50, K=100, T=1, r=0.05, sigma=0.2, option_type="call")
        assert result.value < 0.003  # Should be nearly worthless

    def test_deep_itm_call(self) -> None:
        """Test deep in-the-money call."""
        result = black_scholes_price_validated(S=200, K=100, T=1, r=0.05, sigma=0.2, option_type="call")
    ...
    def test_put_call_parity(self):
        ...
        call = black_scholes_price_validated(S, K, T, r, sigma, "call")
        put = black_scholes_price_validated(S, K, T, r, sigma, "put")
        parity = call - put
    ...
    def test_zero_time_to_expiry(self):
        ...
        # Call at expiry
        result = black_scholes_price_validated(S=110, K=100, T=0, r=0.05, sigma=0.2, option_type="call")
        assert abs(result.value - 10) < 1e-10  # Intrinsic value

        # Put at expiry
        result = black_scholes_price_validated(S=90, K=100, T=0, r=0.05, sigma=0.2, option_type="put")
        assert abs(result.value - 10) < 1e-10  # Intrinsic value

    def test_zero_volatility(self):
        """Test with zero volatility."""
        # Call with zero vol - should be max(S - K*exp(-rT), 0)
        result = black_scholes_price_validated(S=110, K=100, T=1, r=0.05, sigma=0, option_type="call")
        expected = 110 - 100 * np.exp(-0.05)
        assert abs(result.value - expected) < 1e-10

        # OTM call with zero vol
        result = black_scholes_price_validated(S=90, K=100, T=1, r=0.05, sigma=0, option_type="call")
        assert result.value == 0

    def test_high_volatility(self):
        """Test with very high volatility."""
        result = black_scholes_price_validated(S=100, K=100, T=1, r=0.05, sigma=2.0, option_type="call")
...
class TestImpliedVolatility:
    ...
    def test_call_implied_vol(self):
        ...
        true_vol = 0.2
        result = black_scholes_price_validated(S=100, K=100, T=1, r=0.05, sigma=true_vol, option_type="call")
    ...
    def test_put_implied_vol(self):
        ...
        true_vol = 0.3
        result = black_scholes_price_validated(S=100, K=100, T=1, r=0.05, sigma=true_vol, option_type="put")
    ...
    def test_vectorized_implied_vol(self):
        ...
        prices = black_scholes_price_validated(S=100, K=100, T=1, r=0.05, sigma=vols, option_type="call")
    ...
    def test_otm_options(self):
        ...
        # OTM call
        result = black_scholes_price_validated(S=90, K=100, T=1, r=0.05, sigma=0.25, option_type="call")
        iv = implied_volatility(price, S=90, K=100, T=1, r=0.05, option_type="call")
        assert abs(iv - 0.25) < 1e-6

        # OTM put
        result = black_scholes_price_validated(S=110, K=100, T=1, r=0.05, sigma=0.25, option_type="put")
    ...
    def test_short_maturity(self):
        ...
        # 1 day to expiry
        result = black_scholes_price_validated(S=100, K=100, T=1 / 365, r=0.05, sigma=0.3, option_type="call")
...
class TestEdgeCases:
    ...
    def test_very_small_time(self):
        ...
        # 1 minute to expiry
        T = 1 / (365 * 24 * 60)
        result = black_scholes_price_validated(S=101, K=100, T=T, r=0.05, sigma=0.2, option_type="call")
    ...
    def test_negative_interest_rate(self):
        ...
        result = black_scholes_price_validated(S=100, K=100, T=1, r=-0.01, sigma=0.2, option_type="call")
        assert result.value > 0  # Should still be positive

        # Put-call parity should still hold
        call = black_scholes_price_validated(S=100, K=100, T=1, r=-0.01, sigma=0.2, option_type="call")
        put = black_scholes_price_validated(S=100, K=100, T=1, r=-0.01, sigma=0.2, option_type="put")
        parity = call - put
    ...
    def test_zero_stock_price(self):
        ...
        result = black_scholes_price_validated(S=0, K=100, T=1, r=0.05, sigma=0.2, option_type="call")
        assert result.value == 0

        # Put should be worth discounted strike
        result = black_scholes_price_validated(S=0, K=100, T=1, r=0.05, sigma=0.2, option_type="put")
    ...
    def test_zero_strike(self):
        ...
        result = black_scholes_price_validated(S=100, K=0, T=1, r=0.05, sigma=0.2, option_type="call")
        assert abs(result.value - 100) < 1e-10

        # Put should be worthless
        result = black_scholes_price_validated(S=100, K=0, T=1, r=0.05, sigma=0.2, option_type="put")
        assert result.value == 0
    ...
    def test_array_broadcasting(self):
        ...
        prices = black_scholes_price_validated(S=S, K=K, T=T, r=0.05, sigma=0.2, option_type="call")
        assert prices.shape == (3,)

        # Test with multiple dimensions
        S = np.array([[90, 100], [110, 120]])
        prices = black_scholes_price_validated(S=S, K=100, T=1, r=0.05, sigma=0.2, option_type="call")
    ...
    def test_mixed_edge_cases(self):
        ...
        calls = black_scholes_price_validated(S=S, K=K, T=T, r=0.05, sigma=sigma, option_type="call")
