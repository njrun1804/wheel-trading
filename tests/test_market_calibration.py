...
async def test_calibration():
    ...
    prices_df = pd.DataFrame(data, columns=['date', 'open', 'high', 'low', 'close', 'volume', 'returns'])
    ...
    optimal_params = await calibrator.calibrate_from_history(
        returns=returns,
        prices=prices_df,
        iv_history=None  # Would include IV data in production
    )
    ...
    print(f"   - Delta Range: {optimal_params.delta_range[0]:.2f} to {optimal_params.delta_range[1]:.2f}")
    ...
    print(f"\n   Position Sizing:")
    print(f"   - Kelly Fraction: {optimal_params.kelly_fraction:.2f} ({optimal_params.kelly_fraction*100:.0f}%)")
    ...
    print(f"   Dynamic approach: {optimal_params.put_delta_target:.2f} delta, {optimal_params.dte_target} DTE, {optimal_params.kelly_fraction*100:.0f}% Kelly")
    ...
    position_size = portfolio_value * optimal_params.kelly_fraction * optimal_params.max_position_pct
...
async def main():
    ...
    print(f"   - This reduces assignment probability from ~30% to ~{params.put_delta_target*100:.0f}%")
