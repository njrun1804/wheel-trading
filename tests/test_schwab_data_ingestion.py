...
from src.unity_wheel.schwab.types import SchwabPosition, SchwabAccount, PositionType
        ...
        for i in range(5):
            ...
            with sqlite3.connect(temp_storage.db_path) as conn:
                conn.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    timestamp, "TEST123", position.symbol,
                    float(position.quantity), position.position_type.value,
        ...
        with sqlite3.connect(temp_storage.db_path) as conn:
            ...
            for i in range(10):
                ...
                """, (
                    ...
                    "TEST123", f"SYMBOL{i % 3}", 100, "STOCK",
                    5000, 4500, 500, 0, "{}"
                ))
            ...
            for i in range(5):
                conn.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    f"ORDER{i}", "TEST123", "U", 100, "LIMIT",
                    "FILLED" if i < 3 else "CANCELLED", 50,
                    datetime.now(timezone.utc) - timedelta(days=i), "{}"
                ))
    ...
    async def test_data_validation(self, temp_ingestion):
        ...
        with sqlite3.connect(temp_ingestion.storage.db_path) as conn:
            ...
            """, (
                ...
                "TEST123", "U231220P00045000", -1, "OPTION",
                0, -200, 200, 0,
                "2023-12-20", "{}"
            ))
