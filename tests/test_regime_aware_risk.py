...
def main():
    ...
    for regime, stats in regime_results['regime_stats'].items():
        regime_name = ['Low Vol', 'Medium Vol', 'High Vol'][regime] if regime < 3 else f'Regime {regime}'
        print(f"\n   {regime_name}:")
    ...
    print(f"   High vol days: {cvar_results['high_vol_days']} ({cvar_results['high_vol_days']/cvar_results['total_days']*100:.1f}%)")
    ...
    if current_regime == 0:  # Low volatility
        ...
        print("   - Use regime-specific VaR: {:.1f}%".format(regime_results['regime_vars'][1]*100))
