...
class JarvisSelfTest:
    ...
    async def analyze_and_fix_failures(self):
        ...
        for i, failure in enumerate(self.test_results['failed_tests'], 1):
            print(f"[{i}/{len(self.test_results['failed_tests'])}] Analyzing: {failure['test']}")
    ...
    async def run_performance_analysis(self):
        ...
        test_queries = [
            ...
            ("Complex", "refactor this module to use async/await with proper error handling and type hints"),
            ("Trading", "optimize options pricing calculation for real-time performance"),
        ...
        for difficulty, query in test_queries:
            ...
            print(f"  ⏱️  {elapsed:.2f}s | 🎯 {solution.confidence:.0%} | 🔄 {solution.metrics.simulations_performed} sims")
        
        # Analyze results
        print("\n📈 Performance Summary:")
        print(f"{'Difficulty':<10} {'Time (s)':<10} {'Confidence':<12} {'GPU %':<8} {'Memory MB':<10}")
        print("-" * 60)
        
        for r in results:
            print(f"{r['difficulty']:<10} {r['time']:<10.2f} {r['confidence']:<12.0%} {r['gpu_usage']:<8.1f} {r['memory_mb']:<10.0f}")
