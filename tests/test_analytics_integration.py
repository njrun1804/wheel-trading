...
class TestIVSurfaceAnalyzer:
    ...
    def test_term_structure_detection(self):
        ...
        chain = {
            'spot_price': 25.0,
            'puts': [
                {'dte': 30, 'implied_volatility': 0.80, 'expiration': datetime.now() + timedelta(30)},
                {'dte': 60, 'implied_volatility': 0.70, 'expiration': datetime.now() + timedelta(60)},
                {'dte': 90, 'implied_volatility': 0.65, 'expiration': datetime.now() + timedelta(90)}
            ]
        }
