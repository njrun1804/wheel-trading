...
class NeuralBackendManager:
        ...
        for backend_name, init_func in backends_to_test:
            try:
                ...
                self.available_backends[backend_name] = BackendInfo(
                    name=backend_name, available=False, error=str(e)
                )
    ...
    def _init_mlx_backend(self) -> BackendInfo:
        ...
        try:
            ...
            if hasattr(mx, "metal") and mx.metal.is_available():
                ...
                return BackendInfo(
                    name="mlx", available=False, error="Metal GPU not available"
                )

        except ImportError as e:
            return BackendInfo(
                name="mlx", available=False, error=f"MLX not installed: {e}"
            )
    ...
    def _init_torch_mps_backend(self) -> BackendInfo:
        ...
        try:
            ...
            if torch.backends.mps.is_available():
                ...
                return BackendInfo(
                    name="torch_mps", available=False, error="MPS not available"
                )

        except ImportError as e:
            return BackendInfo(
                name="torch_mps", available=False, error=f"PyTorch not installed: {e}"
            )
    ...
    def _init_torch_cpu_backend(self) -> BackendInfo:
        ...
        try:
            ...
            return BackendInfo(
                name="torch_cpu", available=False, error=f"PyTorch not installed: {e}"
            )
    ...
    def validate_tensor_compatibility(
        self, data: Any, target_backend: str = None
    ) -> bool:
        """Validate tensor compatibility with backend."""
        backend = target_backend or self.current_backend

        if backend == "mlx":
            return isinstance(data, np.ndarray) and data.dtype in [
                np.float32,
                np.float16,
            ]
    ...
    def get_backend_status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "available": {
                name: info.__dict__ for name, info in self.available_backends.items()
            },
            "total_backends": len(self.available_backends),
            "functional_backends": len(
                [info for info in self.available_backends.values() if info.available]
            ),
        }


# Global instance
_backend_manager: NeuralBackendManager | None = None
...
if __name__ == "__main__":
    ...
    print(
        f"Functional backends: {status['functional_backends']}/{status['total_backends']}"
    )
