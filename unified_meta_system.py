...
class UnifiedExecutionMonitor:
    ...
    def monitor_command(self, command: str, cwd: str = ".") -> ExecutionResult:
        ...
        try:
            ...
            self.meta_prime.observe('command_execution', {
            ...
            self.meta_prime.observe('command_timeout', {
                'command': command[:100],
                'timeout_seconds': 300
            })
...
class UnifiedMistakeDetector:
    ...
    def detect_syntax_mistakes(self, file_path: Path) -> Optional[DetectedMistake]:
    ...
    def detect_runtime_mistakes(self, execution_result: ExecutionResult) -> List[DetectedMistake]:
        ...
        if not execution_result.success and execution_result.error_patterns:
            for pattern in execution_result.error_patterns:
                mistake = DetectedMistake(
                    ...
                    severity='warning' if 'warning' in execution_result.stderr.lower() else 'error',
                ...
                self.meta_prime.observe('runtime_mistake_detected', {
                    'type': pattern,
...
class UnifiedClaudeCodeMonitor(FileSystemEventHandler):
    ...
    def _record_claude_edit(self, file_path: Path, edit_type: str):
        ...
        try:
            ...
            self.meta_prime.observe('claude_monitor_error', {
                'error': str(e),
                'file': str(file_path),
                'edit_type': edit_type
            })
...
class AutoCorrector:
    ...
    def _fix_import_error(self, failed_result: ExecutionResult) -> bool:
        ...
        install_result = self.execution_monitor.monitor_command(f"pip install {module_name}")
    ...
    def _fix_dependency_error(self, failed_result: ExecutionResult) -> bool:
        ...
        if Path("requirements.txt").exists():
            ...
            install_result = self.execution_monitor.monitor_command("pip install -r requirements.txt")
...
class UnifiedMetaSystem:
    ...
    async def _monitor_claude_activity(self) -> Dict[str, Any]:
        ...
        activity = {
            ...
            "recent_edits": [edit for edit in self.claude_monitor.claude_edits 
                           if time.time() - edit.timestamp < 300],  # Last 5 minutes
            "modification_rate": len(self.claude_monitor.claude_edits) / max(1, self.loop_count)
        }
    ...
    async def _detect_recent_mistakes(self) -> List[Dict[str, Any]]:
        ...
        for edit in recent_edits[-3:]:  # Last 3 files
            if edit.edit_type in ['created', 'modified']:
                ...
                if file_path.exists():
                    ...
                    syntax_mistake = self.mistake_detector.detect_syntax_mistakes(file_path)
                    if syntax_mistake:
                        mistakes.append({
                            "file": str(file_path),
                        ...
                        if syntax_mistake.mistake_type in ['import_error', 'dependency_error']:
                            ...
                            corrected = self.auto_corrector.attempt_auto_correction(mock_result)
                            if corrected:
        ...
        self.meta_prime.observe("mistake_detection_cycle", {
            "files_checked": len(recent_edits),
    ...
    async def _measure_effectiveness(self) -> Dict[str, Any]:
        ...
        if self.auto_corrector.correction_attempts:
            success_rate = len([c for c in self.auto_corrector.correction_attempts if c['correction_made']]) / len(self.auto_corrector.correction_attempts)
        ...
        effectiveness = {
            ...
            "correction_success_rate": effectiveness_factors[2] if len(effectiveness_factors) > 2 else None
        }
    ...
    async def _trigger_meta_evolution(self, claude_activity: Dict[str, Any], 
        ...
        if len(claude_activity["recent_edits"]) > 3 and effectiveness["overall_score"] < 0.6:
            should_evolve = True
    ...
    def get_status_report(self) -> str:
        ...
        recent_mistakes = [m for m in self.mistake_detector.detected_mistakes 
                         if time.time() - m.timestamp < 3600]  # Last hour
        
        recent_edits = [e for e in self.claude_monitor.claude_edits 
                       if time.time() - e.timestamp < 3600]  # Last hour
        
        recent_executions = [e for e in self.execution_monitor.execution_history 
                           if time.time() - e.timestamp < 3600]  # Last hour
