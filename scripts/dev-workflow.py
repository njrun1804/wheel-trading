...
class DevWorkflowManager:
    ...
    def run_command(
        self, cmd: List[str], capture_output: bool = False, cwd: Optional[Path] = None
    ) -> Tuple[int, str]:
    ...
    def check_environment(self) -> bool:
        ...
        if sys.version_info < (3, 8):
            ...
            self.print_status(
                "success", f"Python {sys.version_info.major}.{sys.version_info.minor} OK"
            )
    ...
    def run_tests(self, pattern: str = None, verbose: bool = False, coverage: bool = False) -> bool:
    ...
    def run_integration_tests(self) -> bool:
        ...
        exit_code, _ = self.run_command(["python", "-m", "pytest", "-m", "integration", "-v"])
...
Examples:
  ...
  ./dev-workflow.py stats          # Show project statistics
    ...
    parser.add_argument("-c", "--coverage", action="store_true", help="Include coverage analysis")
    ...
    if args.command == "check":
        ...
        success = manager.run_tests(
            pattern=args.keyword, verbose=args.verbose, coverage=args.coverage
        )
