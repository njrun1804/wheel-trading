...
from src.unity_wheel.data_providers.databento.ml_enhanced_collector import (
    MLEnhancedDataCollector,
)
...
class IntelligentScheduler:
    ...
    def __init__(self, db_path: str = "data/wheel_trading_optimized.duckdb"):
        ...
        self.collection_rules = {
            "regular": {
                "interval_minutes": 30,
                "conditions": lambda: self._is_market_hours()
                and not self._is_high_vol(),
            },
            "high_volatility": {
                "interval_minutes": 15,
    ...
    def _show_storage_stats(self):
        ...
        try:
            ...
            if stats["ml_data.market_snapshots"] > 100:
                logger.info(
                    f"   Growth rate: ~{db_size / max(1, stats['ml_data.market_snapshots'] / 48):.1f}MB/day"
                )
...
"""
    ...
    print(
        "To run as a service, create: /etc/systemd/system/wheel-ml-collection.service"
    )
...
async def main():
    ...
    parser.add_argument(
        "--service", action="store_true", help="Show service configuration"
    )
