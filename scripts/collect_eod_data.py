...
class EODDataCollector:
    ...
    def __init__(self):
        ...
        if not self.fred_key:
            self.fred_key = os.environ.get("FRED_API_KEY") or os.environ.get("OFRED_API_KEY")
            
        if not self.databento_key:
            raise ValueError("Databento API key not found in SecretManager or environment")
        if not self.fred_key:
    ...
    def _store_options_data(self, definitions, quotes, data_date):
        ...
        merged = pd.merge(
            quotes,
            definitions[['instrument_id', 'strike_price', 'expiration', 'instrument_class']],
            on='instrument_id',
            how='inner'
        )
        ...
        merged['year_month'] = merged['expiration'].dt.year * 100 + merged['expiration'].dt.month
        
        # Get bid/ask from levels
        merged['bid'] = merged['levels'].apply(lambda x: x[0]['bid_px'] / 1e9 if x else 0)
        merged['ask'] = merged['levels'].apply(lambda x: x[0]['ask_px'] / 1e9 if x else 0)
        merged['bid_size'] = merged['levels'].apply(lambda x: x[0]['bid_sz'] if x else 0)
        merged['ask_size'] = merged['levels'].apply(lambda x: x[0]['ask_sz'] if x else 0)
        ...
        for _, row in merged.iterrows():
            if row['bid'] > 0 and row['ask'] > 0:  # Valid quotes only
                records.append({
                    ...
                    'strike': float(row['strike_price']) / 1000,  # Convert from scaled
                ...
                try:
                    ...
                    async with session.get(base_url, params=params) as response:
                        if response.status == 200:
                            ...
                            logger.info(f"Updated {name}: {len(observations)} observations")
    ...
    async def calculate_ml_features(self):
        ...
        try:
            ...
            if vix and unity_price:
                ...
                logger.info(f"Stored ML features - VIX: {vix_level:.2f}, Regime: {regime}")
                
                # Refresh materialized view
                self.conn.execute("REFRESH MATERIALIZED VIEW analytics.wheel_opportunities_mv")
