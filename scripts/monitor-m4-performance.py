        ...
        for proc in psutil.process_iter(['pid', 'name', 'cpu_percent', 'memory_percent']):
            try:
                if 'python' in proc.info['name'].lower():
                    ...
                    if 'wheel' in cmdline or 'run.py' in cmdline or 'orchestrate' in cmdline:
                        processes.append({
                            'pid': proc.info['pid'],
                            'name': proc.info['name'],
        ...
        with TERM.fullscreen(), TERM.hidden_cursor():
            while self.running:
                ...
                with self.data_lock:
                    ...
                    cpu_color = TERM.red if cpu_current > 80 else TERM.yellow if cpu_current > 60 else TERM.green
                    ...
                    mem_color = TERM.red if mem_current > 80 else TERM.yellow if mem_current > 60 else TERM.green
                    
                    print(TERM.bold('Memory:'), f'{mem_current:.1f}% ({mem_info.used / 1024**3:.1f}GB / {mem_info.total / 1024**3:.1f}GB)')
                    ...
                    gpu_color = TERM.red if gpu_current > 80 else TERM.yellow if gpu_current > 60 else TERM.green
                    ...
                    temp_color = TERM.red if temp_current > 80 else TERM.yellow if temp_current > 65 else TERM.green
                    
                    print(TERM.bold('CPU Temperature:'), temp_color(f'{temp_current:.1f}°C'))
                ...
                if processes:
                    ...
                    for proc in processes:
                        cpu_color = TERM.red if proc['cpu'] > 50 else TERM.yellow if proc['cpu'] > 25 else TERM.normal
                        print(f"{proc['pid']:<8} {cpu_color}{proc['cpu']:<8.1f}{TERM.normal} {proc['memory']:<8.1f} {proc['cmdline']}")
                else:
