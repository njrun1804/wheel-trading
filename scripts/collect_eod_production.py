...
class EODCollector:
    ...
    def _load_credentials(self):
        ...
        try:
            ...
            self.fred_key = secret_mgr.get_secret(
                "ofred_api_key"
            ) or secret_mgr.get_secret("fred_api_key")
    ...
    async def collect_unity_stock(self):
        ...
        try:
            ...
            end_dt = datetime.combine(data_date, datetime.min.time()).replace(
                tzinfo=UTC
            )
            start_dt = end_dt - timedelta(days=5)  # Get 5 days of data

            logger.info(
                f"Fetching stock data from {start_dt.date()} to {end_dt.date()}"
            )

            # Log API request details
            logger.info(
                "Databento API request: dataset=EQUS.MINI, symbol=U, schema=ohlcv-1d"
            )
            ...
            if len(df) > 0:
                sample = df.iloc[0]
                logger.debug(
                    f"Sample raw data: open={sample.get('open')}, close={sample.get('close')}, volume={sample.get('volume')}"
                )
                logger.debug(
                    f"Price range before scaling: {df['close'].min()} - {df['close'].max()}"
                )
    ...
    async def collect_unity_options(self):
        ...
        try:
            ...
            data_dt = datetime.combine(data_date, datetime.min.time()).replace(
                tzinfo=UTC
            )
            ...
            logger.debug(
                f"Raw strike_price values: min={def_df['strike_price'].min()}, max={def_df['strike_price'].max()}"
            )
            ...
            logger.debug(
                f"Strike range after scaling: {def_df['strike_unscaled'].min():.2f} - {def_df['strike_unscaled'].max():.2f}"
            )
            logger.debug(
                f"Looking for strikes between {min_strike:.2f} and {max_strike:.2f}"
            )
            ...
            logger.info(
                f"Filtered to {len(filtered_defs)} options within ±35% moneyness"
            )
    ...
    def _store_options(self, definitions, quotes):
        ...
        quote_columns = []
        if (
            "bid_px_00" in latest_quotes.columns
            and "ask_px_00" in latest_quotes.columns
        ):
            ...
            logger.error(
                f"Cannot find bid/ask columns in: {latest_quotes.columns.tolist()}"
            )
            return 0
            ...
            try:
                ...
                if not is_valid:
                    ...
                    self.metrics["errors"].append(
                        f"Option validation failed: {error_msg}"
                    )
                    continue
                ...
                """,
                    [
                        ...
                        iv,
                        timestamp,
                        expiration.year * 100 + expiration.month,
                    ],
                )
                stored += 1
                    ...
                    async with session.get(url, params=params) as response:
                        if response.status == 200:
                                ...
                                if obs["value"] != "." and obs["value"]:
                                    try:
                                        ...
                                        (
                                            is_valid,
                                            error_msg,
                                        ) = self.validator.validate_fred_data(
                                            indicator=series_id,
                                        ...
                                        if not is_valid:
                                            logger.error(
                                                f"Invalid FRED data: {error_msg}"
                                            )
                                            continue

                                        # Store using our proper storage function
                                        if store_fred_observation(
                                            self.conn, series_id, date, value
                                        ):
                                            series_stored += 1
                            ...
                            if series_stored > 0:
                                ...
                                logger.info(
                                    f"✅ {name}: {latest['value']} ({series_stored} records)"
                                )
                            ...
                            error_msg = (
                                f"FRED API error for {name}: HTTP {response.status}"
                            )
                            logger.error(error_msg)
                            self.metrics["errors"].append(error_msg)
    ...
    def calculate_ml_features(self):
        ...
        try:
            ...
            self._get_latest_fred_value("TEDRATE")
            self._get_latest_fred_value("DGS10")
            ...
            try:
                self.conn.execute(
                    "DROP VIEW IF EXISTS analytics.wheel_opportunities_mv"
                )
                self.conn.execute(
                    """
            ...
            if vix is not None:
                logger.info(
                    f"✅ ML Features: VIX={vix:.2f}, Regime={regime}, Unity Vol={unity_vol:.2%}"
                )
            else:
                logger.info(
                    f"✅ ML Features: VIX=N/A, Regime={regime}, Unity Vol={unity_vol:.2%}"
                )
