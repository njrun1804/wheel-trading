...
class DevConsole:
    ...
    def test_black_scholes(
        self, spot=35.0, strike=35.0, time_to_expiry=0.123, risk_free_rate=0.05, volatility=0.25
    ):
        ...
        for option_type in ["call", "put"]:
            ...
            print(
                f"   {option_type.title()}: ${result.value:.3f} (confidence: {result.confidence:.1%})"
            )
...
class DevUtils:
    ...
    def validate_environment(self):
        ...
        required_paths = ["src/unity_wheel", "config.yaml", "tests"]
