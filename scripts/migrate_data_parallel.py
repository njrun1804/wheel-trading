...
class DataMigrator:
    ...
    def log_migration(
        self, table_name: str, rows: int, duration: float, status: str = "completed"
    ):
    ...
    def migrate_market_data_parallel(self):
        ...
        logger.info(
            f"✅ Market data migration complete: {total_rows:,} rows in {duration:.1f}s"
        )
    ...
    def _process_market_chunk(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "symbol" in df.columns and df["symbol"].str.len().iloc[0] > 5:
            ...
            df = (
                df.groupby(["underlying_symbol", "ts_event"])
                .agg(
                    {
                        "open": "mean",
                        "high": "max",
    ...
    def migrate_options_data_parallel(self):
        ...
        sources = [
            ...
            (
                ...
                "SELECT * FROM options_data WHERE bid > 0",
            ),
            (
                ...
                try:
                    ...
                    if count > 0:
                        ...
                        for offset in range(0, count, BATCH_SIZE):
                            ...
                            logger.info(
                                f"    Processed {min(offset + BATCH_SIZE, count):,}/{count:,} rows"
                            )
        ...
        duration = time.time() - start_time
        logger.info(
            f"✅ Options data migration complete: {total_rows:,} rows in {duration:.1f}s"
        )
        logger.info(f"   Speed: {total_rows/duration:,.0f} rows/sec")
