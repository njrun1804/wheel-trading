...
class CollectionMonitor:
    ...
    def check_recent_collections(self, days=7):
        ...
        if stock_data:
            ...
            print(
                f"   Latest: {stock_data[0][0]} - Close: ${stock_data[0][1]:.2f}, Volume: {volume:,}"
            )
        ...
        if options_data:
            ...
            print(
                f"   Latest: {latest[0]} - {latest[1]} contracts, {latest[2]} expirations"
            )
    ...
    def check_ml_features(self):
        ...
        if features:
            ...
            print(
                f"   Realized Vol: {latest[3]:.2%}"
                if latest[3]
                else "   Realized Vol: N/A"
            )
                    ...
                    if line.strip() and not line.startswith("#"):
                        try:
                            ...
                            if age.total_seconds() < 3600:
                                print(
                                    f"   Last activity: {int(age.total_seconds() / 60)} minutes ago"
                                )
                            else:
                                print(
                                    f"   Last activity: {int(age.total_seconds() / 3600)} hours ago"
                                )
                            break
                        except:
                            pass
...
def main():
    ...
    parser.add_argument(
        "--quality", action="store_true", help="Show detailed quality metrics"
    )
    ...
    try:
        if args.watch:
            ...
            while True:
                ...
                print(
                    f"{BLUE}Unity Data Collection Monitor - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}{RESET}"
                )
