...
class CodebaseAnalyzer:
    ...
    def init_database(self):
        ...
        self.conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_import_from ON imports(from_module)"
        )
        self.conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_import_to ON imports(to_module)"
        )
    ...
    def analyze_codebase(self):
        ...
        py_files = [
            ...
            if not any(
                p in str(f)
                for p in [".venv", "venv", "__pycache__", ".git", "build", "dist"]
            )
        ]
    ...
    def _print_summary(self):
        ...
        print(
            f"Total lines of code: {sum(m.lines_of_code for m in self.file_metrics.values()):,}"
        )
        ...
        print(
            f"  - Circular dependencies: {len(list(nx.simple_cycles(self.import_graph)))}"
        )
        ...
        if self.file_metrics:
            ...
            print(
                f"  - Files with high complexity (>10): {sum(1 for m in self.file_metrics.values() if m.cyclomatic_complexity > 10)}"
            )
...
def main():
    ...
    with open(startup_file, "w") as f:
        json.dump(
            {
                "preload_modules": list(analyzer.imports.keys())[
                    :50
                ],  # Top 50 most imported
                "hot_files": [
                    f
                    for f, m in analyzer.file_metrics.items()
                    if m.cyclomatic_complexity > 10
                ],
                "critical_symbols": [
                    name
                    for name in analyzer.symbols
                    if "Strategy" in name or "Manager" in name
                ],
                "timestamp": time.time(),
            },
            f,
