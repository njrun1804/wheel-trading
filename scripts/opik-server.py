...
def get_traces():
    ...
    try:
        ...
        return jsonify(
            {"traces": recent_traces, "count": len(recent_traces), "total": len(traces)}
        )
    except Exception as e:
        return jsonify({"status": "error", "message": str(e)}), 400
    ...
    try:
        ...
        for op in operations:
            ...
            operations[op]["avg_duration"] = (
                operations[op]["total_duration"] / count if count > 0 else 0
            )
            operations[op]["error_rate"] = (
                operations[op]["errors"] / count if count > 0 else 0
            )
