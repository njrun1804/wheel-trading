...
class SimpleMigrator:
    ...
    def _migrate_market_data(self):
        ...
        try:
            ...
            if 'market_data' in [t[0] for t in tables]:
                count = source_conn.execute("SELECT COUNT(*) FROM market_data").fetchone()[0]
                ...
                if count > 0:
                    ...
                    df['volatility_20d'] = df.groupby('symbol')['daily_return'].transform(
                        lambda x: x.rolling(window=20, min_periods=1).std() * (252**0.5)
                    )
                    df['volatility_60d'] = df.groupby('symbol')['daily_return'].transform(
                        lambda x: x.rolling(window=60, min_periods=1).std() * (252**0.5)
                    )
                    ...
                    target_conn.execute("INSERT INTO market.price_data SELECT * FROM df")
                    target_conn.close()
    ...
    def _migrate_options_data(self):
        ...
        sources = [
            ...
            ("data/cache/data/wheel_trading_optimized.duckdb", ["options_data", "option_chains"]),
            (str(Path.home() / "data/wheel_trading_optimized.duckdb"), 
             ["unity_options_daily", "unity_options_comprehensive", "unity_options_highperf"])
        ]
                        ...
                        if count > 0:
                            ...
                            for offset in range(0, count, chunk_size):
                                ...
                                if len(df) > 0:
                                    # Insert into target
                                    target_conn = duckdb.connect(self.target_db)
                                    target_conn.execute("INSERT INTO options.contracts SELECT * FROM df")
                                    target_conn.close()
                                    total_rows += len(df)
                                
                                logger.info(f"      Processed {min(offset + chunk_size, count)}/{count} rows")
    ...
    def _create_materialized_views(self):
        ...
        count = conn.execute("SELECT COUNT(*) FROM analytics.wheel_opportunities_mv").fetchone()[0]
    ...
    def show_summary(self):
        ...
        for table, name in tables:
            try:
                ...
                if count > 0:
                    # Get date range
                    if 'timestamp' in conn.execute(f"SELECT * FROM {table} LIMIT 0").description:
                        date_col = 'timestamp'
                    elif 'date' in conn.execute(f"SELECT * FROM {table} LIMIT 0").description:
                        date_col = 'date'
                    else:
                        continue
                        
                    min_date, max_date = conn.execute(f"""
