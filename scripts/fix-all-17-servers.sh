...
echo -e "${GREEN}=== Fixing All 17 MCP Servers ===${NC}"

# 1. Fix the one broken server: duckdb
echo -e "\n${YELLOW}1. Fixing duckdb server configuration...${NC}"

# The duckdb MCP server uses a console script, not -m
python3 << 'EOF'
import json
...
# Fix duckdb to use the correct command
config["mcpServers"]["duckdb"] = {
    "transport": "stdio",
    "command": "/Users/mikeedwards/.pyenv/shims/mcp-server-duckdb",
    "args": [
        "--db-path",
...
print("✓ Fixed duckdb server configuration")
EOF
...
# Core MCP
/Users/mikeedwards/.pyenv/shims/pip install --quiet mcp

# Python module servers
/Users/mikeedwards/.pyenv/shims/pip install --quiet mcp-server-stats mcp-server-duckdb mcp-py-repl

# Dependencies for community servers
/Users/mikeedwards/.pyenv/shims/pip install --quiet scikit-learn mlflow pandas numpy

echo "✓ All Python dependencies installed"

# 3. Ensure NPX cache is primed (optional but speeds up first run)
echo -e "\n${YELLOW}3. Pre-caching NPX packages...${NC}"
echo "This may take a moment on first run..."

# Just check if npx packages exist, don't actually run them
npx_packages=(
...
for pkg in "${npx_packages[@]}"; do
    ...
    if npx -y "$pkg" --version >/dev/null 2>&1; then
        ...
        echo " (will download on first use)"
    fi
done

# 4. Create comprehensive test script
echo -e "\n${YELLOW}4. Creating comprehensive test script...${NC}"
cat > test-all-17-servers.py << 'EOF'
#!/usr/bin/env python3
...
if __name__ == "__main__":
    main()
EOF
chmod +x test-all-17-servers.py

# 5. Run the test
echo -e "\n${YELLOW}5. Testing all servers...${NC}"
cd "/Users/mikeedwards/Library/Mobile Documents/com~apple~CloudDocs/pMike/Wheel/wheel-trading"
./test-all-17-servers.py

echo -e "\n${GREEN}=== Fix Complete ===${NC}"
echo "All servers should now be working!"
