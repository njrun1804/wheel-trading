...
class IntradayCollector:
    ...
    async def collect_snapshot(self):
        ...
        try:
            ...
            logger.info(f"✅ Snapshot {self.metrics['snapshots']} collected successfully")
    ...
    async def run_once(self):
        ...
        logger.info(f"Collection complete in {duration:.1f}s - Snapshots: {self.metrics['snapshots']}, Errors: {len(self.metrics['errors'])}")
    ...
    async def run_continuous(self, interval_minutes=15):
        ...
        logger.info(f"Starting continuous intraday collection (every {interval_minutes} minutes)")
