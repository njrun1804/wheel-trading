...
REPLACEMENTS = [
    ...
    (r'portfolio_value\s*=\s*100000', 'portfolio_value = config.trading.portfolio_value'),
    (r'portfolio_value\s*=\s*200000', 'portfolio_value = config.trading.portfolio_value'),
    ...
    (r'max_position_size\s*=\s*0\.\d+', 'max_position_size = config.trading.max_position_size'),
    ...
    (r'["\']~/\.wheel_trading/cache/wheel_cache\.duckdb["\']', 'config.storage.database_path'),
    (r'["\']wheel_cache\.duckdb["\']', 'Path(config.storage.database_path).name'),
    
    # Config file replacements
    (r'["\']config\.yaml["\']', 'os.getenv("WHEEL_CONFIG_PATH", "config/unified.yaml")'),
    (r'["\']config_unified\.yaml["\']', 'os.getenv("WHEEL_CONFIG_PATH", "config/unified.yaml")'),
]

# Files to add config import to
CONFIG_IMPORT = "from unity_wheel.config.unified_config import get_config\nconfig = get_config()\n"
...
def main():
    ...
    for py_file in project_root.rglob("*.py"):
        ...
        if modified:
            ...
            print(f"✅ Modified {py_file.relative_to(project_root)} ({replacements} replacements)")
