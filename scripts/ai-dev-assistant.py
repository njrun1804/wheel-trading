...
class AIDevAssistant:
    ...
    def execute_command(self, cmd: List[str], cwd: Optional[Path] = None) -> Dict[str, Any]:
        ...
        try:
            result = subprocess.run(
                cmd, cwd=cwd or self.project_root, capture_output=True, text=True, check=False
            )
    ...
    def validate_system_health(self) -> Dict[str, Any]:
        ...
        required_paths = ["src/unity_wheel", "tests", "config.yaml", "pyproject.toml"]
        ...
        try:
            ...
            health_report["checks"]["configuration"] = {"status": "fail", "error": str(e)}
    ...
    def analyze_code_quality(self) -> Dict[str, Any]:
        ...
        black_result = self.execute_command(
        ...
        # Flake8 linting
        flake8_result = self.execute_command(
            ["python", "-m", "flake8", "--format=json", "src", "tests"]
        )
        flake8_issues = []
        ...
        # MyPy type checking
        mypy_result = self.execute_command(
            ["python", "-m", "mypy", "--json-report", "mypy-report", "src"]
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="AI Development Assistant for Unity Wheel Trading Bot"
    )
    parser.add_argument(
        "action",
        choices=["health", "quality", "performance", "coverage", "optimize", "fix", "export"],
