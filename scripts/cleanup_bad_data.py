...
class DataCleanup:
    ...
    def clean_zero_prices(self):
        ...
        if zero_records:
            ...
            for date, symbol, close in zero_records[:5]:  # Show first 5
            ...
            if not self.dry_run:
                ...
                logger.info(f"DRY RUN: Would delete {len(zero_records)} zero price records")
    ...
    def clean_wrong_price_range(self):
        ...
        if wrong_prices:
            ...
            if not self.dry_run:
                ...
                logger.info(f"DRY RUN: Would delete {len(wrong_prices)} wrong price records")
    ...
    def clean_zero_option_quotes(self):
        ...
        if count > 0:
            ...
            if not self.dry_run:
                ...
                logger.info(f"DRY RUN: Would delete {count} zero bid/ask option records")
    ...
    def verify_cleanup(self):
        ...
        logger.info(f"Stock data after cleanup:")
    ...
    def show_recent_data(self):
        ...
        for date, open_p, high, low, close, volume in recent:
            ...
            logger.info(f"  {date}: O={o_str} H={h_str} L={l_str} C={c_str} Vol={vol_str}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Clean up bad data from Unity Wheel database")
    parser.add_argument(
        "--execute", 
        action="store_true",
