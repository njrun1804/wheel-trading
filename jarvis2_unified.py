...
from jarvis2.search.mcts_simple import SimplifiedMCTS
...
class UnifiedJarvis2:
    ...
    def __init__(self, enable_meta: bool = False):
        ...
        print(f"🧠 Unified Jarvis2 initialized")
    ...
    async def search_solutions(self, problem: str, max_iterations: int = 1000) -> List[Dict[str, Any]]:
...
async def main():
    ...
    parser.add_argument("command", choices=[
        "generate", "analyze", "optimize", "search", "status", "interactive"
    ...
    parser.add_argument("--optimization-type", default="performance", 
    ...
    try:
        if args.command == "generate":
            ...
            print("Commands: generate <prompt>, analyze [path], optimize <code>, search <problem>, status, quit")
            ...
            while True:
                try:
                    ...
                    if cmd == "generate" and arg:
                        ...
                        status = jarvis.get_system_status()
                        print(f"Status: {status['neural_backend']} backend, {status['cpu_cores_allocated']} cores")
                    else:
                        print("Invalid command. Use: generate, analyze, optimize, search, status, quit")
