...
from .exceptions import SecretNotFoundError, SecretConfigError, SecretProviderError
...
class SecretManager:
    ...
    def __init__(
        self,
        provider: Optional[Union[SecretProvider, str]] = None,
        **kwargs: Any
    ):
    ...
    def _detect_provider(self) -> SecretProvider:
        ...
        if os.environ.get("GCP_PROJECT_ID") or os.environ.get("GOOGLE_APPLICATION_CREDENTIALS"):
            try:
                ...
                logger.warning("GCP environment detected but google-cloud-secret-manager not installed")
