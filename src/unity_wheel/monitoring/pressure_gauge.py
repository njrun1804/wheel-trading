...
class AdaptiveMemoryMonitor:
    ...
    def __init__(
        ...
        pressure_callbacks: list[Callable] | None = None,
    ):
    ...
    def _take_reading(self) -> PressureReading:
        ...
        available_pressure = max(
            0, 1.0 - (mem.available / (self.thresholds["available_min_gb"] * 1024**3))
        )

        # Weighted average
        pressure_level = (
            memory_pressure * 0.5 + swap_pressure * 0.3 + available_pressure * 0.2
        )
    ...
    def get_stats(self) -> dict:
        ...
        return {
            ...
            "throttle_events": sum(
                1
                for e in self._pressure_events
                if e["fanout_reduced_to"] < self._base_fanout
            ),
        }
