...
class FilesystemIndex:
    ...
    def __init__(self, project_root: str, index_path: str = None):
        ...
        self.index_path = index_path or str(
            self.project_root / ".mcp_index" / "filesystem.duckdb"
        )
        self.conn = None
    ...
    async def _process_batch(self, files: list[Path]) -> list[tuple]:
        ...
        tasks = [
            loop.run_in_executor(self.executor, self._analyze_file, f) for f in files
        ]
    ...
    def _analyze_file(self, file_path: Path) -> tuple | None:
        ...
        try:
            ...
            complexity = (
                function_count * 2 + class_count * 3 + len(content.split("\n")) // 50
            )
    ...
    def search_files_indexed(
        self, query: str, limit: int = 250
    ) -> list[dict[str, Any]]:
        """
        ...
        results = self.conn.execute(
            sql, [escaped_query, escaped_query, limit]
        ).fetchall()
...
def search_codebase(
    project_root: str, query: str, limit: int = 250
) -> list[dict[str, Any]]:
