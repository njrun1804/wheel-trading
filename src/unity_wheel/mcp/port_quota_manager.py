...
class PortQuotaManager:
    ...
    @contextmanager
    def allocate_port(self, service: str, preferred_port: int | None = None):
    ...
    def _find_available_port(self, start=15000, end=16000) -> int | None:
    ...
    def cleanup_stale_allocations(self):
        ...
        with self._lock:
            ...
            stale_ports = [
                port
                for port, alloc in self._allocations.items()
...
class QuotaAwareSocket(socket.socket):
    ...
    def bind(self, address):
        ...
        if isinstance(address, tuple) and len(address) >= 2:
            _host, _port = address[0], address[1]
...
if __name__ == "__main__":
    ...
    async def test_allocation():
        ...
        console.logger.info(
            "[green]Testing port allocation with quota management...[/green]"
        )
        ...
        # Cleanup test
        manager.cleanup_stale_allocations()
