...
class IncrementalIndexer(FileSystemEventHandler):
    ...
    def __init__(self, project_root: str, index_path: str | None = None):
        ...
        self.index_path = index_path or str(
            self.project_root / ".mcp_index" / "incremental.duckdb"
        )
        self.conn = None
            ...
            "tasks": [
                {
                    ...
                    "args": [
                        "-c",
                        "from unity_wheel.mcp.incremental_indexer import notify_file_save; notify_file_save('${file}')",
                    ],
...
def notify_file_save(file_path: str) -> None:
    ...
    try:
        ...
        if socket_path.exists():
            ...
            sock.send(json.dumps({"type": "file_save", "path": file_path}).encode())
            sock.close()
