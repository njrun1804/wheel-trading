...
from .dynamic_chunking import ChunkedFileReader, DynamicChunker
...
class EmbeddingPipeline:
    ...
    def __init__(
        self, cache_path: Path | None = None, embedding_func: Callable | None = None
    ):
        """
    ...
    async def _process_chunk(
        self, chunk: dict[str, Any], file_path: str, force_refresh: bool
    ) -> dict[str, Any]:
        ...
        if embedding is not None:
            ...
            return {**chunk, "embedding": embedding, "cached": True}
        else:
            ...
            self.cache.store_embedding(
                content, file_path, start_line, end_line, embedding, token_count
            )
    ...
    async def embed_search_results(
        self, pattern: str, directory: str, context_lines: int = 5
    ) -> list[dict[str, Any]]:
        """
        ...
        if not files:
            logger.info(
                f"No matches found for pattern '{pattern}' in directory '{directory}'"
            )
            return []
        ...
        for file_path in files:
            ...
            for chunk in chunks:
                result = await self._process_chunk(
                    chunk, file_path, force_refresh=False
                )
    ...
    async def warmup_cache(self, directory: str, file_pattern: str = "*.py"):
        ...
        logger.info(
            f"Warming up cache for {len(files)} files in directory '{directory}'"
        )
...
if __name__ == "__main__":
    ...
    async def test_pipeline():
        ...
        search_results = await pipeline.embed_search_results(
            "async def", Path(__file__).parent
        )
