...
class DynamicChunker:
    ...
    def calculate_optimal_chunk_size(self, file_path: str, content: str) -> int:
        ...
        if self.performance_history:
            similar_metrics = [
                m
                for m in self.performance_history[-20:]  # Last 20 chunks
            ...
            if similar_metrics:
                ...
                best_metric = min(
                    similar_metrics, key=lambda m: m.processing_time / m.chunk_size
                )
                base_chunk = int(base_chunk * 0.7 + best_metric.chunk_size * 0.3)

        # Apply bounds
        chunk_size = max(
            self.strategy.min_chunk_size, min(base_chunk, self.strategy.max_chunk_size)
        )
    ...
    def create_semantic_chunks(
        ...
        for i, line in enumerate(lines):
            ...
            is_boundary = line.strip().startswith(("def ", "class ", "async def ")) or (
                indent == 0 and line.strip() and current_chunk
            )  # Top-level code
            ...
            should_split = (
                is_boundary
                and current_tokens > self.strategy.target_tokens * 0.5
                or current_tokens + line_tokens > self.strategy.max_tokens
                or len(current_chunk) > self.strategy.max_chunk_size
            )
    ...
    def get_performance_stats(self) -> dict[str, Any]:
        ...
        tokens_per_second = sum(
            m.token_count / m.processing_time for m in successful
        ) / len(successful)
...
class ChunkedFileReader:
    ...
    def read_file_chunked(
        self, file_path: str, semantic: bool = True
    ) -> list[dict[str, Any]]:
    ...
    def read_with_ripgrep(
        self, pattern: str, file_path: str, context_lines: int = 3
    ) -> list[dict[str, Any]]:
...
if __name__ == "__main__":
    ...
    for chunk in chunks:
        print(
            f"Chunk {chunk['chunk_id']}/{chunk['total_chunks']}: "
