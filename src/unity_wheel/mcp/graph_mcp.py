...
class GraphMCP:
    ...
    def _setup_schema(self):
        ...
        self.conn.execute("CREATE INDEX IF NOT EXISTS idx_edge_from ON edges(from_symbol)")
        self.conn.execute("CREATE INDEX IF NOT EXISTS idx_edge_to ON edges(to_symbol)")
        self.conn.execute("CREATE INDEX IF NOT EXISTS idx_edge_type ON edges(edge_type)")
    ...
    def _parse_file(self, file_path: Path) -> Optional[Tuple[List[Symbol], List[Edge]]]:
        ...
        try:
            ...
            if relative_path in self.file_hashes and self.file_hashes[relative_path] == file_hash:
                return None
            ...
            for symbol in analyzer.symbols:
                ...
                self.conn.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    symbol_id, symbol.name, symbol.type, symbol.file_path,
                    symbol.line_number, symbol.column, symbol.parent,
                    symbol.docstring, symbol.signature
                ))
            ...
            for edge in analyzer.edges:
                self.conn.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?)
                """, (
                    edge.from_symbol, edge.to_symbol, edge.edge_type,
                    edge.file_path, edge.line_number
                ))
            ...
            self.conn.execute("""
                ...
                VALUES (?, ?, datetime('now'), ?, ?, ?)
            """, (
                relative_path, file_hash, parse_time,
                len(analyzer.symbols), len(analyzer.edges)
            ))
    ...
    def _build_graph_tables(self):
        ...
        for from_sym, to_sym, edge_type, _, _ in edges:
    ...
    async def incremental_update(self, changed_files: List[Path]):
        ...
        for file_path in changed_files:
            ...
            # Remove old data
            self.conn.execute("DELETE FROM symbols WHERE file_path = ?", [relative_path])
