...
class TokenBudgetAllocator:
        ...
        try:
            ...
            if file_path.endswith('.py'):
                try:
                    ...
                    complexity.class_count = len([n for n in ast.walk(tree) if isinstance(n, ast.ClassDef)])
                    complexity.function_count = len([n for n in ast.walk(tree) if isinstance(n, ast.FunctionDef)])
            ...
            complexity.complexity_score = min(1.0, (
    ...
    def allocate_budget(self, files: List[str], task_type: str = 'general') -> Dict[str, Dict]:
        ...
        if total_tokens <= self.safe_limit:
            ...
            for comp in complexities:
                if comp.complexity_score > 0.7 and comp.token_estimate < remaining_budget * 0.3:
                    allocations[comp.path] = {
                        'tokens': comp.token_estimate,
                        'strategy': 'full',
                        'mcp': self._select_mcp(comp, task_type)
    ...
    def _select_mcp(self, complexity: FileComplexity, task_type: str) -> str:
        ...
        for mcp, caps in self.mcp_capabilities.items():
            if file_size <= caps['max_file_size'] and task_type in caps['specialization']:
    ...
    def get_usage_report(self) -> Dict:
        ...
        return {
            ...
            'avg_files_per_operation': sum(h['total_files'] for h in recent) / len(recent),
            'avg_tokens_used': sum(h['total_tokens'] for h in recent) / len(recent),
            'strategy_distribution': self._aggregate_strategies(recent),
            'efficiency_rate': sum(h['allocated_files'] / h['total_files'] for h in recent) / len(recent)
        }
    ...
    def optimize_from_history(self):
        ...
        if report and report['efficiency_rate'] < 0.8:
            ...
            self.safe_limit = min(
                int(self.max_context_tokens * 0.9),
