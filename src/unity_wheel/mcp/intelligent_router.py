...
class IntelligentRouter:
    ...
    async def route_request(
        self, task_type: str, requirements: dict[str, Any], context: dict | None = None
    ) -> RoutingDecision:
    ...
    def _find_capable_mcps(self, task_type: str, requirements: dict) -> list[str]:
        ...
        for name, cap in self.capabilities.items():
            ...
            if all(req in cap.capabilities for req in required_capabilities):
                ...
                if (
                    task_type in cap.specializations
                    and cap.specializations[task_type] > 0.5
                    or not cap.specializations
                ):
                    capable.append(name)
    ...
    def _score_mcp(
        self, mcp_name: str, task_type: str, requirements: dict, context: dict | None
    ...
    def _record_routing_decision(
        self, decision: RoutingDecision, task_type: str, requirements: dict
    ):
    ...
    def record_performance(
        ...
        if mcp_name in self.metrics:
            ...
            if history:
                metrics.avg_latency_ms = sum(h["latency_ms"] for h in history) / len(
                    history
                )
                metrics.success_rate = sum(1 for h in history if h["success"]) / len(
                    history
                )
    ...
    def optimize_routing(self):
        ...
        for mcp_name, history in self.performance_history.items():
            ...
            if mcp_name in self.capabilities:
                ...
                # Weighted update
                cap.performance_score = cap.performance_score * 0.7 + success_rate * 0.3
                ...
                for task_type, records in task_performance.items():
                    task_success = sum(1 for r in records if r["success"]) / len(
                        records
                    )
                    if task_type in cap.specializations:
                        cap.specializations[task_type] = (
                            cap.specializations[task_type] * 0.8 + task_success * 0.2
                        )
...
async def demo():
    ...
    for task_type, requirements in examples:
        ...
        router.record_performance(
            decision.primary_mcp, task_type, decision.estimated_latency_ms * 1.1, True
        )
