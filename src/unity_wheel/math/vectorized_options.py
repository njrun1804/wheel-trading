...
@cached(
    cache_name="options",
    ttl_seconds=1800,
    key_func=lambda spot, strikes, time_to_expiry, risk_free_rate, volatility, option_type: 
        f"vec_bs_{hash(str(spot))}_k{hash(str(strikes))}_t{time_to_expiry:.4f}_r{risk_free_rate:.4f}_v{volatility:.4f}_{option_type}"
)
    ...
    if np.any(valid_mask):
        ...
        if np.any(active_mask):
            ...
            if np.any(nonzero_vol_mask):
                ...
                # Calculate d1 and d2
                d1 = (np.log(S_nz / K_nz) + (r_nz + 0.5 * sigma_nz**2) * T_nz) / (sigma_nz * sqrt_T_nz)
                d2 = d1 - sigma_nz * sqrt_T_nz
                
                # Calculate option values
                if option_type == "call":
                    option_values = (S_nz * norm.cdf(d1) - 
                                   K_nz * np.exp(-r_nz * T_nz) * norm.cdf(d2))
                else:
                    option_values = (K_nz * np.exp(-r_nz * T_nz) * norm.cdf(-d2) - 
                                   S_nz * norm.cdf(-d1))
...
@cached(
    ...
    ttl_seconds=1200,
    key_func=lambda spot, strikes, time_to_expiry, risk_free_rate, volatility, option_type:
        f"vec_greeks_{hash(str(spot))}_k{hash(str(strikes))}_t{time_to_expiry:.4f}_r{risk_free_rate:.4f}_v{volatility:.4f}_{option_type}"
)
...
def vectorized_wheel_analysis(
    ...
    valid_mask = (
        (np.abs(deltas - (-target_delta)) <= 0.10) &  # Delta within 10% of target
        (premium_pcts >= min_premium_pct) &             # Minimum premium
        (~np.isnan(prices))                             # Valid prices
    )
    ...
    analysis = {
        ...
        'summary': {
            ...
            'avg_annualized_yield': np.mean([r['annualized_yield'] for r in results]) if results else 0,
            'computation_time_ms': computation_time_ms
        },
        'market_data': {
...
def compare_scenario_analysis(
    ...
    prices_result = vectorized_black_scholes(
        spots, strike, expiration, rates, vols, option_type
    )
    
    greeks_result = vectorized_greeks(
        spots, strike, expiration, rates, vols, option_type
    )
    
    # Base case calculation
    base_price = vectorized_black_scholes(
        base_case['spot_price'], strike, expiration,
        base_case['risk_free_rate'], base_case['volatility'], option_type
    )
...
def quick_strike_comparison(
    ...
    for i, strike in enumerate(strikes):
        results[strike] = {
            ...
            'premium_pct': (prices.values[i] / strike) * 100 if option_type == "put" else (prices.values[i] / spot) * 100
        }
