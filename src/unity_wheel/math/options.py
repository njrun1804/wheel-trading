...
def black_scholes_price_validated(
    ...
    if not is_valid and len(warnings_list) > 0 and "Stock price must be positive" in warnings_list[0]:
...
def implied_volatility_validated(
    ...
    return CalculationResult(
        float(sigma) if np.ndim(sigma) == 0 else sigma,
        confidence,
        warnings_list
    )
...
def probability_itm_validated(
    ...
    try:
        ...
        return CalculationResult(
            float(prob) if np.ndim(prob) == 0 else prob,
            confidence,
            warnings_list
        )
