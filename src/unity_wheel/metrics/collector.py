...
class MetricsCollector:
    ...
    def get_performance_summary(
        ...
        return PerformanceMetrics(
            ...
            average_return_error=np.mean([d.return_error for d in with_outcomes if d.return_error is not None]) if with_outcomes else 0.0,
    ...
    def identify_patterns(self) -> List[Dict[str, Any]]:
            ...
            if len(outcomes) >= 10:  # Minimum sample size
                ...
                if abs(success_rate - expected_rate) > 0.2:
                    patterns.append({
                        ...
                        "recommendation": (
                            "Increase confidence" if success_rate > expected_rate
        ...
        for hour, returns in decisions_by_hour.items():
            if len(returns) >= 5:
                ...
                overall_avg = np.mean([
                    r for returns_list in decisions_by_hour.values()
