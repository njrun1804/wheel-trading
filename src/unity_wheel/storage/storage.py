...
class Storage:
    ...
    async def get_or_fetch_option_chain(
        ...
        if cached:
            logger.info("option_chain_cache_hit", 
                       symbol=symbol,
                       expiration=expiration.date())
            return cached
            
        # Cache miss - fetch fresh data
        logger.info("option_chain_cache_miss", 
                   symbol=symbol,
                   expiration=expiration.date())
    ...
    async def get_or_fetch_positions(
        ...
        if self.gcs and self.gcs.enabled:
            await self.gcs.upload_raw_response(
                source=f"positions_{account_id}",
                timestamp=datetime.utcnow(),
                data=data
            )
    ...
    async def get_historical_data(
        ...
        if self.gcs and self.gcs.enabled and len(results) == 0:
            ...
            for file_path in files[:5]:  # Limit to avoid huge downloads
                local_file = await self.gcs.download_parquet(
                    file_path, 
                    self.config.cache_config.cache_dir / "downloads"
                )
        ...
        logger.info("historical_data_retrieved",
                   dataset=dataset,
                   records=len(results),
