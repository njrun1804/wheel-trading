...
class OptimizedStorage(Storage):
    ...
    async def get_options_arrow(self, symbol: str, 
                               lookback_hours: int = 24) -> pa.Table:
        ...
        cache_age = datetime.now() - cache_file.stat().st_mtime if cache_file.exists() else timedelta(days=1)
    ...
    async def get_options_polars(self, symbol: str,
                                lookback_hours: int = 24) -> pl.DataFrame:
        ...
        df = df.with_columns([
            ...
            ((pl.col("expiration") - pl.col("quote_datetime")).dt.days()).alias("dte"),
    ...
    async def get_price_history_optimized(self, symbol: str,
    ...
    async def batch_get_options(self, symbols: List[str],
                              lookback_hours: int = 24) -> Dict[str, pl.DataFrame]:
        """Batch fetch options for multiple symbols efficiently."""
        # Use asyncio to fetch in parallel
        tasks = [
            self.get_options_polars(symbol, lookback_hours)
    ...
    async def find_liquid_strikes(self, symbol: str,
                                current_price: float,
                                dte_range: tuple[int, int] = (20, 45)) -> pl.DataFrame:
        ...
        liquid_df = df.filter(
            ...
            (pl.col("spread") <= 0.10) &  # Max 10 cent spread
    ...
    async def calculate_historical_assignment_rates(self, symbol: str) -> pl.DataFrame:
        ...
        options_df = await self.get_options_polars(symbol, lookback_hours=24*90)  # 90 days
        ...
        for row in options_df.filter(pl.col("option_type") == "put").iter_rows(named=True):
            ...
            if not expiry_prices.is_empty():
                ...
                results.append({
                    'strike_distance': (strike - row['underlying_price']) / row['underlying_price'],
                    'dte': row['dte'],
                    'assigned': assigned,
        ...
        assignment_rates = assignment_df.groupby(
            pl.col("strike_distance").map_elements(
                lambda x: round(x * 100) / 100  # 1% buckets
            ).alias("strike_bucket")
        ).agg([
    ...
    def get_query_performance_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'cache_size_mb': sum(f.stat().st_size for f in self.cache_dir.rglob("*.parquet")) / 1024 / 1024
        }
