...
class GCSAdapter:
    ...
    async def upload_raw_response(self, source: str, timestamp: datetime, 
        ...
        try:
            ...
            logger.info("uploaded_raw_to_gcs", 
                       source=source,
                       blob=blob_name,
                       size=blob.size)
            ...
            logger.error("gcs_upload_failed", 
                        error=str(e),
                        source=source)
            return None
    ...
    async def upload_parquet(self, local_file: Path, dataset: str) -> Optional[str]:
        ...
        try:
            ...
            logger.info("uploaded_parquet_to_gcs",
                       dataset=dataset,
                       blob=blob_name,
            ...
            logger.error("gcs_parquet_upload_failed",
                        error=str(e),
                        file=str(local_file))
            return None
    ...
    async def download_parquet(self, blob_path: str, local_dir: Path) -> Optional[Path]:
        ...
        try:
            ...
            logger.info("downloaded_from_gcs",
                       blob=blob_path,
            ...
            logger.error("gcs_download_failed",
                        error=str(e),
                        blob=blob_path)
            return None
            
    async def list_parquet_files(self, dataset: str, 
