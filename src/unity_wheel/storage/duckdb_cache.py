...
class DuckDBCache:
    ...
    async def initialize(self):
        ...
        async with self.connection() as conn:
            ...
            conn.execute("CREATE INDEX IF NOT EXISTS idx_chains_created ON option_chains(created_at)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_positions_created ON position_snapshots(created_at)")
            
    async def store_option_chain(self, symbol: str, expiration: datetime, 
                                timestamp: datetime, spot_price: float, 
    ...
    async def get_option_chain(self, symbol: str, expiration: datetime,
        ...
        async with self.connection() as conn:
            result = conn.execute("""
                ...
                LIMIT 1
            """, [symbol, expiration.date(), cutoff]).fetchone()
    ...
    async def store_positions(self, account_id: str, positions: List[Dict],
    ...
    async def get_latest_positions(self, account_id: str, 
                                 max_age_minutes: int = 30) -> Optional[Dict[str, Any]]:
    ...
    async def export_to_parquet(self, table: str, output_dir: Path):
        ...
        async with self.connection() as conn:
            ...
            logger.info("exported_to_parquet", 
                       table=table, 
                       rows=len(df),
    ...
    async def _check_vacuum(self):
        ...
        if datetime.utcnow() - self._last_vacuum > timedelta(hours=self.config.vacuum_interval_hours):
            await self._vacuum()
