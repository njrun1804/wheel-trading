...
class SliceCache:
    ...
    def __init__(self, db_path: Path | None = None, hash_bytes: int = 12):
        """
        ...
        """
        self.db_path = (
            db_path or Path.home() / ".wheel_trading" / "cache" / "slice_cache.duckdb"
        )
        self.hash_bytes = (
            hash_bytes  # 12 bytes = 96 bits, collision prob < 10^-20 for 1B slices
        )
        self._ensure_db_dir()
    ...
    def initialize(self):
        ...
        with duckdb.connect(str(self.db_path)) as conn:
            ...
            logger.info(f"Slice cache initialized at {self.db_path}")
    ...
    def get_embedding(
        ...
        with duckdb.connect(str(self.db_path)) as conn:
            ...
            if result:
                ...
                logger.debug(
                    f"Cache hit: {file_path}:{start_line}-{end_line} ({token_count} tokens)"
                )
    ...
    def store_embedding(
        ...
        with duckdb.connect(str(self.db_path)) as conn:
            ...
            logger.debug(
                f"Stored embedding: {file_path}:{start_line}-{end_line} ({token_count} tokens)"
            )
    ...
    def evict_old_slices(self, days: int = 30) -> int:
        ...
        with duckdb.connect(str(self.db_path)) as conn:
            ...
            if evicted > 0:
                ...
                conn.execute("VACUUM")
                logger.info(f"Evicted {evicted} old slices older than {days} days")
    ...
    def export_stats(self, output_path: Path):
        ...
        logger.info(f"Exported cache stats to {output_path}")
