...
class StressTestScenarios:
    ...
    def _run_single_scenario(
        ...
        recovery_days = int(
            abs(total_pnl) / (portfolio_value * 0.001)
        )  # 0.1% daily return
    ...
    def _calculate_position_pnl(
        self, position: dict[str, Any], scenario: dict[str, Any]
    ) -> float:
    ...
    def _calculate_var_for_scenario(
        self, pnl: float, scenario: dict[str, Any]
    ) -> float:
    ...
    def _calculate_cvar_for_scenario(
        self, pnl: float, scenario: dict[str, Any]
    ) -> float:
