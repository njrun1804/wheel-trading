...
from ..data import get_market_validator, get_anomaly_detector
from ..metrics import metrics_collector
from ..monitoring import get_performance_monitor
from ..utils import get_feature_flags, get_logger, StructuredLogger
...
class ObservabilityExporter:
    ...
    def _ensure_metrics_tables(self, conn: duckdb.DuckDBPyConnection) -> None:
        ...
        # Create indexes
        conn.execute("CREATE INDEX IF NOT EXISTS idx_metrics_created ON observability_metrics(created_at)")
        conn.execute("CREATE INDEX IF NOT EXISTS idx_events_created ON observability_events(created_at)")
    ...
    def _calculate_system_health(
        ...
        # Performance health
        slow_ops = sum(
            1 for stats in perf_stats.values()
        ...
        degraded_features = sum(
            1 for f in flag_report["features"].values()
        ...
        return {
            ...
            "components": {
                ...
                "data_quality": "healthy" if validation_stats["success_rate"] >= 0.95 else "degraded",
