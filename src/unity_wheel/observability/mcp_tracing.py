...
@contextmanager
def trace_operation(operation: str, attributes: dict[str, Any] | None = None) -> None:
...
def trace_mcp_workflow(workflow_name: str, total_files: int = 0) -> None:
    ...
    with tracer.start_as_current_span(f"mcp.workflow.{workflow_name}") as span:
        ...
        def record_step(step_name: str, duration_ms: float):
            ...
            span.add_event(
                f"Step completed: {step_name}", {"step.duration_ms": duration_ms}
            )
...
def record_token_usage(
    span: trace.Span, prompt_tokens: int, completion_tokens: int
) -> None:
...
def record_db_metrics(
    span: trace.Span, rows_scanned: int, rows_returned: int, cache_hit: bool = False
) -> None:
