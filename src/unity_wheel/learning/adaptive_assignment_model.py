...
class AdaptiveAssignmentModel(FeedbackLoop):
    ...
    def predict_assignment(
        ...
        if self.is_trained and len(self.assignment_history) >= 100:
            ...
            adjusted_probability = self._apply_calibration(
                base_probability, market_regime
            )
            ...
            if moneyness < 1.0:  # ITM put
                base_probability = 0.5 + 0.5 * (1.0 - moneyness) / (
                    vol_factor * time_factor
                )
            else:  # OTM put
                base_probability = 0.5 * np.exp(
                    -2 * (moneyness - 1.0) / (vol_factor * time_factor)
                )
    ...
    def apply_parameter_update(self, updates: list[ParameterUpdate]) -> None:
        ...
        for update in updates:
            if update.parameter_name == "threshold_adjustment":
                ...
                self.calibration_params["regime_adjustments"][
                    regime
                ] = update.recommended_value
    ...
    def _retrain_model(self) -> None:
        ...
        logger.info(
            f"Retraining assignment model with {len(self.assignment_history)} samples"
        )
        ...
        logger.info(
            f"Model retrained. Top features: {sorted(self.feature_importance.items(), key=lambda x: x[1], reverse=True)[:3]}"
        )

    def _apply_calibration(
        self, base_probability: float, market_regime: str | None
    ) -> float:
        ...
        if (
            market_regime
            and market_regime in self.calibration_params["regime_adjustments"]
        ):
    ...
    def _calculate_prediction_confidence(self, features_scaled: np.ndarray) -> float:
        ...
        scaled_confidence = (
            base_confidence * self.calibration_params["confidence_scaling"]
        )
    ...
    def get_model_performance(self) -> dict[str, float]:
        ...
        if len(self.assignment_history) < 100:
            return {
                ...
                "samples": len(self.assignment_history),
            }
        ...
        for outcome in recent_outcomes:
            error = abs(
                outcome.predicted_probability - (1.0 if outcome.was_assigned else 0.0)
            )
            errors.append(error)

            # Count correct predictions (within 0.3 threshold)
            if (
                outcome.predicted_probability > 0.5
                and outcome.was_assigned
                or outcome.predicted_probability <= 0.5
                and not outcome.was_assigned
            ):
