...
class AnomalyDetector:
        ...
        if 'open' in current and 'prev_close' in current:
            ...
            if gap > 0.10:  # 10% gap
                ...
                anomaly = MarketAnomaly(
                    ...
                    metrics={'gap_size': gap, 'direction': np.sign(current['open'] - current['prev_close'])},
                    z_score=z_score,
        ...
        if 'realized_vol' in current:
            ...
            if abs(z_score) > 3.0:  # 3 standard deviations
                anomaly = MarketAnomaly(
                    ...
                    z_score=z_score,
                    percentile=percentile,
                    historical_frequency=(abs(historical_vols - historical_vols.mean()) > 3 * historical_vols.std()).mean(),
                    confidence=0.95
                )
                anomalies.append(anomaly)
    ...
    def _prepare_ml_features(self, current: Dict[str, float]) -> Optional[np.ndarray]:
        ...
        feature_names = [
            'returns', 'volume_ratio', 'realized_vol',
            'price_momentum', 'rsi', 'spread'
        ]
    ...
    def fit_ml_detector(self, historical_data: pd.DataFrame) -> None:
        ...
        for i in range(20, len(historical_data)):
            ...
            features = {
                ...
                'price_momentum': (current['close'] - window.iloc[0]['close']) / window.iloc[0]['close'],
