...
from .iv_surface import IVSurfaceAnalyzer, IVMetrics
...
class IntegratedDecisionEngine:
    ...
    async def get_recommendation(
        ...
        if event_calendar:
            ...
            should_adjust, adjustments = self.event_analyzer.should_adjust_for_event(
                optimization.dte_target,
                iv_metrics.iv_rank if iv_metrics else 50
            )
            if should_adjust:
        ...
        contracts = self._calculate_position_size(
            final_params['kelly'],
            strike or current_prices.get('close', 100)
        )
    ...
    def _calculate_market_state(
        ...
        if len(historical) >= 20:
            momentum = (historical['close'].iloc[-1] - historical['close'].iloc[-20]) / historical['close'].iloc[-20]
        else:
            momentum = 0.0
    ...
    def generate_decision_report(
        ...
        report.append(f"{action_emoji.get(recommendation.action, '❓')} ACTION: {recommendation.action}")
        ...
        # Confidence
        conf_emoji = "🟢" if recommendation.confidence > 0.7 else "🟡" if recommendation.confidence > 0.4 else "🔴"
