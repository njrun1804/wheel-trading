...
class MarketCalibrator:
    ...
    async def calibrate_from_history(
        ...
        optimal_params = self._calculate_optimal_parameters(
            current_regime, regime_performance, iv_rank
        )
    ...
    def _calculate_breach_probability(
        self, returns: np.ndarray, threshold: float, days: int
    ) -> float:
    ...
    def _find_optimal_dte(self, prices: pd.DataFrame, volatility: float) -> int:
    ...
    def _estimate_win_rate(self, returns: np.ndarray, volatility: float) -> float:
