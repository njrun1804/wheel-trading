...
from ..utils.logging import get_logger, StructuredLogger, timed_operation
from ..utils.cache import IntelligentCache
from ..utils.recovery import with_recovery, RecoveryStrategy
from ..auth.rate_limiter import RateLimiter
from .client import SchwabClient
from .types import SchwabPosition, SchwabAccount
...
class DataStorage:
    ...
    def _init_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            conn.execute("CREATE INDEX IF NOT EXISTS idx_orders_entered_time ON orders(entered_time)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_orders_status ON orders(status)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_transactions_date ON transactions(transaction_date)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_account_snapshots_timestamp ON account_snapshots(timestamp)")
...
class SchwabDataIngestion:
    ...
    def __init__(
        ...
        logger.info("SchwabDataIngestion initialized", extra={
        ...
        try:
            ...
            if self.requirements.orders_needed:
                orders = await self._pull_orders(
                    account_number,
                    days=self.requirements.order_history_days
                )
            ...
            if self.requirements.transactions_needed:
                transactions = await self._pull_transactions(
                    account_number,
                    days=self.requirements.transaction_history_days
                )
