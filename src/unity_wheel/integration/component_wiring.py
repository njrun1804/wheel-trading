...
class IntegratedWheelStrategy(WheelStrategy):
    ...
    def find_optimal_put_strike(
        ...
        logger.info(
            f"Reduced {len(available_strikes)} strikes to {len(bucketed_strikes)} buckets"
        )
...
class IntegratedRiskAnalyzer(EVRiskAnalyzer):
    ...
    def analyze_portfolio(
        ...
        for position in positions:
            ev_result = self.analyze_position(
                position,
                {
        ...
        return {
            ...
            "recommendations": self._generate_recommendations(
                ev_results, optimization, stress_results
            ),
        }
    ...
    def _generate_recommendations(self, ev_results, optimization, stress_results):
        ...
        if (
            optimization.get("total_allocation", 0)
            > config.risk.max_portfolio_allocation
        ):
...
class IntegratedDecisionTracker(DecisionTracker):
    ...
    async def record_outcome_integrated(
...
class IntegratedStatsAnalyzer:
    ...
    async def detect_anomalies(
        ...
        for _i, opt in enumerate(options_data):
