...
class DataValidator:
    ...
    def validate_stock_data(
        ...
        if any(p == 0 for p in [open_price, high_price, low_price] if p is not None):
            ...
            logger.warning(
                f"Zero OHLC prices for {symbol} on {date}: O={open_price} H={high_price} L={low_price}"
            )
        ...
        if symbol == "U":
            ...
            if not (self.UNITY_MIN_PRICE <= close_price <= self.UNITY_MAX_PRICE):
                return (
                    False,
                    f"Unity price ${close_price:.2f} outside valid range ${self.UNITY_MIN_PRICE}-${self.UNITY_MAX_PRICE}",
                )

            # Warn on unusual but not impossible prices
            if not (
                self.UNITY_TYPICAL_RANGE[0]
                <= close_price
                <= self.UNITY_TYPICAL_RANGE[1]
            ):
                logger.warning(
                    f"Unity price ${close_price:.2f} outside typical range ${self.UNITY_TYPICAL_RANGE[0]}-${self.UNITY_TYPICAL_RANGE[1]}"
                )
    ...
    def validate_option_quote(
        ...
        if bid_price == 0 and ask_price == 0:
            return (
                False,
                f"Both bid and ask are zero for {symbol} {strike} {option_type}",
            )
        ...
        if bid_price > 0 and ask_price > 0:
            ...
            if spread < self.MIN_OPTION_SPREAD:
                logger.warning(
                    f"Unusually tight spread ${spread:.4f} for {symbol} {strike} {option_type}"
                )

            if spread_pct > self.MAX_OPTION_SPREAD_PCT:
                logger.warning(
                    f"Wide spread {spread_pct:.1%} for {symbol} {strike} {option_type}"
                )

        # Strike validation for Unity
        if symbol == "U" and not (5.0 <= strike <= 100.0):
            return False, f"Unity strike ${strike} outside reasonable range"

        # Expiration validation
        if expiration.date() <= datetime.now(UTC).date():
            return False, f"Option already expired: {expiration.date()}"

        return True, None

    def validate_fred_data(
        self, indicator: str, date: datetime, value: float
    ) -> tuple[bool, str | None]:
    ...
    def validate_data_consistency(
        self, symbol: str, current_data: dict, previous_data: dict | None = None
    ) -> tuple[bool, str | None]:
        ...
        if current_price > 0 and previous_price > 0:
            ...
            if symbol == "U" and pct_change > 0.30:
                return (
                    False,
                    f"Extreme price move {pct_change:.1%} from ${previous_price:.2f} to ${current_price:.2f}",
                )
