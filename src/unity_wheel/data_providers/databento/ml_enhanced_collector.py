...
class MLEnhancedDataCollector:
    ...
    def __init__(self, db_path: str = "data/wheel_trading_optimized.duckdb"):
        ...
        self.snapshot_rules = {
            ...
            'near_expiry': {  # For positions < 2 DTE
                'frequency_minutes': 5
            },
            'high_volatility': {  # When VIX > 30
                'frequency_minutes': 15
            },
            'around_events': {  # ±2 hours of earnings/Fed
                'frequency_minutes': 5
            }
        }
    ...
    async def _collect_option_surface(self, symbol: str) -> Dict:
            ...
            for option in chain.options:
                ...
                if 0.5 <= moneyness <= 1.5:
                    option_data = {
                        ...
                        'bid_ask_spread_pct': (option.ask - option.bid) / option.ask if option.ask > 0 else None,
        ...
        if all_options:
            surface_data['surface_metrics'] = self._calculate_surface_metrics(all_options)
    ...
    def _calculate_surface_metrics(self, options: List[Dict]) -> Dict:
        ...
        if not put_25d.empty and not call_25d.empty:
            metrics['iv_skew'] = put_25d['implied_volatility'].mean() - call_25d['implied_volatility'].mean()
        ...
        if not puts.empty and not calls.empty:
            metrics['put_call_volume_ratio'] = puts['volume'].sum() / calls['volume'].sum()
            metrics['put_call_oi_ratio'] = puts['open_interest'].sum() / calls['open_interest'].sum()
        ...
        for bucket, (low, high) in [('otm', (0.7, 0.9)), ('atm', (0.9, 1.1)), ('itm', (1.1, 1.3))]:
            bucket_options = df[df['moneyness'].between(low, high)]
            if not bucket_options.empty:
                metrics[f'avg_spread_{bucket}'] = bucket_options['bid_ask_spread_pct'].mean()
    ...
    def _calculate_market_indicators(self, underlying: Dict, options: Dict) -> Dict:
        ...
        if all(k in underlying for k in ['high', 'low', 'open']):
            indicators['intraday_range'] = (underlying['high'] - underlying['low']) / underlying['open']
            indicators['intraday_trend'] = (underlying['spot_price'] - underlying['open']) / underlying['open']
        
        # Microstructure health
        if 'quote_count' in underlying and 'trade_count' in underlying:
            indicators['quote_to_trade_ratio'] = underlying['quote_count'] / underlying['trade_count']
    ...
    def _determine_collection_type(self) -> str:
        ...
        if (self.snapshot_rules['regular_hours']['start'] <= 
            current_time <= 
        ...
        while True:
            try:
                ...
                if collection_type == 'regular':
                    wait_minutes = self.snapshot_rules['regular_hours']['frequency_minutes']
                else:
