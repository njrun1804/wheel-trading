...
class UnityLiveClient:
    ...
    def __init__(self, api_key: str | None = None):
        ...
        self.book: dict[
            int, dict[str, tuple]
        ] = {}  # instrument_id -> {bid/ask: (price, size)}
    ...
    async def get_live_stock_price(self, symbol: str = "U") -> float | None:
    ...
    async def _collect_with_timeout(
        self, streamer: LiveStreamer, handler: Callable, timeout_seconds: int
    ):
        """Run collection with timeout"""
        try:
            # Run collection
            await asyncio.wait_for(streamer.run_async(handler), timeout=timeout_seconds)
        except TimeoutError:
