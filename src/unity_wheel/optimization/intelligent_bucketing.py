...
class IntelligentBucketing:
    ...
    def create_buckets(
        self, options: list[dict], underlying_price: float
    ) -> list[StrikeBucket]:
        """
    ...
    def _process_bucket(
        ...
        expected_value = (
            premium * (1 - probability_itm)
            - (strike - underlying_price) * probability_itm
        )
    ...
    def _calculate_risk_score(
        self, strike: float, underlying_price: float, premium: float
    ) -> float:
    ...
    def select_optimal_bucket(
        self, buckets: list[StrikeBucket], target_delta: float = None
    ) -> StrikeBucket | None:
        """
        ...
        valid_buckets = [b for b in buckets if abs(abs(b.delta) - target_delta) <= 0.1]
        ...
        optimal_bucket = max(
            valid_buckets, key=lambda b: b.expected_value / (b.risk_score + 1)
        )
