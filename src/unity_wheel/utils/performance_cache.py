...
class MemoryAwareLRUCache:
    ...
    def _get_memory_usage(self, value: Any) -> int:
        ...
        try:
            ...
            if isinstance(value, (int, float)):
                ...
                return 56 + sum(self._get_memory_usage(item) for item in value[:10])  # Sample first 10
    ...
    async def put(self, key: str, value: Any, computation_time_ms: Optional[float] = None) -> None:
        ...
        async with self._lock:
            ...
            if computation_time_ms is not None:
                ...
                self._stats.avg_computation_time_ms = sum(self._computation_times) / len(self._computation_times)
                self._stats.max_computation_time_ms = max(
                    self._stats.max_computation_time_ms, 
                    computation_time_ms
                )
    ...
    async def cleanup_expired(self) -> int:
        ...
        if removed > 0:
            logger.debug(
                "cache_cleanup",
                extra={"name": self.name, "removed_count": removed}
            )
