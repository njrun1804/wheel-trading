...
class DatabentoStorageAdapter:
    ...
    async def store_wheel_candidates(
        ...
        if records:
            ...
            for r in records:
                values.extend([
                    r['symbol'], r['target_delta'], r['expiration'],
                    r['strike'], r['option_type'], r['bid'], r['ask'],
                    r['mid'], r['implied_volatility'], r['delta'],
                    r['expected_return'], r['annualized_return'],
                    r['probability_profit'], r['timestamp']
                ])
                
            await self.storage.cache.conn.execute(
                f"INSERT OR REPLACE INTO wheel_candidates VALUES {placeholders}",
                values
            )
    ...
    async def get_storage_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'cache_hit_rate': (
                ...
                if self._metrics['cache_misses'] > 0 else 0
            )
        }
