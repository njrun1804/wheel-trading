...
class CodeHelper:
    ...
    async def get_function_info(
        ...
        """Get function info (API compatibility method)."""
        # Handle built-in modules differently  
        if module_path in ['json', 'os', 'sys', 'time', 'math', 'random']:
            try:
                import importlib
                ...
                if hasattr(module, function_name):
                    ...
                    try:
                        ...
                        return {
                            "function": function_name,
                            "signature": "Built-in function - signature not available",
                            "module": module_path
                        }
                else:
                    return {"error": f"Function {function_name} not found in {module_path}"}
            ...
            return await self.get_function_signature(module_path, function_name)
                ...
                for name, obj in inspect.getmembers(module, inspect.isfunction):
                    ...
                    if not name.startswith("_"):
                        try:
                            ...
                            if inspect.iscoroutinefunction(obj) and name in [
                                "test",
                                "main",
                                "run",
                            ]:
                                logger.debug(
                                    f"Skipping async function {name} in {module_path} to avoid coroutine warnings"
                                )
                                continue
        ...
        if hasattr(module, class_name):
            ...
            return {
                ...
                "attributes": [
                    ...
                    if not name.startswith("_")
                    and not inspect.ismethod(getattr(cls, name))
                ],
    ...
    async def find_usages(
        ...
        for i in range(0, len(results), batch_size):
            ...
            batch_details = await loop.run_in_executor(
                self.executor, _analyze_usages_batch, batch, symbol
            )
    ...
    async def analyze_project_structure(self, root_dir: str = ".") -> dict[str, Any]:
        ...
        for i in range(0, len(py_files), chunk_size):
            ...
            analysis = await loop.run_in_executor(
                self.executor, _analyze_project_chunk, chunk, str(root)
            )
    ...
    async def refactor_rename(
        self, old_name: str, new_name: str, directory: str = "."
    ) -> dict[str, Any]:
    ...
    def _load_module(self, module_path: str):
        ...
        if self._should_skip_module(path):
            raise ImportError(
                f"Skipping module {module_path} - likely contains async code at module level"
            )
    ...
    def _should_skip_module(self, path: Path) -> bool:
        ...
        return bool(
            any(
                part in ["tests", "test", "testing", "examples", "demos"]
...
def _analyze_usages_batch(results: list[dict], symbol: str) -> list[dict]:
    ...
    for result in results:
        try:
            ...
            logger.warning(
                f"Error analyzing usage in {result.get('file', 'unknown')}: {e}",
                exc_info=True,
                extra={"operation": "usage_analysis", "symbol": symbol},
            )
    ...
    for file_path in files:
        try:
            ...
            logger.warning(
                ...
                exc_info=True,
                extra={"operation": "project_analysis", "file": str(file_path)},
            )
            analyses.append(
                {
                    "file": str(file_path),
...
# Singleton instance
_helper_instance: CodeHelper | None = None
...
def get_code_helper() -> CodeHelper:
    ...
    Returns:
        CodeHelper: Singleton instance providing combined Python analysis tools
                   including function signatures, class inspection, module
                   analysis, and code intelligence with M4 Pro optimization.
    """
