...
class RiskConfig(BaseModel):
    ...
    max_position_size: float = Field(
        0.20, ge=0.01, le=0.50, description="Max % of portfolio per position"
    )
    ...
    max_volatility: float = Field(
        1.50, ge=0.50, le=3.0, description="Stop trading above this volatility"
    )
...
def migrate_legacy_config(legacy_config: Dict[str, Any]) -> MinimalWheelConfig:
    ...
    minimal_data["strategy"] = {
        ...
        "expiration": {
            "days_to_expiry_target": strategy_section.get("expiration", {}).get(
