...
class RollTriggers(BaseModel):
    ...
    dte_threshold: int = Field(
        7, ge=0, le=365, description="Roll when DTE below threshold"
    )
    profit_threshold_1: float = Field(
        0.50, ge=0.0, le=1.0, description="First profit trigger"
    )
    profit_threshold_2: float = Field(
        0.95, ge=0.0, le=1.0, description="Second profit trigger"
    )
...
class StrikeRange(BaseModel):
    ...
    min_moneyness: float = Field(
        0.80, ge=0.0, le=1.0, description="Minimum moneyness (% of spot)"
    )
    max_moneyness: float = Field(
        1.10, ge=1.0, le=2.0, description="Maximum moneyness (% of spot)"
    )
...
class StrategyConfig(BaseModel):
    ...
    delta_target: float = Field(
        0.30, ge=0.0, le=1.0, description="Target delta for short puts"
    )
    days_to_expiry_target: int = Field(
        35, ge=1, le=365, description="Target days to expiry"
    )
    ...
    def validate_dte_consistency(self) -> "StrategyConfig":
        if self.min_days_to_expiry >= self.days_to_expiry_target:
            raise ValueError(
                "min_days_to_expiry must be less than days_to_expiry_target"
            )
...
class RiskLimits(BaseModel):
    ...
    # VaR and CVaR limits
    max_var_95: float = Field(
        0.05, ge=0.0, le=1.0, description="Max VaR at 95% confidence"
    )
    max_cvar_95: float = Field(
        0.075, ge=0.0, le=1.0, description="Max CVaR at 95% confidence"
    )
    ...
    max_contracts_per_trade: int = Field(
        100, ge=1, description="Max contracts per trade"
    )
...
class GreekLimits(BaseModel):
    ...
    max_delta_exposure: float = Field(
        100.0, ge=0.0, description="Maximum delta exposure"
    )
    max_gamma_exposure: float = Field(
        10.0, ge=0.0, description="Maximum gamma exposure"
    )
    max_vega_exposure: float = Field(
        1000.0, ge=0.0, description="Maximum vega exposure ($)"
    )
    max_theta_decay: float = Field(
        100.0, ge=0.0, description="Maximum daily theta decay"
    )
...
class CircuitBreakers(BaseModel):
    ...
    min_portfolio_value: float = Field(
        10000, ge=0.0, description="Stop trading below this value"
    )

    # Market condition circuit breakers
    max_volatility: float = Field(
        1.5, ge=0.0, description="Max annual volatility (150%)"
    )
    max_gap_percent: float = Field(
        0.10, ge=0.0, le=1.0, description="Max price gap (10%)"
    )
    min_volume_ratio: float = Field(
        0.5, ge=0.0, le=1.0, description="Min volume vs average"
    )

    # Loss circuit breakers
    max_daily_loss_pct: float = Field(
        0.02, ge=0.0, le=1.0, description="Max daily loss %"
    )
    max_weekly_loss_pct: float = Field(
        0.05, ge=0.0, le=1.0, description="Max weekly loss %"
    )
    max_consecutive_losses: int = Field(
        3, ge=1, description="Stop after N consecutive losses"
    )

    # System health circuit breakers
    min_confidence: float = Field(
        0.30, ge=0.0, le=1.0, description="Min confidence to trade"
    )
    max_warnings: int = Field(3, ge=0, description="Max warnings before stopping")
    blackout_hours: list[int] = Field(
        [], description="Hours to avoid trading (24h format)"
    )
...
class AdaptiveLimits(BaseModel):
    ...
    base_max_position_pct: float = Field(
        0.20, ge=0.0, le=1.0, description="Base position size %"
    )
    base_min_confidence: float = Field(
        0.30, ge=0.0, le=1.0, description="Base minimum confidence"
    )
    ...
    ml_confidence_threshold: float = Field(
        0.60, ge=0.0, le=1.0, description="Min ML confidence"
    )
...
class CacheTTL(BaseModel):
    ...
    options_chain: int = Field(
        900, ge=0, description="Options chain cache TTL in seconds"
    )
    ...
    historical: int = Field(
        86400, ge=0, description="Historical data cache TTL in seconds"
    )
...
class RetryConfig(BaseModel):
    ...
    delays: list[int] = Field(
        [1, 2, 5], description="Exponential backoff delays in seconds"
    )
    rate_limit_wait: int = Field(
        60, ge=0, description="Wait after rate limit in seconds"
    )
...
class LiquidityRequirements(BaseModel):
    ...
    max_bid_ask_spread: float = Field(
        0.50, ge=0.0, description="Maximum bid-ask spread"
    )
...
class DataQuality(BaseModel):
    ...
    stale_data_seconds: int = Field(
        30, ge=0, description="Stale data threshold in seconds"
    )
    ...
    max_price_change_pct: float = Field(
        50.0, ge=0.0, description="Maximum price change %"
    )
    min_options_per_expiry: int = Field(
        10, ge=0, description="Minimum option contracts"
    )
...
class BrokerConfig(BaseModel):
    ...
    api_secret: SecretStr | None = Field(None, description="API secret")
...
class TradingExecutionConfig(BaseModel):
    ...
    max_concurrent_puts: int = Field(
        3, ge=1, description="Max concurrent put positions"
    )
...
class UnityEarnings(BaseModel):
    ...
    typical_move_pct: float = Field(
        0.20, ge=0.0, le=1.0, description="Typical earnings move %"
    )
    blackout_days_before: int = Field(
        7, ge=0, description="Days before earnings to stop trading"
    )
    iv_expansion_factor: float = Field(
        1.5, ge=1.0, description="IV expansion factor into earnings"
    )
...
class UnityGapBehavior(BaseModel):
    ...
    typical_overnight_move: float = Field(
        0.05, ge=0.0, description="Typical overnight move"
    )
    max_acceptable_gap: float = Field(
        0.15, ge=0.0, description="Max gap to trade after"
    )
    gap_recovery_hours: int = Field(
        48, ge=0, description="Hours to wait after major gap"
    )
...
class UnityCorrelations(BaseModel):
    ...
    sector_etfs: list[str] = Field(
        ["GAMR", "METV", "XLC"], description="Related sector ETFs"
    )
...
class RegimeParams(BaseModel):
    ...
    roll_profit_target: float = Field(
        0.50, ge=0.0, le=1.0, description="Roll at profit %"
    )
    position_size_factor: float = Field(
        1.0, ge=0.0, description="Position size multiplier"
    )
...
class AdaptiveStopConditions(BaseModel):
    ...
    max_drawdown: float = Field(
        0.20, ge=0.0, le=1.0, description="Stop at this drawdown"
    )
    min_days_to_earnings: int = Field(
        7, ge=0, description="Skip if earnings within days"
    )
...
class OutcomeTracking(BaseModel):
    ...
    database_path: str = Field(
        "~/.wheel_trading/wheel_outcomes.db", description="Database path"
    )
...
class AdaptiveConfig(BaseModel):
    ...
    regime_persistence_days: int = Field(
        3, ge=1, description="Days to confirm regime change"
    )
    base_position_pct: float = Field(
        0.20, ge=0.0, le=1.0, description="Base position size %"
    )
    max_position_pct: float = Field(
        0.25, ge=0.0, le=1.0, description="Max position size %"
    )
    ...
    regime_params: dict[str, RegimeParams] = Field(
        default_factory=lambda: {
            ...
            "low_volatility": RegimeParams(
                put_delta=0.35, target_dte=42, position_size_factor=1.2
            ),
        }
    )

    stop_conditions: AdaptiveStopConditions = Field(
        default_factory=AdaptiveStopConditions
    )
...
class ModelConfig(BaseModel):
    ...
    retrain_days: int | None = Field(
        None, ge=1, description="Retrain frequency in days"
    )
    min_samples: int | None = Field(
        None, ge=1, description="Minimum samples for training"
    )
    lookback_days: int | None = Field(None, ge=1, description="Lookback period in days")
...
class AlertsConfig(BaseModel):
    ...
    delta_warning: float = Field(
        0.40, ge=0.0, le=1.0, description="Delta warning threshold"
    )
...
class DatabentoFilters(BaseModel):
    ...
    moneyness_range: float = Field(
        0.20, ge=0.0, le=1.0, description="Range around spot price"
    )
...
class DatabentoRateLimits(BaseModel):
    ...
    max_concurrent_live: int = Field(
        10, ge=1, description="Max concurrent live connections"
    )
    max_historical_rps: int = Field(100, ge=1, description="Max requests per second")
    max_symbols_per_request: int = Field(
        2000, ge=1, description="Max symbols per request"
    )
...
class DatabentoLoader(BaseModel):
    ...
    retry_delays: list[int] = Field(
        [1, 2, 5, 10], description="Retry delays in seconds"
    )
...
class IVSurfaceConfig(BaseModel):
    ...
    lookback_days: int = Field(
        252, ge=1, description="Historical days for IV calculation"
    )
    min_history: int = Field(30, ge=1, description="Minimum days required")
    default_iv_rank: float = Field(
        50.0, ge=0.0, le=100.0, description="Default IV rank"
    )
...
class PerformanceTrackerConfig(BaseModel):
    ...
    database_path: str = Field(
        "data/wheel_trading_optimized.duckdb", description="Database path"
    )
...
class APIOperationConfig(BaseModel):
    ...
    min_confidence: float = Field(
        0.75, ge=0.0, le=1.0, description="Min confidence for trades"
    )
    max_decision_time: float = Field(
        0.2, ge=0.0, description="Max decision time in seconds"
    )
    max_bid_ask_spread: float = Field(
        0.50, ge=0.0, description="Maximum spread allowed"
    )
...
class BacktestConfig(BaseModel):
    ...
    commission_per_contract: float = Field(
        0.65, ge=0.0, description="Commission per contract"
    )
...
class PerformanceSLA(BaseModel):
    ...
    black_scholes_ms: int = Field(
        50, ge=1, description="Black-Scholes calculation time"
    )
...
class PerformanceMonitoring(BaseModel):
    ...
    sample_rate: float = Field(
        0.1, ge=0.0, le=1.0, description="Sample rate for operations"
    )
...
class DatabasePaths(BaseModel):
    ...
    performance: str = Field(
        "data/wheel_trading_optimized.duckdb", description="Performance DB"
    )
    schwab_data: str = Field(
        "data/wheel_trading_optimized.duckdb", description="Schwab data DB"
    )
...
class AuthConfig(BaseModel):
    ...
    client_id: SecretStr | None = Field(None, description="OAuth client ID")
    client_secret: SecretStr | None = Field(None, description="OAuth client secret")
    redirect_uri: str = Field(
        "http://localhost:8182/callback", description="OAuth callback URL"
    )
    auth_url: str = Field(
    ...
    token_url: str = Field(
        "https://api.schwabapi.com/v1/oauth/token", description="Token endpoint"
    )
    scope: str = Field(
        "AccountsRead MarketDataRead", description="OAuth scopes to request"
    )
    ...
    @classmethod
    def validate_credentials(cls, v: SecretStr | None) -> SecretStr | None:
...
class OptimizationConfig(BaseModel):
    ...
    mode: str = Field(
        "dynamic", pattern="^(dynamic|tiered)$", description="Optimization mode"
    )
    min_confidence: float = Field(
    ...
    min_history_days: int = Field(
        250, ge=1, description="History required for optimization"
    )
...
class WheelConfig(BaseModel):
    ...
    def validate_global_consistency(self) -> "WheelConfig":
        ...
        if self.risk.kelly_fraction > 1.0:
            raise ValueError(
                "Half-Kelly (0.5) is recommended, Full-Kelly (1.0) is maximum"
            )

        # Validate Unity volatility average is within typical range
        unity_vol = self.unity.volatility
        if not (
            unity_vol.typical_range[0]
            <= unity_vol.average
            <= unity_vol.typical_range[1]
        ):
...
def load_config(config_path: str | Path) -> WheelConfig:
...
def validate_config_health(config: WheelConfig) -> dict[str, bool | str]:
    """
