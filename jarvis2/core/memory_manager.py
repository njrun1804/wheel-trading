...
try:
    ...
    # Python 3.13+ moved shared_memory
...
# M4 Pro memory limits
TOTAL_MEMORY_GB = 24
METAL_LIMIT_GB = 18
BUFFER_POOL_GB = 4  # Pre-allocated for tensor passing
...
class UnifiedMemoryManager:
    ...
    def _init_pool(self):
        ...
        buffer_sizes = [
            (100, 1024 * 1024),      # 100x 1MB buffers
            (50, 10 * 1024 * 1024),  # 50x 10MB buffers  
            (20, 50 * 1024 * 1024),  # 20x 50MB buffers
            (10, 100 * 1024 * 1024), # 10x 100MB buffers
        ]
        ...
        for count, size in buffer_sizes:
            for _ in range(count):
                ...
                try:
                    ...
                    self.buffers[name] = MemoryBuffer(
                        name=name,
                        size=size,
                        dtype=np.float32,
                        shape=(size // 4,)  # Default float32
                    )
        ...
        logger.info(f"Allocated {len(self.buffers)} buffers, "
                   f"{total_allocated / (1024**3):.1f}GB total")
    ...
    def allocate_tensor(self, shape: Tuple[int, ...], 
        ...
        """
        # Ensure dtype is a numpy dtype instance
        ...
        if not best_buffer:
            raise MemoryError(f"No buffer available for {size / (1024**2):.1f}MB tensor")
    ...
    def get_tensor(self, buffer_name: str) -> np.ndarray:
        ...
        # Create view with correct shape/dtype
    ...
    def _check_memory_pressure(self):
        ...
        # On macOS, also check Metal GPU memory if available
        if platform.system() == 'Darwin':
            try:
                import torch
                if torch.backends.mps.is_available():
                    # Add MPS allocated memory
        ...
        if used_gb > METAL_LIMIT_GB:
            logger.warning(f"Memory pressure high: {used_gb:.1f}GB used "
                         f"(limit: {METAL_LIMIT_GB}GB)")
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        used_buffer_size = sum(b.size for b in self.buffers.values() if b.in_use)
        
        return {
            'system_memory_gb': vm.total / (1024**3),
            'system_used_gb': vm.used / (1024**3),
            'system_available_gb': vm.available / (1024**3),
            'metal_limit_gb': METAL_LIMIT_GB,
            'buffer_pool_gb': self.pool_size_bytes / (1024**3),
            'buffers_total': len(self.buffers),
            'buffers_in_use': buffers_in_use,
            'buffer_utilization': used_buffer_size / total_buffer_size if total_buffer_size > 0 else 0
        }
...
class TensorQueue:
    ...
    def __init__(self, memory_manager: UnifiedMemoryManager, maxsize: int = 100):
        ...
        # Store metadata separately since numpy arrays don't allow custom attributes
    ...
    def put(self, tensor: np.ndarray, copy: bool = True) -> str:
        ...
        """
        if copy:
            # Allocate shared memory and copy
            buffer_name, shared_array = self.memory_manager.allocate_tensor(
                tensor.shape, tensor.dtype
            )
            shared_array[:] = tensor
        else:
            # For zero-copy, the buffer name must be provided through metadata
            # since numpy arrays don't allow custom attributes
            if hasattr(tensor, '_buffer_name'):
                buffer_name = tensor._buffer_name
            else:
                # Check if we have metadata for this tensor
                tensor_id = id(tensor)
