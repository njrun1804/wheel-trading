...
class UnifiedMemoryManager:
            ...
            for _ in range(count):
                ...
                try:
                    ...
                    self.buffers[name] = MemoryBuffer(
                        name=name, size=size, dtype=np.float32, shape=(size // 4,)
                    )
    ...
    def allocate_tensor(
        ...
        for name, buffer in self.buffers.items():
            if not buffer.in_use and buffer.size >= size and buffer.size < best_size:
                best_buffer = name
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "buffer_utilization": used_buffer_size / total_buffer_size
...
class TensorQueue:
    ...
    def put(self, tensor: np.ndarray, copy: bool = True) -> str:
        ...
        self.metadata_queue.put(
            {"buffer_name": buffer_name, "shape": tensor.shape, "dtype": tensor.dtype}
        )
        return buffer_name
    ...
    def get(self, timeout: float | None = None) -> tuple[str, np.ndarray]:
