        ...
        class TypeHintCollector(cst.CSTVisitor):
            def visit_FunctionDef(self, node: cst.FunctionDef) -> None:
                ...
                for param in node.params.params:
                    if param.annotation:
                        type_hints[
                            f"{node.name}_{param.name.value}"
...
class CodeTransformer:
    ...
    def add_import(self, code: str, import_name: str, from_module: str = None) -> str:
        ...
        if from_module:
                ...
                body=[
                    cst.ImportFrom(
                        module=cst.Attribute(
                            value=cst.Name(from_module.split(".")[0]),
                            attr=cst.Name(".".join(from_module.split(".")[1:])),
                        )
                        if "." in from_module
                        else cst.Name(from_module),
            ...
            import_stmt = cst.SimpleStatementLine(
                body=[cst.Import(names=[cst.ImportAlias(name=cst.Name(import_name))])]
            )
