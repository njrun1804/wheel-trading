...
class CodeAnalyzer:
    ...
    def analyze(self, code: str) ->CodeContext:
        ...
        try:
            ...
            return CodeContext(imports = imports, functions = functions,
                classes = classes, variables = variables, docstring = docstring,
                complexity = complexity, type_hints = type_hints)
        except SyntaxError:
    ...
    def _extract_functions(self, tree: ast.AST) ->List[Dict[str, Any]]:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.FunctionDef):
                func_info = {'name': node.name, 'args': [arg.arg for arg in
                    node.args.args], 'defaults': len(node.args.defaults),
                    'docstring': ast.get_docstring(node), 'decorators': [
                    self._get_decorator_name(d) for d in node.
                    decorator_list], 'returns': self._get_annotation(node.
                    returns), 'is_async': isinstance(node, ast.
                    AsyncFunctionDef), 'lineno': node.lineno}
    ...
    def _extract_classes(self, tree: ast.AST) ->List[Dict[str, Any]]:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ClassDef):
                ...
                class_info = {'name': node.name, 'bases': [self._get_name(
                    base) for base in node.bases], 'methods': methods,
                    'docstring': ast.get_docstring(node), 'decorators': [
                    self._get_decorator_name(d) for d in node.
    ...
    def _extract_type_hints(self, module: cst.Module) ->Dict[str, str]:
        ...
        class TypeHintCollector(cst.CSTVisitor):
            ...
            def visit_FunctionDef(self, node: cst.FunctionDef) ->None:
                if node.returns:
                    type_hints[f"{node.name}_return"
                        ] = node.returns.annotation.value
                for param in node.params.params:
                    if param.annotation:
                        type_hints[f"{node.name}_{param.name.value}"
...
class CodeTransformer:
    ...
    def add_function(self, code: str, func_name: str, func_body: str, args:
        List[str]=None, return_type: str = None, docstring: str = None) ->str:
    ...
    def add_import(self, code: str, import_name: str, from_module: str = None
        ...
        if from_module:
            import_stmt = cst.SimpleStatementLine(body=[cst.ImportFrom(
                module = cst.Attribute(value = cst.Name(from_module.split('.')[
                0]), attr = cst.Name('.'.join(from_module.split('.')[1:]))) if
                '.' in from_module else cst.Name(from_module), names=[cst.
                ImportAlias(name = cst.Name(import_name))])])
        else:
            import_stmt = cst.SimpleStatementLine(body=[cst.Import(names=[
                cst.ImportAlias(name = cst.Name(import_name))])])
            ...
            def leave_FunctionDef(self, original_node: cst.FunctionDef,
                ...
                for param in updated_node.params.params:
                    if not param.annotation:
                        new_param = param.with_changes(annotation = cst.
                            Annotation(annotation = cst.Name('Any')))
                        new_params.append(new_param)
                    else:
                        new_params.append(param)
                return updated_node.with_changes(params = updated_node.params
                    .with_changes(params = new_params))
        ...
        class ErrorHandlerAdder(cst.CSTTransformer):
            ...
            def leave_FunctionDef(self, original_node: cst.FunctionDef,
                ...
                if updated_node.name.value != func_name:
                    return updated_node
                try_stmt = cst.Try(body = updated_node.body, handlers=[cst.
                    ExceptHandler(type = cst.Name('Exception'), name = cst.
                    AsName(name = cst.Name('e')), body = cst.IndentedBlock(body
                    =[cst.SimpleStatementLine(body=[cst.Raise(exc = cst.Name(
                    'e'))])]))])
...
class CodeValidator:
    ...
    def check_complexity(self, code: str, max_complexity: int = 10) ->Tuple[
        bool, int]:
