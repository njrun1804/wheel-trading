...
class LightweightCodeEmbedder:
    ...
    def __init__(self, vector_dim: int = 128):
        ...
        self.tfidf = TfidfVectorizer(
            max_features=1000,
            token_pattern="\\b[a-zA-Z_][a-zA-Z0-9_]*\\b",
    ...
    def _extract_structural_features(self, code: str) -> np.ndarray:
        ...
        try:
            ...
            for i, (_category, count) in enumerate(import_types.items()):
                if i + 4 < len(features):
                    features[i + 4] = min(count / 5, 1.0)
            func_patterns = self._analyze_function_patterns(context.functions)
            offset = 10
            for i, (_pattern, score) in enumerate(func_patterns.items()):
...
class MLXCodeEmbedder:
    ...
    def embed(self, code: str) -> np.ndarray:
        """Generate embedding using MLX with proper memory management."""
        if not self.use_mlx:
        ...
        try:
            ...
            # Use safe no_grad context for compatibility
            from ...einstein.mlx_no_grad_fix import safe_no_grad_context

            with safe_no_grad_context():
                outputs = self.model(input_ids)
            # Evaluate immediately to complete computation
            self.mx.eval(outputs)
