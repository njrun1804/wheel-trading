...
class DeviceRouter:
    ...
    def __init__(self):
        ...
        logger.info(f"Device Router initialized on {'M4 Pro' if self.is_m4pro else platform.machine()}")
    ...
    def _build_routing_table(self) -> Dict[OperationType, Backend]:
        ...
        # Neural operations - prefer MLX
        for op in [OperationType.NEURAL_FORWARD, OperationType.MATRIX_MULTIPLY,
        ...
        # Tree search - test GPU vs CPU
        routing[OperationType.TREE_SEARCH] = Backend.CPU_NATIVE  # Default, benchmark will decide
        
        # Vector search - CPU is fast
        routing[OperationType.VECTOR_SEARCH] = Backend.CPU_NATIVE
        
        # Training - PyTorch for now
    ...
    def benchmark_operation(self, operation: OperationType, 
                          input_size: tuple, 
                          iterations: int = 100) -> Dict[Backend, float]:
    ...
    def select_optimal_backend(self, operation: OperationType, 
                             input_size: tuple) -> Backend:
