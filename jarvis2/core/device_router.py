...
if IS_M4PRO:
    ...
    os.environ['PYTORCH_METAL_WORKSPACE_LIMIT_BYTES'] = str(18 * 1024 * 
        1024 * 1024)
...
class DeviceRouter:
    ...
    def _build_routing_table(self) ->Dict[OperationType, Backend]:
        ...
        routing = {}
        for op in [OperationType.NEURAL_FORWARD, OperationType.
            MATRIX_MULTIPLY, OperationType.EMBEDDING]:
    ...
    def benchmark_operation(self, operation: OperationType, input_size:
        tuple, iterations: int = 100) ->Dict[Backend, float]:
        ...
        for backend in self._backends:
            ...
            try:
                ...
                logger.info(
                    f"{backend.value}: {results[backend] * 1000:.2f}ms per op")
            except Exception as e:
                logger.error(f"Benchmark failed for {backend}: {e}")
        return results

    def select_optimal_backend(self, operation: OperationType, input_size:
        tuple) ->Backend:
        ...
        if results:
            ...
            logger.info(
                f"Optimal backend for {operation.value}: {optimal.value}")
