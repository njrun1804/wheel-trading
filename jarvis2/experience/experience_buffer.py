...
class ExperienceReplaySystem:
    ...
    def __init__(
        self, storage_path: Path, buffer_size: int = 10000, prioritized: bool = True
    ):
    ...
    def _calculate_priority(
        self, experience: Experience, evaluations: list[dict[str, Any]]
    ) -> float:
        ...
        priority = (
            0.4 * confidence_factor + 0.3 * complexity_factor + 0.3 * diversity_factor
        )
    ...
    def update_priorities(self, indices: list[int], td_errors: list[float]):
        ...
        for idx, td_error in zip(indices, td_errors, strict=False):
            if 0 <= idx < len(self.memory_buffer):
                priority = (
                    abs(td_error) + self.priority_epsilon
                ) ** self.priority_alpha
                exp, _ = self.memory_buffer[idx]
    ...
    async def get_statistics(self) -> dict[str, Any]:
        ...
        if self.memory_buffer:
            ...
            confidences = [
                exp.metrics.get("confidence_score", 0) for exp, _ in self.memory_buffer
            ]
            complexities = [
                exp.metrics.get("complexity_score", 0) for exp, _ in self.memory_buffer
            ]
        else:
        ...
        return {
            ...
            "success_rate": self.successful_experiences
            / max(1, self.total_experiences),
            "memory_stats": {
    ...
    async def _update_statistics(self):
        ...
        """,
            (current_hour * 3600, (current_hour + 1) * 3600),
        ).fetchone()
        if stats[0] > 0:
            self.db_connection.execute(
                """
...
class ExperienceAnalyzer:
    ...
    def _analyze_query_patterns(self) -> dict[str, Any]:
        ...
        for row in cursor:
            patterns.append(
                {"query": row[0], "count": row[1], "avg_reward": row[2] or 0}
            )
        return {
