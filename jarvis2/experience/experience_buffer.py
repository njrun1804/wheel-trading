...
class ExperienceReplaySystem:
    ...
    def __init__(self, storage_path: Path, 
        ...
        self.priority_alpha = 0.6  # How much to use priorities
        self.priority_beta = 0.4  # Importance sampling
    ...
    async def initialize(self):
        ...
        logger.info(f"Experience replay initialized with {len(self.memory_buffer)} recent experiences")
    
    async def record(self, query: str, solution: Dict[str, Any],
        ...
        experience = Experience(
            ...
            metrics={
                ...
                'simulations_performed': metrics.simulations_performed,
    ...
    def _calculate_priority(self, experience: Experience,
                          evaluations: List[Dict[str, Any]]) -> float:
        """Calculate priority for experience replay."""
        # Base priority on multiple factors
        confidence = experience.metrics.get('confidence_score', 0.5)
        complexity = experience.metrics.get('complexity_score', 0.5)
        
        # Higher priority for:
        # - Medium confidence (most to learn from)
        # - High complexity (harder problems)
        # - Diverse evaluations (interesting cases)
        
        confidence_factor = 4 * confidence * (1 - confidence)  # Peaks at 0.5
        complexity_factor = complexity
        
        # Evaluation diversity
        if evaluations:
            eval_scores = [e.get('overall', 0) for e in evaluations]
            diversity_factor = np.std(eval_scores) if len(eval_scores) > 1 else 0.5
        else:
    ...
    async def _persist_experience(self, experience: Experience, priority: float):
        ...
        """, (
            ...
            json.dumps(experience.solution),
            json.dumps(experience.metrics),
            experience._calculate_reward(),
            experience.was_selected,
    ...
    async def sample(self, batch_size: int = 32,
                    beta: Optional[float] = None) -> List[Dict[str, Any]]:
    ...
    async def _prioritized_sample(self, batch_size: int,
                                beta: Optional[float] = None) -> List[Dict[str, Any]]:
        """Sample using prioritized experience replay."""
        beta = beta or self.priority_beta
        
        # Get priorities from memory buffer
    ...
    async def get_statistics(self) -> Dict[str, Any]:
        ...
        if self.memory_buffer:
            ...
            confidences = [exp.metrics.get('confidence_score', 0) for exp, _ in self.memory_buffer]
            complexities = [exp.metrics.get('complexity_score', 0) for exp, _ in self.memory_buffer]
        else:
        ...
        return {
            ...
            'success_rate': self.successful_experiences / max(1, self.total_experiences),
            'memory_stats': {
            ...
            'database_stats': {
                'total_stored': db_stats[0],
                'avg_reward': db_stats[1] or 0,
                'max_reward': db_stats[2] or 0,
    ...
    async def get_learning_curves(self, window_size: int = 100) -> Dict[str, List[float]]:
        """Get learning curves over time."""
        # Get experiences ordered by time
        cursor = self.db_connection.execute("""
            SELECT 
                reward,
    ...
    def _analyze_query_patterns(self) -> Dict[str, Any]:
        ...
        for row in cursor:
            patterns.append({
                'query': row[0],
                'count': row[1],
                'avg_reward': row[2] or 0
            })
        
        return {
            'most_common_queries': patterns,
            'total_unique_queries': self.buffer.db_connection.execute(
                "SELECT COUNT(DISTINCT query) FROM experiences"
            ).fetchone()[0]
        }
