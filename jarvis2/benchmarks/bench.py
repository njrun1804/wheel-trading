...
class BenchmarkSuite:
    ...
    def __init__(self, backend: str, batch_size: int = 4096):
        ...
        if backend == "mlx":
            ...
            self.mx = mx
            self.device = mx.gpu
            print(f"MLX initialized on Metal GPU")
        elif backend == "torch":
            import torch
            self.torch = torch
            if torch.backends.mps.is_available():
                self.device = torch.device('mps')
                print(f"PyTorch initialized on MPS")
            else:
                self.device = torch.device('cpu')
                print(f"PyTorch initialized on CPU")
    ...
    def benchmark_embedding(self, vocab_size: int = 50000, embed_dim: int = 768):
        ...
        if self.backend == "mlx":
            ...
            indices = self.torch.randint(0, vocab_size, (self.batch_size, 128), device=self.device)
    ...
    def benchmark_tree_ops(self, num_nodes: int = 10000):
        ...
        if self.backend == "mlx":
            ...
            def ucb(values, visits, parent_visits, c_puct):
                return values + c_puct * self.mx.sqrt(self.mx.log(parent_visits) / visits)
            ...
            visits = self.torch.randint(1, 100, (num_nodes,), device=self.device, dtype=self.torch.float32)
            parent_visits = self.torch.full((num_nodes,), 1000, device=self.device, dtype=self.torch.float32)
            c_puct = 1.414
            
            def ucb(values, visits, parent_visits, c_puct):
                return values + c_puct * self.torch.sqrt(self.torch.log(parent_visits) / visits)
    ...
    def run_all(self):
        """Run all benchmarks."""
        print(f"\n{'='*60}")
        print(f"Running benchmarks on {self.backend.upper()} (batch_size={self.batch_size})")
        print(f"{'='*60}")
...
def compare_backends(batch_size: int = 4096):
    ...
    if len(results) == 2:
        ...
        print(f"{'Operation':15s} {'MLX (ms)':>10s} {'PyTorch (ms)':>15s} {'Speedup':>10s}")
        print("-" * 55)
...
def main():
    ...
    parser.add_argument("--backend", choices=["mlx", "torch"], help="Backend to benchmark")
