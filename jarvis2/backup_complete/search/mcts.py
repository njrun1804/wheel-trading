...
from ..neural.lazy_networks import LazyValueNetwork, LazyPolicyNetwork
...
class MCTSConfig:
    ...
    def __init__(self):
        self.c_puct = 1.414  # Exploration constant
        self.batch_size = 256  # GPU batch size
        self.virtual_loss = 3.0  # For parallel MCTS
        self.max_depth = 10
        self.temperature = 1.0
        self.epsilon = 0.25  # Dirichlet noise
        self.alpha = 0.3  # Dirichlet alpha
...
class NeuralGuidedMCTS:
    ...
    def __init__(self, config: Optional[MCTSConfig] = None):
        ...
        # Neural networks - use lazy loading to avoid init deadlocks
    ...
    async def initialize(self):
        ...
        # Networks will be initialized on first use
    ...
    async def explore(self, query: str, context: Dict[str, Any], 
        ...
        # Store hardware executor if provided
        ...
        logger.info(
            f"MCTS complete: {simulations} simulations in {elapsed:.2f}s "
            f"({simulations/elapsed:.0f} sims/sec)"
        )
    ...
    async def fast_search(self, query: str, context: Dict[str, Any],
        ...
        # Use greedy policy with limited exploration
    ...
    async def _run_batch_simulations(self, root: SearchNode, query: str,
                                   context: Dict[str, Any], batch_size: int):
        """Run a batch of simulations in parallel."""
        # Select batch_size leaf nodes for expansion
        paths = []
    ...
    def _select_child(self, node: SearchNode) -> SearchNode:
        ...
        for child in node.children:
            ...
            u_value = self.config.c_puct * child.prior_probability * \
    ...
    async def _batch_evaluate(self, nodes: List[SearchNode], query: str,
                            context: Dict[str, Any]) -> Tuple[np.ndarray, List[np.ndarray]]:
    ...
    def _extract_features(self, node: SearchNode, query: str, 
        ...
        # Code state embedding (simplified)
    ...
    def _embed_state(self, state: Any) -> np.ndarray:
        ...
        if isinstance(state, str):
            # Simple hash-based embedding for code strings
            features = []
            # Length features
    ...
    async def _expand_node(self, node: SearchNode, action_probs: np.ndarray,
        ...
        # Sample top-k actions based on policy
        k = min(5, len(valid_actions))  # Expand top-5 actions
        ...
        for idx in action_indices:
            if idx < len(valid_actions):
                ...
                # Apply action to generate new code
    ...
    def _backpropagate(self, path: List[SearchNode], value: float):
        ...
        for node in reversed(path):
            ...
            # In code generation, we don't have opponents, so value stays same
    
    async def _add_exploration_noise(self, root: SearchNode):
        """Add Dirichlet noise to root node for exploration."""
        if not root.children:
            # Expand root first
            dummy_probs = np.ones(10) / 10  # Uniform priors
        ...
        if root.children:
            ...
            for i, child in enumerate(root.children):
                child.prior_probability = \
                    (1 - self.config.epsilon) * child.prior_probability + \
                    self.config.epsilon * noise[i]
    
    async def _warmup_models(self):
        """Warm up neural networks."""
        try:
            # Create smaller dummy batch for faster warmup
    ...
    async def train_batch(self, experiences: List[Dict[str, Any]]) -> Tuple[float, float]:
        """Train networks on a batch of experiences."""
        if not experiences:
            return 0.0, 0.0
        
        # Extract states and values from experiences
        states = []
        values = []
        actions = []
        advantages = []
        ...
        # Train value network
        value_data = list(zip(states, values))
        value_metrics = await self.value_net.train_batch(value_data)
        
        # Train policy network  
        policy_data = list(zip(states, zip(actions, advantages)))
...
class CodeActionSpace:
    ...
    def __init__(self):
        # Define possible code transformation actions
    ...
    def get_valid_actions(self, node: SearchNode, query: str, 
                         context: Dict[str, Any]) -> List[Dict[str, Any]]:
        """Get valid actions for current code state."""
        # In practice, would analyze code AST to determine valid actions
        # For now, return all actions if we're not too deep
        ...
        for action in self.action_templates:
            ...
            if "optimize" in query_lower and "optimize" in action["type"]:
                valid_actions.append(action)
            elif "test" in query_lower and "test" in action["type"]:
                valid_actions.append(action)
            elif "error" in query_lower and "error" in action["type"]:
                valid_actions.append(action)
            elif node.depth < 3:  # Early stages - all actions valid
                valid_actions.append(action)
        
        return valid_actions or self.action_templates[:5]  # Fallback
    
    def apply_action(self, current_code: str, action: Dict[str, Any]) -> str:
        """Apply an action to generate new code."""
        # Simplified code generation
        # In practice, would use AST manipulation or LLM
...
class MCTSVisualizer:
    ...
    def print_tree(root: SearchNode, max_depth: int = 3):
        ...
        def _print_node(node: SearchNode, depth: int = 0, prefix: str = ""):
            ...
            indent = "  " * depth
            print(f"{indent}{prefix}Visits: {node.visits}, "
