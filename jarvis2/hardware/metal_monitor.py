...
class MetalGPUMonitor:
    ...
    def _get_ioreg_stats(self) ->Optional[Dict[str, float]]:
        """Try to get GPU stats from ioreg."""
        try:
            output = subprocess.check_output(['ioreg', '-r', '-d', '1',
                '-w', '0', '-c', 'AGXAccelerator'], text = True, stderr = subprocess.DEVNULL)
            stats = {}
        ...
        try:
            for proc in psutil.process_iter(['pid', 'name', 'memory_info']):
                try:
                    ...
                    gpu_keywords = ['python', 'julia',
                        'metalperformanceshaders', 'tensorflow', 'pytorch',
                        'mlx']
                    if any(keyword in name for keyword in gpu_keywords):
                        memory_mb = pinfo['memory_info'].rss / (1024 * 1024)
                        if memory_mb > 100:
                            gpu_processes.append({'pid': pinfo['pid'],
                                'name': pinfo['name'], 'memory_mb': memory_mb})
    ...
    def get_metrics(self) ->Dict[str, float]:
        """Get all GPU metrics."""
        return {'utilization_percent': self._gpu_utilization,
            'memory_used_mb': self._gpu_memory_used, 'monitoring': self.
            _monitoring}
...
class EnhancedResourceMonitor:
    ...
    def get_system_metrics(self) ->Dict[str, any]:
        ...
        return {'timestamp': time.time(), 'uptime': time.time() - self.
            _start_time, 'cpu': {'percent_per_core': cpu_percent,
            'percent_avg': sum(cpu_percent) / len(cpu_percent),
            'frequency_mhz': cpu_freq.current if cpu_freq else 0,
            'frequency_max_mhz': cpu_freq.max if cpu_freq else 0}, 'memory':
            {'total_gb': vm.total / 1024 ** 3, 'used_gb': vm.used / 1024 **
            3, 'available_gb': vm.available / 1024 ** 3, 'percent': vm.
            percent, 'swap_used_gb': swap.used / 1024 ** 3, 'swap_percent':
            swap.percent}, 'gpu': gpu_metrics, 'io': {'disk_read_mb': 
            disk_io.read_bytes / 1024 ** 2 if disk_io else 0,
            'disk_write_mb': disk_io.write_bytes / 1024 ** 2 if disk_io else
            0, 'network_sent_mb': net_io.bytes_sent / 1024 ** 2 if net_io else
            0, 'network_recv_mb': net_io.bytes_recv / 1024 ** 2 if net_io else
            0}, 'temperature': temps}
    ...
    def _get_temperatures(self) ->Dict[str, float]:
        ...
        try:
            ...
            if not temps:
                try:
                    output = subprocess.check_output(['sudo', '-n',
                        'powermetrics', '-i', '1', '-n', '1', '-s',
                        'thermal'], text = True, stderr = subprocess.DEVNULL,
                        timeout = 2)
                    for line in output.split('\n'):
                        if 'temperature' in line.lower():
                            ...
                            if len(parts) == 2:
                                try:
                                    temp = float(parts[1].strip().replace(
                                        'C', ''))
                                    temps['system'] = temp
                                    break
                                except Exception as e:
