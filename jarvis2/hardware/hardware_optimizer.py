...
class HardwareAwareExecutor:
    ...
    def __init__(self):
        ...
        self.memory_allocations = {
            "index_cache": int(self.usable_memory_gb * 0.2 * 1024),
            "model_cache": int(self.usable_memory_gb * 0.3 * 1024),
            "experience_buffer": int(self.usable_memory_gb * 0.2 * 1024),
            "working_memory": int(self.usable_memory_gb * 0.3 * 1024),
        }
        logger.info(
    ...
    def _configure_threading(self):
        ...
        if MLX_AVAILABLE:
            ...
            os.environ["MLX_GPU_MEMORY_LIMIT"] = str(
                int(self.usable_memory_gb * 0.5 * 1024)
            )
        if MPS_AVAILABLE:
    ...
    async def initialize(self):
        ...
        if self.p_core_executor is None:
            self.p_core_executor = ThreadPoolExecutor(
                max_workers=self.p_cores, thread_name_prefix="p_core"
            )
        if self.e_core_executor is None:
            self.e_core_executor = ThreadPoolExecutor(
                max_workers=self.e_cores, thread_name_prefix="e_core"
            )
        self._monitor_task = None
    ...
    async def _execute_on_p_cores(self, task: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            result = await loop.run_in_executor(
                self.p_core_executor, func, *args, **kwargs
            )
    ...
    async def _execute_on_e_cores(self, task: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            result = await loop.run_in_executor(
                self.e_core_executor, func, *args, **kwargs
            )
            execution_time = time.time() - start_time
            return {
                "status": "success",
    ...
    async def _execute_with_memory_optimization(
        ...
        try:
            while True:
                try:
                    ...
                    if cpu_avg > 90 or memory_percent > 85 or self.gpu_utilization > 90:
                        logger.warning(
    ...
    def get_optimal_batch_size(
        self, task_type: str = "general", item_size_mb: float = 10
    ) -> int:
        ...
        while True:
            try:
                ...
                if pending_tasks:
                    ...
                    batch_size = min(
                        len(pending_tasks), self.executor.get_optimal_batch_size()
                    )
...
class MemoryOptimizer:
    ...
    def allocate_pool(self, name: str, size_mb: int) -> bool:
        ...
        if size_mb > available_mb * 0.8:
            ...
            return False
        self.memory_pools[name] = {"size_mb": size_mb, "allocated_at": time.time()}
