...
class DeepIndexManager:
    ...
    async def _init_kv_store(self):
        ...
        try:
            ...
            self.kv_env = lmdb.open(
                str(kv_path), map_size=100 * 1024 * 1024, max_dbs=10, sync=False
            )
    ...
    async def _index_file(self, file_path: Path):
        ...
        with self.kv_env.begin(write=True) as txn:
            txn.put(
                f"emb:{file_path}".encode(), embedding.tobytes(), db=self.kv_embeddings
            )
        for imp in metadata["imports"]:
            self.dependency_graph.add_edge(str(file_path), imp)
        with self.kv_env.begin(write=True) as txn:
            txn.put(
                f"ast:{file_path}".encode(), pickle.dumps(tree), db=self.kv_ast_cache
            )
    ...
    def _extract_metadata(self, tree: ast.AST, content: str) -> dict[str, Any]:
        ...
        code_lines = (
            total_lines - comment_lines - sum(1 for line in lines if not line.strip())
        )
        return {
    ...
    def get_index_stats(self) -> dict[str, Any]:
        ...
        fts_count = self.fts_db.execute("SELECT COUNT(*) FROM code_search").fetchone()[
            0
        ]
