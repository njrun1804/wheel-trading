...
class MetalGPUMonitor:
    ...
    def _update_metrics(self):
        ...
        # Method 1: Use ioreg to get GPU metrics
    ...
    def _get_ioreg_stats(self) -> Optional[Dict[str, float]]:
        ...
        try:
            ...
            # Parse output for relevant metrics
            for line in output.split('\n'):
                ...
                if '"PercentUtilization"' in line:
                    try:
                        ...
                        logger.debug(f"Failed to parse utilization from line: {line.strip()}, error: {e}")
                ...
                if '"AllocatedSize"' in line:
                    try:
                        ...
                        logger.debug(f"Failed to parse memory size from line: {line.strip()}, error: {e}")
    ...
    def _estimate_gpu_usage(self):
        ...
        try:
            ...
            # Check if common ML frameworks are using memory
            ...
            if gpu_processes:
                ...
                # Assume GPU is utilized proportionally to memory usage
                # (rough estimate for unified memory architecture)
                max_gpu_memory = vm.total * 0.5 / (1024 * 1024)  # 50% of total
        ...
        try:
            for proc in psutil.process_iter(['pid', 'name', 'memory_info']):
                try:
                    ...
                    # Check for known GPU-using processes
                    gpu_keywords = [
                        'python',  # ML frameworks
                        'julia',   # Julia with Metal
                    ...
                    if any(keyword in name for keyword in gpu_keywords):
                        # Check if process has significant memory usage
...
class EnhancedResourceMonitor:
    ...
    def _get_temperatures(self) -> Dict[str, float]:
        ...
        try:
            ...
            # On macOS, try powermetrics (requires sudo)
            if not temps:
                try:
                    # This would need sudo access
                    output = subprocess.check_output(
                        ['sudo', '-n', 'powermetrics', '-i', '1', '-n', '1', '-s', 'thermal'],
                        text=True, stderr=subprocess.DEVNULL, timeout=2
                    )
                    # Parse output for temperatures
                    for line in output.split('\n'):
