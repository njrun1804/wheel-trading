...
# Import only the enums at module level, not the DeviceRouter class
# The DeviceRouter will be imported inside the worker process
...
class NeuralRequest:
    ...
    id: str
    type: str  # 'value', 'policy', 'embedding'
    data: np.ndarray
...
class NeuralWorkerProcess:
    ...
    def start(self):
        ...
        # Wait for initialization (spawn method is slower than fork)
        if not self.ready_event.wait(timeout=60):
            raise RuntimeError(f"Neural worker {self.worker_id} failed to initialize")
        ...
        while not shutdown_event.is_set():
            try:
                ...
                response = NeuralResponse(
                    ...
                    backend_used=models.current_backend
                )
                ...
                if 'request' in locals():
                    response_queue.put(NeuralResponse(
                        id=request.id,
                        result=np.array([]),
                        compute_time_ms=-1,
                        backend_used="error"
...
class NeuralModels:
    ...
    def __init__(self):
        # Create a fresh DeviceRouter instance in the worker process
        # This ensures all heavy imports happen in the subprocess
    ...
    def _init_torch_models(self):
        ...
        device = self.router.get_device(
            self.router.route(OperationType.NEURAL_FORWARD)
        )
...
class NeuralWorkerPool:
    ...
    async def compute_async(self, request_type: str, 
                          data: np.ndarray,
        ...
        # Check queue depth (not available on macOS with spawn)
        try:
            queue_size = worker.request_queue.qsize()
            if queue_size > 50:
                logger.warning(f"Worker {worker.worker_id} queue depth: {queue_size}")
        except NotImplementedError:
            # qsize() not available on macOS with spawn method
            pass
        ...
        while (time.perf_counter() - start_time) < timeout:
            try:
                ...
                if response.id == request.id:
                    ...
                    logger.debug(f"Neural compute took {response.compute_time_ms:.1f}ms on {response.backend_used}")
            ...
            if not worker.process.is_alive():
                raise RuntimeError(f"Worker {worker.worker_id} died during computation")
