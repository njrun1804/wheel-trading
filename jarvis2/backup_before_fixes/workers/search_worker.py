...
class CodeActionSpace:
    ...
    def get_actions(self, state: str, policy_probs: Optional[np.ndarray] = None) -> List[Tuple[str, float]]:
    ...
    def _filter_relevant_actions(self, state: str, context) -> List[str]:
        ...
        if not context.functions:
            ...
            relevant.extend(["modify_function", "add_type_hint", "add_docstring", 
                           "add_error_handling", "optimize_algorithm"])
    ...
    def _generate_action(self, action_type: str, state: str, context) -> str:
        ...
        try:
            if action_type == "add_function":
                ...
                class_query = f"create a class for {query}" if query else "create a utility class"
                from core.code_generator import GenerationRequest
                req = GenerationRequest(
                    query=class_query,
    ...
    def _infer_needed_import(self, state: str, context) -> Optional[str]:
