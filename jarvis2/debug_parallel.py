...
async def test_minimal():
    ...
    try:
        ...
        try:
            sol2, sol3 = await asyncio.wait_for(asyncio.gather(task2, task3
                ), timeout = 30.0)
            t2 = time.time() - start
...
async def test_components():
    ...
    try:
        ...
        tasks = [pool.compute_async('value', data), pool.compute_async(
            'policy', data)]
    ...
    try:
        result = await pool.parallel_search('test query', {}, {'value': np.
            array([[0.5]]), 'policy': np.ones(50) / 50}, simulations = 20)
        print(
