                                ...
                                self.issues.append(
                                    {
                                        ...
                                        "name": node.name,
                                        "file": str(
                                            py_file.relative_to(self.jarvis_path)
                                        ),
                                        "line": node.lineno,
                ...
                for node in ast.walk(tree):
                    if isinstance(node, ast.AsyncFunctionDef):
                        ...
                        for child in ast.walk(node):
                            if isinstance(
                                child, ast.Await | ast.AsyncWith | ast.AsyncFor
                            ):
                                has_await = True
                                break
            ...
            for issue_type, issues in issues_by_type.items():
                ...
                for issue in issues[:5]:  # Show first 5
                    if issue_type == "anti-pattern":
                        ...
                        print(
                            f"  {issue['file']}:{issue['line']} - {issue['name']}() - {issue['reason']}"
                        )
                    elif issue_type == "async_without_await":
                        print(
                            f"  {issue['file']}:{issue['line']} - async {issue['name']}() has no await"
                        )
                    elif issue_type == "hardcoded_value":
                        print(
                            f"  {issue['file']}:{issue['line']} - {issue['value_type']}: {issue['content']}"
                        )
