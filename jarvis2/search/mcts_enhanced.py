...
class MLXPolicyNetwork(nn.Module):
    ...
    def __init__(
        self, input_dim: int = 768, hidden_dim: int = 512, action_dim: int = 128
    ):
        super().__init__()
        self.layers = [
...
class HardwareAcceleratedMCTS:
    ...
    def __init__(self, config: MCTSSearchConfig | None = None):
        ...
        self.executor = concurrent.futures.ThreadPoolExecutor(
            max_workers=self.config.parallel_cores, thread_name_prefix="MCTS"
        )
    ...
    async def initialize(self):
        ...
        logger.info(
            f"🔧 Hardware: {hw_info['cpu_cores']} cores, {hw_info['gpu_cores']} GPU cores"
        )
    ...
    async def _warmup_mlx_networks(self):
        ...
        try:
            ...
            policy_output = self.policy_net(dummy_input)
            value_output = self.value_net(dummy_input)

            # Ensure evaluation completes
            mx.eval(policy_output, value_output)

            logger.info("🔥 MLX networks warmed up")
        finally:
            # Clean up warmup arrays
            for arr in [dummy_input, policy_output, value_output]:
                if arr is not None:
                    del arr
            # Force cleanup
            import gc
    ...
    async def search(
        self, query: str, context: dict[str, Any], max_simulations: int | None = None
    ) -> SearchNode:
        """
        ...
        self.root_node = SearchNode(
            state={"query": query, "context": context}, parent=None
        )

        logger.info(
            f"🔍 Starting accelerated search: {max_sims} simulations on {self.config.parallel_cores} cores"
        )
        ...
        self.search_statistics[
            "parallel_efficiency"
        ] = self._calculate_parallel_efficiency(search_time, max_sims)

        logger.info(
            f"✅ Search complete: {search_time:.2f}s, efficiency: {self.search_statistics['parallel_efficiency']:.1%}"
        )
    ...
    async def _core_simulation_batch(self, num_simulations: int, core_id: int):
        ...
        for sim_idx in range(num_simulations):
            try:
                ...
                if (
                    self.config.batch_neural_eval
                    and sim_idx % self.config.batch_size == 0
                ):
    ...
    async def _expand_node(self, node: SearchNode) -> SearchNode:
        ...
        for action in actions[:8]:  # Limit branching factor
            ...
            child = SearchNode(state=child_state, parent=node, action=action)
    ...
    async def _evaluate_node(self, node: SearchNode):
        """Evaluate node using neural value network with memory management"""
        ...
        if self.config.use_mlx and MLX_AVAILABLE:
            ...
            try:
                ...
                value = self.value_net(features_array)
                mx.eval(value)  # Ensure computation completes
                node.value = float(value[0])
            finally:
                # Clean up MLX arrays
                for arr in [features_array, value]:
                    if arr is not None:
                        del arr
        else:
            device = torch.device(self.device_type)
    ...
    async def _batch_evaluate_nodes(self, nodes: list[SearchNode]):
        """Batch evaluate multiple nodes for efficiency with memory management"""
        if not nodes:
            return
        ...
        if self.config.use_mlx and MLX_AVAILABLE:
            ...
            try:
                ...
                mx.eval(values)  # Ensure computation completes
                for i, node in enumerate(nodes):
                    node.value = float(values[i])
            finally:
                # Clean up MLX arrays
                for arr in [batch_array, values]:
                    if arr is not None:
                        del arr
            ...
            batch_tensor = torch.tensor(
                features_batch, dtype=torch.float32, device=device
            )
            with torch.no_grad():
    ...
    def _select_best_child(self, root: SearchNode) -> SearchNode:
        ...
        best_child = max(
            ...
            + (child.accumulated_value / max(child.visit_count, 1)),
        )
        return best_child

    def _calculate_parallel_efficiency(
        self, search_time: float, simulations: int
    ) -> float:
        ...
        # Actual speedup
        actual_speedup = (
            theoretical_single_core_time / search_time if search_time > 0 else 1.0
        )
...
def create_hardware_accelerated_mcts(
    config: MCTSSearchConfig | None = None,
) -> HardwareAcceleratedMCTS:
