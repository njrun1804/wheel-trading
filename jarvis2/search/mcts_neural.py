...
class NeuralPolicy:
    ...
    def __init__(self):
        ...
        config = get_config()
        self.weights = {
            'policy': np.random.randn(config.neural.input_features, config.neural.hidden_size) * config.neural.weight_init_scale,
            'value': np.random.randn(config.neural.hidden_size, 1) * config.neural.weight_init_scale
        }
    ...
    def update(self, state_features: np.ndarray, target_policy: np.ndarray, target_value: float):
...
class Jarvis2MCTS:
    ...
    def __init__(self, neural_policy: NeuralPolicy):
        ...
        print(f"🎯 Jarvis2 MCTS initialized")
    ...
    async def search(self, initial_context: Dict[str, Any], simulations: int = None) -> Dict[str, Any]:
        ...
        return {
            ...
            'alternatives': [(c.action, c.visits, c.value_sum/max(c.visits,1)) 
    ...
    def learn_from_outcome(self, context: Dict[str, Any], chosen_action: str, 
