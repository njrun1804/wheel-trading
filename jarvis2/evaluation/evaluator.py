...
class MultiObjectiveEvaluator:
    ...
    async def _evaluate_single(
        ...
        overall = sum(
            results.get(m, 0) * self.objective_weights.get(m, 0.25) for m in metrics
        )
...
class BenchmarkRunner:
    ...
    async def benchmark_solution(
        ...
        results = {"execution_time": 0.0, "memory_usage": 0.0, "throughput": 0.0}
        ...
        try:
            for test_case in test_cases[:3]:
                ...
                stdout, stderr = await asyncio.wait_for(
                    process.communicate(input_data), timeout=5.0
                )
        ...
        try:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.FunctionDef):
                    assigns_global = any(
                        isinstance(n, ast.Global) for n in ast.walk(node)
                    )
