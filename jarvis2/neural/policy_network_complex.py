...
class CodePolicyNetwork(nn.Module):
    ...
    def __init__(
        ...
        self.location_head = nn.Sequential(
            nn.Linear(hidden_dim, 256), nn.ReLU(), nn.Linear(256, 100)
        )
    ...
    def sample_action(
        self, x: torch.Tensor, temperature: float = 1.0, top_k: int | None = None
    ) -> dict[str, Any]:
        ...
        with torch.no_grad():
            ...
            if top_k is not None:
                top_probs, top_indices = torch.topk(
                    action_probs, min(top_k, action_probs.size(-1))
                )
    ...
    async def train_batch(self, data: list[tuple[Any, Any]]) -> float:
        ...
        actions = torch.tensor(
            [x[1].get("action_type", 0) for x in data], dtype=torch.long
        )
        logits = self(states)
