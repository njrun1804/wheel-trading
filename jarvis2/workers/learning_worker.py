...
class ReplayBuffer:
    ...
    def sample(self, batch_size: int) -> list[Experience]:
        ...
        indices = np.random.choice(
            self.size, size=min(batch_size, self.size), p=probs, replace=False
        )
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "mean_priority": float(np.mean(self.priorities[: self.size]))
            if self.size > 0
            else 0,
        }
...
class ModelTrainer:
    ...
    def _init_torch_models(self):
        ...
        self.policy_optimizer = optim.Adam(self.policy_model.parameters(), lr=0.0001)
        self.torch = torch
        self.device = device
    ...
    def _train_mlx(self, batch: LearningBatch) -> tuple[ModelUpdate, ModelUpdate]:
        ...
        for exp in batch.experiences:
            ...
            for i, _action in enumerate(exp.policy_actions[:50]):
        ...
        for _ in range(10):
            ...
            loss, grads = self.mx.value_and_grad(value_loss_fn)(
                self.value_model, X, y_value
            )
        ...
        for _ in range(10):
            ...
            def policy_loss_fn(model, X, y):
                ...
                return -self.mx.mean(
                    self.mx.sum(y * self.mx.log(probs + 1e-08), axis=-1)
                )

            loss, grads = self.mx.value_and_grad(policy_loss_fn)(
                self.policy_model, X, y_policy
            )
    ...
    def _train_torch(self, batch: LearningBatch) -> tuple[ModelUpdate, ModelUpdate]:
        ...
        y_value = self.torch.tensor(value_targets, dtype=self.torch.float32).unsqueeze(
            1
        )
...
class LearningWorker:
    ...
    def get_model_update(self) -> ModelUpdate | None:
        ...
        while not shutdown_event.is_set():
            try:
                ...
                if replay_buffer.size >= batch_size:
                    ...
                    batch = LearningBatch(
                        experiences=experiences, batch_id=f"batch_{experience_count}"
                    )
    ...
    @staticmethod
    def _save_models(
        model_dir: Path, value_update: ModelUpdate, policy_update: ModelUpdate
    ):
