...
class NeuralModels:
    ...
    def _init_torch_models(self):
        ...
        device = self.router.get_device(self.router.route(OperationType.NEURAL_FORWARD))
    ...
    def _process_mlx(self, request: NeuralRequest) -> np.ndarray:
        """Process with MLX with proper memory management."""
        data = None
        result = None
        try:
...
class NeuralWorkerPool:
    ...
    async def compute_async(
        self, request_type: str, data: np.ndarray, metadata: dict | None = None
    ) -> np.ndarray:
        """Async interface for neural computation."""
        import asyncio

        request = NeuralRequest(
            id=str(uuid.uuid4()), type=request_type, data=data, metadata=metadata or {}
        )
