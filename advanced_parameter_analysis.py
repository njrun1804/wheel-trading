    ...
    for regime_name, volatility in vol_regimes.items():
        ...
        for delta, dte, pos_size, take_profit in itertools.product(
            ...
            result = calculate_wheel_returns(
                volatility, delta, dte, pos_size, take_profit
            )
...
def correlation_analysis():
    ...
    corr_vars = [
        'returns', 'volatility_20d', 'volatility_250d', 
        'volume', 'var_95', 'risk_free_rate', 'vix',
        'prev_return', 'prev_vol', 'avg_volume_5d'
    ]
    ...
    data['assignment_risk'] = (
        (data['volatility_20d'] > 0.8) & 
        (data['returns'] < -0.05)
    ).astype(int)
...
def optimal_kelly_by_regime():
    ...
    for _, row in regime_data.iterrows():
        if row['avg_win'] and row['avg_loss']:
            ...
            print(f"{row['regime']:<8} | {p:>7.1%} | {avg_win:>7.3f} | {avg_loss:>8.3f} | "
...
def test_profit_taking_strategies():
    ...
    for vol in volatilities:
        ...
        print(f"{vol:.0%} | {returns[0]:>8.1%} | {returns[1]:>8.1%} | "
...
def earnings_window_analysis():
    ...
    for window in windows:
        ...
        print(f"{window:>6} | {trading_days:>12} | {lost_premium:>12.1%} | "
