...
class ImportUsageAnalyzer(ast.NodeVisitor):
    ...
    def __init__(self, content_lines: list[str]):
        ...
        self.name_usage: dict[str, list[int]] = defaultdict(
            list
        )  # name -> line numbers
        self.current_line = 0
...
def analyze_file_imports(file_path: Path) -> tuple[list[ImportInfo], dict[str, Any]]:
    ...
    try:
        ...
        for import_info in analyzer.imports:
            for name in import_info.names:
                if name in analyzer.name_usage:
                    ...
                    usage_lines = [
                        line
                        for line in analyzer.name_usage[name]
        ...
        stats = {
            ...
            "import_types": {
                "import": len(
                    [i for i in analyzer.imports if i.import_type == "import"]
                ),
                "from_import": len(
                    [i for i in analyzer.imports if i.import_type == "from_import"]
                ),
            },
        }
...
def generate_audit_report():
    ...
    for py_file in sorted(einstein_dir.glob("*.py")):
        ...
        if used:
            ...
            for imp in used:
                ...
                print(
                    f"   Line {imp.line_number}: {names_str} (used {usage_count} times)"
                )
    ...
    if unused_by_name:
        ...
        for name, count in sorted(
            unused_by_name.items(), key=lambda x: x[1], reverse=True
        )[:10]:
    ...
    for module, count in sorted(
        import_sources.items(), key=lambda x: x[1], reverse=True
    )[:10]:
