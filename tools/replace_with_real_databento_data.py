...
class RealDataCollector:
    ...
    async def initialize(self):
        ...
        try:
            ...
            print("   This system REQUIRES real market data. Cannot proceed without valid credentials.")
            raise
    ...
    async def collect_real_unity_data(self):
        ...
        for expiration in expirations_to_check:
            ...
            try:
                ...
                print(f"   ✅ Retrieved {real_puts} puts, {real_calls} calls ({total_options} total)")
                ...
                print(f"   ❌ Failed to get real options for {expiration.strftime('%Y-%m-%d')}: {e}")
                continue
    ...
    async def store_real_options(self, chain, expiration):
        ...
        for put in chain.puts:
            try:
                ...
                moneyness = (float(put.strike_price) - float(chain.spot_price)) / float(chain.spot_price)
                ...
                self.conn.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, [
                    'U', expiration.date(), float(put.strike_price), 'PUT',
                    ...
                    chain.timestamp, float(chain.spot_price), moneyness
                ])
                stored_count += 1
        ...
        for call in chain.calls:
            try:
                # Calculate moneyness for validation
                moneyness = (float(call.strike_price) - float(chain.spot_price)) / float(chain.spot_price)
                ...
                self.conn.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, [
                    'U', expiration.date(), float(call.strike_price), 'CALL',
