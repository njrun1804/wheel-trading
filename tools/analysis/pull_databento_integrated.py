...
async def pull_wheel_data_integrated():
    ...
    storage = Storage(StorageConfig(
        enable_gcs_backup=bool(os.getenv("GCP_PROJECT_ID"))
    ))
    ...
    try:
        ...
        if chain_data:
            print(f"   ✅ Retrieved {len(chain_data.get('calls', []))} calls, {len(chain_data.get('puts', []))} puts")
