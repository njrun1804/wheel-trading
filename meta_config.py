...
class MetaConfig:
    ...
    def _load_from_environment(self):
        ...
        if os.getenv('META_RECENT_ACTIVITY_WINDOW'):
            self.timing.recent_activity_window_seconds = int(os.getenv('META_RECENT_ACTIVITY_WINDOW'))
            
        if os.getenv('META_RAPID_DEV_THRESHOLD'):
            self.timing.rapid_development_threshold_seconds = int(os.getenv('META_RAPID_DEV_THRESHOLD'))
            
        if os.getenv('META_MIN_OBSERVATIONS'):
            self.timing.minimum_observations_for_evolution = int(os.getenv('META_MIN_OBSERVATIONS'))
            
        if os.getenv('META_MIN_ACTIVITY'):
            self.timing.minimum_recent_activity_count = int(os.getenv('META_MIN_ACTIVITY'))
...
if __name__ == "__main__":
    ...
    print(f"  Rapid Development Threshold: {config.timing.rapid_development_threshold_seconds}s")
    ...
    print(f"  Evolution Threshold: {config.evolution.evolution_readiness_threshold:.1%}")
