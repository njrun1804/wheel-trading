...
class OptimizedResultMerger:
    ...
    def merge_results(
        ...
        try:
            ...
            location_map: dict[ResultKey, list[tuple[str, SearchResult]]] = defaultdict(
                list
            )
    ...
    async def _process_modality_batch(
        self, modality: str, results: list[SearchResult]
    ...
    def _fast_merge_location(
        self, location: ResultKey, modality_results: list[tuple[str, SearchResult]]
    ) -> FastMergedResult:
    ...
    def _fast_merge(
        self, existing: FastMergedResult, new: FastMergedResult
    ) -> FastMergedResult:
    ...
    def deduplicate_streaming(
        self, results: Iterator[SearchResult]
    ...
    def get_diversity_subset_fast(
        ...
        for result in results:
            ...
            if (
                result_hash not in selected_set
                and file_counts[result.file_path] < max_per_file
            ):
...
if __name__ == "__main__":
    ...
    async def test_streaming():
        ...
        async for _result in merger.merge_results_streaming(results_by_modality):
            count += 1

        elapsed = (time.time() - start) * 1000
        print(f"   Streamed {count} results in {elapsed:.1f}ms")
        print("   First result latency: <1ms (streaming)")
