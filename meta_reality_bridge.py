...
class RealCodebaseHandler(FileSystemEventHandler):
    ...
    def _should_observe(self, file_path: str) -> bool:
        ...
        return bool(path.suffix == ".py" and path.parent.name in ["wheel-trading", "."])
    ...
    def _debounced_handle(self, file_path: str, change_type: str):
        ...
        if (
            ...
            and now - self.last_events[key]
            < self.bridge.config.timing.file_change_debounce_seconds
        ):
            return
        ...
        # Async handle
        asyncio.create_task(
            self.bridge.handle_real_code_change(Path(file_path), change_type)
        )
...
class MetaRealityBridge:
    ...
    def __init__(self):
        ...
        self.p_core_executor = ThreadPoolExecutor(
            max_workers=8, thread_name_prefix="M4_P"
        )
        self.e_core_executor = ThreadPoolExecutor(
            max_workers=4, thread_name_prefix="M4_E"
        )
        ...
        print(
            f"⚡ M4 optimization: {self.p_core_executor._max_workers} P-cores, {self.e_core_executor._max_workers} E-cores"
        )
    ...
    def start_reality_observation(self):
        ...
        if trading_src.exists():
            self.reality_observer.schedule(
                self.reality_handler, path=str(trading_src), recursive=True
            )
            print(f"👁️ Watching real codebase: {trading_src}")

        # Watch root directory for config/script changes
        self.reality_observer.schedule(self.reality_handler, path=".", recursive=False)
        ...
        self.meta_prime.observe(
            "reality_bridge_activated",
            {
                "timestamp": time.time(),
                "watching_paths": ["src/unity_wheel", "."],
                "m4_cores_allocated": {"p_cores": 8, "e_cores": 4},
            },
        )
    ...
    async def handle_real_code_change(self, file_path: Path, change_type: str):
        ...
        try:
            ...
            analysis = analysis_future.result(
                timeout=self.config.timing.reality_bridge_timeout_seconds
            )
            learning_future.result(
                timeout=self.config.timing.reality_bridge_learning_timeout_seconds
            )
            ...
            print(
                f"🔄 Real change processed: {file_path.name} ({processing_time:.1f}ms)"
            )
    ...
    def _analyze_code_change(self, file_path: Path, change_type: str) -> dict[str, Any]:
        ...
        if file_path.exists() and file_path.suffix == ".py":
            try:
                ...
                if any(
                    trading_keyword in content.lower()
                    for trading_keyword in [
                        "wheel",
                        "option",
                        "strike",
                        "premium",
    ...
    def _learn_from_change(self, file_path: Path, change_type: str) -> dict[str, Any]:
        ...
        self.claude_patterns["editing_sequences"].append(
            {"file": file_key, "change_type": change_type, "timestamp": time.time()}
        )
        ...
        if len(self.claude_patterns["editing_sequences"]) > limit:
            self.claude_patterns["editing_sequences"] = self.claude_patterns[
                "editing_sequences"
            ][-limit:]
    ...
    def _is_significant_event(self, event: RealityEvent) -> bool:
        ...
        if (
            event.details.get("line_count", 0)
            > self.config.evolution.major_file_change_threshold
        ):
            return True

        # Trading strategy files are always significant
        return "strategy" in event.target_path.lower()
