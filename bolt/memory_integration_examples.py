...
class MemoryAwareDuckDBConnection:
    ...
    def load_dataframe(self, df, table_name: str):
        ...
        estimated_mb = df.memory_usage(deep=True).sum() / (1024 * 1024) * 2  # 2x for overhead
...
class MemoryAwareJarvisIndex:
    ...
    def search(self, query: str, k: int = 10):
        ...
        with memory_manager.allocate_context("jarvis", 10, f"Search in {self.index_name}"):
            # Perform search...
...
class MemoryAwareMetaOperation:
    ...
    def allocate_for_generation(template_size_kb: int, output_count: int):
        ...
        with memory_manager.allocate_context(
            "meta_system",
            estimated_mb,
            f"Generating {output_count} files",
            priority=7
        ):
            yield
...
def demo_memory_management():
    ...
    for component, stats in report['components'].items():
        if stats['allocated_mb'] > 0:
            print(f"   {component}: {stats['allocated_mb']:.1f}MB / {stats['max_mb']:.1f}MB ({stats['usage_percent']:.1f}%)")
