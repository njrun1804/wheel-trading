...
# Global cache for memory manager
cache: dict[str, Any] = {}


# Configuration is now loaded from Bolt config
def get_memory_config():
    """Get memory configuration from Bolt config."""
    try:
        from bolt.core.config import get_default_config

        config = get_default_config()
        return {
            "total_memory_gb": config.hardware.total_memory_gb,
            "max_allocation_gb": config.hardware.max_allocation_gb,
            "memory_pressure_threshold": config.hardware.memory_pressure_threshold,
            "critical_threshold": config.hardware.critical_threshold,
            "component_budgets": {
                "duckdb": config.memory.duckdb_budget_ratio,
                "jarvis": config.memory.jarvis_budget_ratio,
                "einstein": config.memory.einstein_budget_ratio,
                "meta_system": config.memory.meta_system_budget_ratio,
                "cache": config.memory.cache_budget_ratio,
                "other": config.memory.other_budget_ratio,
                "test_component": 0.01,  # 1% for testing
                "integration_test": 0.01,  # 1% for integration testing
            },
        }
    except Exception as e:
        logger.warning(f"Failed to load memory config, using defaults: {e}")
        # Fallback to original hardcoded values
        return {
            "total_memory_gb": 24,
            "max_allocation_gb": 18,
...
TOTAL_SYSTEM_MEMORY_GB = _memory_config["total_memory_gb"]
MAX_ALLOCATION_GB = _memory_config["max_allocation_gb"]
MEMORY_PRESSURE_THRESHOLD = _memory_config["memory_pressure_threshold"]
CRITICAL_THRESHOLD = _memory_config["critical_threshold"]
COMPONENT_BUDGETS = _memory_config["component_budgets"]
...
class MemoryPool:
    ...
    def allocate(
        ...
        with self.lock:
            ...
            if self.allocated_bytes + size_bytes > self.max_size_bytes:
                ...
                if not evicted:
                    logger.warning(
                        f"{self.name}: Cannot allocate {size_bytes} bytes, pool full"
                    )
                    return None
    ...
    def _evict_for_space(self, needed_bytes: int, min_priority: int) -> bool:
        ...
        evictable = sorted(
            [
                ...
                if a.can_evict and a.priority < min_priority
            ],
        ...
        for alloc in evictable:
            ...
            evicted_ids.append(
                [k for k, v in self.allocations.items() if v == alloc][0]
            )
        ...
        logger.info(
            f"{self.name}: Evicted {len(evicted_ids)} allocations to free {evicted_bytes} bytes"
        )
        return True
    ...
    def get_usage_percent(self) -> float:
        ...
        with self.lock:
            return (
                (self.allocated_bytes / self.max_size_bytes) * 100
...
class BoltMemoryManager:
    ...
    def allocate(
        ...
        if system_usage > CRITICAL_THRESHOLD:
            ...
            if system_usage > CRITICAL_THRESHOLD:  # Still critical after measures
                logger.error(
                    f"System memory critical ({system_usage:.1%}), allocation denied"
                )
                return None
    ...
    def allocate_context_async(
        self, component: str, size_mb: float, description: str, **kwargs
    ):
        """Async context manager for automatic allocation/deallocation"""

        class AsyncContextManager:
            def __init__(self, manager, component, size_mb, description, kwargs):
            ...
            async def __aenter__(self):
                self.alloc_id = self.manager.allocate(
                    self.component, self.size_mb, self.description, **self.kwargs
                )
                if not self.alloc_id:
                    raise MemoryError(
                        f"Could not allocate {self.size_mb}MB for {self.component}"
                    )
                return self.alloc_id

            async def __aexit__(self, exc_type, exc_val, exc_tb):
                if self.alloc_id:
                    self.manager.deallocate(self.alloc_id)

        return AsyncContextManager(self, component, size_mb, description, kwargs)
        ...
        while self.running:
            try:
                ...
                if system_usage > MEMORY_PRESSURE_THRESHOLD:
                    logger.warning(
                        f"Memory pressure: System {system_usage:.1%}, Allocated {total_allocated:.1f}MB"
                    )
                ...
                for pool in self.pools.values():
                    if (
                        pool.get_usage_percent() > 80
                        and current_time - pool.stats.last_gc > 60
                    ):  # GC every minute max
                        gc.collect()
    ...
    def _evict_low_priority_allocations(self):
        ...
        for pool in self.pools.values():
            with pool.lock:
                ...
                evictable = [
                    (aid, alloc)
                    for aid, alloc in pool.allocations.items()
        ...
        logger.info(
            f"Evicted {total_evicted / 1024 / 1024:.1f}MB of low priority allocations"
        )
    ...
    def start_monitoring(self):
        ...
        if not self.running:
            ...
            self.monitor_thread = threading.Thread(
                target=self._monitor_memory, daemon=True, name="BoltMemoryMonitor"
            )
    ...
    def enforce_limits(self, strict: bool = True):
        ...
        for component, pool in self.pools.items():
            if pool.get_usage_percent() > 100:
                ...
                logger.warning(
                    f"{component} over limit by {excess / 1024 / 1024:.1f}MB, forcing eviction"
                )
                ...
                logger.info(
                    f"{component}: Forced eviction of {evicted / 1024 / 1024:.1f}MB"
                )


# Global instance
_memory_manager: BoltMemoryManager | None = None
...
class DuckDBMemoryGuard:
    ...
    def allocate_for_query(self, estimated_mb: float, query_description: str):
        ...
        with self.memory_manager.allocate_context(
            "duckdb", estimated_mb, f"Query: {query_description}", priority=6
        ):
            yield
...
class EinsteinMemoryGuard:
    ...
    def allocate_for_embeddings(self, batch_size: int, embedding_dim: int):
        ...
        estimated_mb = (batch_size * embedding_dim * 4) / (
            1024 * 1024
        )  # 4 bytes per float
...
if __name__ == "__main__":
    ...
    alloc_id = manager.allocate(
        "jarvis", 500, "Search index", can_evict=False, priority=9
    )
    if alloc_id:
