...
class MetaSystemMonitor:
    ...
    def record_metric(self, metric_name: str, value: float, 
                     component: str = "meta_system", **tags):
    ...
    def health_check(self, component: str) -> HealthCheck:
        ...
        try:
            ...
            elif any(v > 75 for k, v in metrics.items() if k.endswith('_usage_percent')):
                status = "warning"
    ...
    def _check_meta_prime_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            cursor = db.execute("""
                ...
                WHERE timestamp > ?
            """, (time.time() - self.config.timing.recent_activity_window_seconds * 12,))  # Recent activity window
            ...
            return {
                ...
                "database_usage_percent": min(100, (db_size_mb / 100) * 100)  # Assume 100MB limit
            }
    ...
    def _check_database_health(self) -> Dict[str, Any]:
        ...
        try:
            from meta_config import get_meta_config
            config = get_meta_config()
            db_files = [config.database.evolution_db, config.database.monitoring_db, config.database.reality_db]
            total_size = 0
            file_count = 0
            ...
            return {
                ...
                "database_usage_percent": min(100, (total_size_mb / self.config.system.max_db_size_mb * 5) * 100)  # Based on config limit
            }
    ...
    def _check_general_health(self) -> Dict[str, Any]:
        ...
        return {
            "uptime_hours": uptime / self.config.timing.recent_activity_window_seconds * 12,
    ...
    def _send_alert(self, alert_type: str, severity: str, 
    ...
    def get_monitoring_dashboard(self) -> Dict[str, Any]:
        ...
        metrics_summary = {row[0]: {"avg": row[1], "max": row[2]} 
                          for row in cursor.fetchall()}
        ...
        active_alerts = [{"type": row[0], "severity": row[1], "message": row[2], 
                         "component": row[3], "timestamp": row[4]}
                        for row in cursor.fetchall()]
