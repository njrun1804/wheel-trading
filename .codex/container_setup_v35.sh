...
echo "🚀 UNITY WHEEL CONTAINER SETUP v36"
echo "=================================="
echo ""
...
int64 = int
int32 = int
int_ = int  # Alias for default integer type
uint = int
uint8 = int
uint16 = int
uint32 = int
...
class datetime64:
    def __init__(self, value=None):
        if value is None:
            self._value = _datetime.datetime.now()
        elif isinstance(value, str):
            self._value = _datetime.datetime.fromisoformat(value.replace('T', ' ').replace('Z', ''))
        else:
            self._value = value
    def __str__(self):
        return self._value.isoformat()

timedelta64 = _datetime.timedelta
...
warnings.warn("Using minimal numpy implementation - calculations will use pure Python fallbacks", ImportWarning, stacklevel=2)
EOF
...
class ensemble:
    class IsolationForest:
        def __init__(self, **kwargs):
            warnings.warn("Using minimal sklearn implementation - anomaly detection disabled", ImportWarning, stacklevel=2)
...
def get(url, **kwargs):
    ...
    warnings.warn("Using minimal requests implementation - HTTP calls disabled", ImportWarning, stacklevel=2)
...
# Show warning
warnings.warn("Using minimal pydantic_settings implementation - settings loading limited", ImportWarning, stacklevel=2)
...
class cloud:
    ...
    class secretmanager:
        ...
        class SecretManagerServiceClient:
            def __init__(self, **kwargs):
                warnings.warn("Using minimal Google Cloud implementation - secrets disabled", ImportWarning, stacklevel=2)
    ...
    class storage:
        ...
        class Client:
            def __init__(self, **kwargs):
                warnings.warn("Using minimal Google Cloud implementation - storage disabled", ImportWarning, stacklevel=2)
...
def given(*args, **kwargs):
    ...
    def decorator(test_func):
        def wrapper(*test_args, **test_kwargs):
            ...
            warnings.warn("Using minimal hypothesis implementation - running single test with random data", ImportWarning, stacklevel=2)
...
warnings.warn("Using minimal hypothesis implementation - property-based testing limited", ImportWarning, stacklevel=2)
EOF
...
if [ "$NUMPY_INSTALLED" = true ] && [ "$SKLEARN_INSTALLED" = true ]; then
    ...
    # Still need stubs for missing packages (like Google Cloud)
...
# Generated by container_setup_v35.sh on $(date)
...
# Always add stubs for missing packages (like Google Cloud)
...
if [ -f .codex/.container_env ]; then
    ...
    echo "   Run: ./.codex/container_setup_v35.sh"
fi
EOF
