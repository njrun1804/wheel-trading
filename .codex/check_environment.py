...
def main():
    ...
    print(
        f"📊 Optional dependencies: {colored(f'{optional_count}/5 available', GREEN if optional_count >= 3 else YELLOW)}"
    )
    ...
    print(
        f"🧪 Functionality tests: {colored(f'{tests_passed}/{len(tests)} passed', GREEN if tests_passed == len(tests) else YELLOW)}"
    )
    ...
    if not all_critical_ok:
        print(
            f"🚨 {colored('CRITICAL:', RED)} Missing essential Python modules. Python installation may be incomplete."
        )
