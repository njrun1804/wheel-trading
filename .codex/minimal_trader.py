...
class PureMathEngine:
    ...
    def black_scholes_call(
        ...
        try:
            ...
            call_price = S * PureMathEngine.normal_cdf(d1) - K * math.exp(
                -r * T
            ) * PureMathEngine.normal_cdf(d2)
    ...
    def find_optimal_put_strike(
        ...
        for strike in strikes:
            ...
            if total_score > best_score:
                ...
                best_strike = StrikeData(strike=strike, delta=delta, price=put_price.value)

        return best_strike

    def calculate_position_size(
        self, portfolio_value: float, stock_price: float, max_position_pct: float = 0.20
    ) -> int:
...
class MinimalWheelAdvisor:
    ...
    def generate_recommendation(
        ...
        contracts = self.strategy.calculate_position_size(
            portfolio_value=portfolio_value, stock_price=underlying_price
        )

        # Calculate expected return
        premium_per_contract = optimal_strike.price * 100  # 100 shares per contract
        total_premium = premium_per_contract * contracts
        expected_return_pct = (total_premium / portfolio_value) * (
            365.0 / days_to_expiry
        )  # Annualized
