...
class QueryRouter:
    ...
    def analyze_query(self, query: str) -> QueryPlan:
        ...
        try:
            if not query or not isinstance(query, str):
                logger.warning(f"Invalid query input: {type(query).__name__} with value {query!r}")
            ...
            logger.debug(f"Query analysis completed: {query_type.value} with {len(modalities)} modalities")
            return plan
        ...
        try:
            ...
            for query_type, patterns in self.patterns.items():
                for pattern in patterns:
                    try:
                        ...
                        logger.warning(f"Invalid regex pattern '{pattern}' for query type {query_type}: {e}",
                                     extra={
            ...
            logger.error(f"Failed to classify query: {e}", exc_info=True,
                        extra={
                            ...
                            'available_query_types': [qt.value for qt in self.patterns.keys()]
                        })
    ...
    def _select_modalities(self, query_type: QueryType, query: str) -> list[str]:
        ...
        try:
            ...
            valid_modalities = [m for m in base_modalities if m in self.modality_performance]
            if not valid_modalities:
                logger.warning(f"No valid modalities found for query type {query_type.value}, using text fallback")
                valid_modalities = ['text']
            
            return valid_modalities
            
        except Exception as e:
            logger.error(f"Failed to select modalities: {e}", exc_info=True,
                        extra={
        ...
        try:
            ...
            if plan.estimated_time_ms > 20.0:
                ...
                if fast_modalities:
                    ...
                    plan.estimated_time_ms = sum(self.modality_performance.get(m, 5.0) 
                                                for m in fast_modalities)
                    plan.reasoning += " Optimized for low latency."
                    
                    logger.debug(f"Latency optimization: {original_time:.1f}ms -> {plan.estimated_time_ms:.1f}ms")
                else:
                    logger.warning(f"No fast modalities available for latency optimization, keeping original plan")
            
            return plan
            
        except Exception as e:
            logger.error(f"Failed to optimize for latency: {e}", exc_info=True,
                        extra={
    ...
    def optimize_for_recall(self, plan: QueryPlan) -> QueryPlan:
        ...
        try:
            ...
            available_modalities = [m for m in all_modalities if m in self.modality_performance]
            ...
            if available_modalities:
                ...
                logger.debug(f"Recall optimization: {len(original_modalities)} -> {len(available_modalities)} modalities")
            else:
                logger.warning("No modalities available for recall optimization, keeping original plan")
            
            return plan
            
        except Exception as e:
            logger.error(f"Failed to optimize for recall: {e}", exc_info=True,
                        extra={
                            ...
                            'query': plan.query[:100],
                            'query_type': plan.query_type.value,
                            'available_performance_keys': list(self.modality_performance.keys())
                        })
            return plan  # Return original plan on error
