...
from .query_router import QueryPlan, QueryType
...
class QueryPlanCache:
    ...
    def get(self, query: str) -> Optional[CachedQueryPlan]:
    ...
    def update_performance(self, query: str, latency_ms: float, result_count: int):
        ...
        if query in self.cache:
            ...
            cached.avg_latency_ms = (1 - alpha) * cached.avg_latency_ms + alpha * latency_ms
            cached.avg_result_count = (1 - alpha) * cached.avg_result_count + alpha * result_count
...
class CachedQueryRouter(AdaptiveQueryRouter):
    ...
    async def analyze_query_cached(self, query: str, context: dict[str, Any] = None) -> QueryPlan:
        ...
        logger.debug(f"Computed new plan for query: {query[:50]}... ({routing_time:.1f}ms)")
    ...
    def _update_routing_stats(self, routing_time_ms: float):
        ...
        # Update moving average
        alpha = 0.1
        self.router_stats['avg_routing_time_ms'] = (
            (1 - alpha) * self.router_stats['avg_routing_time_ms'] + 
    ...
    def persist_cache(self, cache_path: Path = None):
        ...
        try:
            cache_data = {
                'plan_cache': {
                    query: cached for query, cached in self.plan_cache.cache.items()
...
async def benchmark_cached_router():
    ...
    for i, query in enumerate(test_queries[:8]):
        start = time.time()
        plan = await router.analyze_query_cached(query)
    ...
    # Test loaded cache
    start = time.time()
    plan = await new_router.analyze_query_cached(test_queries[0])
    elapsed = (time.time() - start) * 1000
