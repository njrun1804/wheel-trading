...
class SearchResultCache:
    ...
    def get(self, query: str, modality: str) -> Optional[list[SearchResult]]:
    ...
    def put(self, query: str, modality: str, results: list[SearchResult]):
    ...
    def _promote_to_l1(self, key: str, results: list[SearchResult]):
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            'l1_hit_rate': round(self.l1_hits / total_requests * 100, 1) if total_requests > 0 else 0,
            'l2_hit_rate': round(self.l2_hits / total_requests * 100, 1) if total_requests > 0 else 0,
            'total_hit_rate': round((self.l1_hits + self.l2_hits) / total_requests * 100, 1) if total_requests > 0 else 0
        }
...
class OptimizedUnifiedSearch:
    ...
    def __init__(self, index_hub):
        ...
        self.search_pool = asyncio.Semaphore(self.config.performance.max_search_concurrency)
    ...
    async def search(
        ...
        try:
            ...
            search_results = await self._parallel_modality_search(query, modalities, metrics)
    ...
    async def _parallel_modality_search(
        ...
        if search_tasks:
            ...
            for (modality, _), results in zip(search_tasks, modality_results):
    ...
    async def _record_search_metrics(
        ...
        satisfaction = min(1.0, result_count / 10.0) * (1.0 - min(1.0, latency_ms / 100.0))
...
async def benchmark_unified_search():
    ...
    class MockIndexHub:
        async def _text_search(self, query: str) -> list[SearchResult]:
            await asyncio.sleep(0.005)  # Simulate 5ms search
            return [
                SearchResult(
        ...
        async def _semantic_search(self, query: str) -> list[SearchResult]:
            await asyncio.sleep(0.015)  # Simulate 15ms search
            return [
                SearchResult(
        ...
        async def _structural_search(self, query: str) -> list[SearchResult]:
            await asyncio.sleep(0.008)  # Simulate 8ms search
            return [
                SearchResult(
        ...
        async def _analytical_search(self, query: str) -> list[SearchResult]:
            await asyncio.sleep(0.010)  # Simulate 10ms search
            return [
                SearchResult(
    ...
    mock_hub = MockIndexHub()
    search_system = OptimizedUnifiedSearch(mock_hub)
    ...
    for i, (results, metrics) in enumerate(burst_results):
        print(f"    Query {i+1}: {metrics.total_time_ms:.1f}ms ({len(results)} results)")
