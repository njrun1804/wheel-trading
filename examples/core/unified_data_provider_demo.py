...
async def demonstrate_basic_data_access():
    ...
    try:
        market_response = await provider.get_market_data(
            symbol="U", start_time=datetime.now() - timedelta(days=30), end_time=datetime.now()
        )
    ...
    try:
        options_response = await provider.get_options_chain(
            symbol="U", expiration=datetime.now() + timedelta(days=45)
        )
        ...
        if isinstance(options_response.data, list) and options_response.data:
            ...
            print(
                f"  Example option: ${first_option.get('strike', 'N/A')} strike, "
            ...
            for option in options_data.data:
                ...
                if strike and bid and strike < 40:  # Focus on puts below $40
                    ...
                    put_candidates.append(
                        {"strike": strike, "bid": bid, "premium_yield": premium_yield}
                    )
...
async def demonstrate_custom_requests():
    ...
    try:
        ...
        if isinstance(response.data, list):
            filtered_count = len([opt for opt in response.data if opt.get("volume", 0) >= 100])
...
async def demonstrate_performance_monitoring():
    ...
    for i in range(3):
        ...
        market_data = await provider.get_market_data(
            symbol="U", start_time=datetime.now() - timedelta(days=7)
        )
...
async def demonstrate_error_handling():
    ...
    try:
        request = DataRequest(
            source_type=DataSourceType.FUNDAMENTAL_DATA, symbol="U"  # Not widely supported
        )
        response = await provider.get_data(request)
    ...
    await provider.invalidate_cache(source_type=DataSourceType.MARKET_DATA, symbol="U")
