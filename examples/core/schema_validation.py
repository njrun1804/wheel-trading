...
async def demonstrate_schema_mapping():
    ...
    print(f"   → Python datetime: {datetime.fromtimestamp(DATABENTO_API_RESPONSE['ts_event'] / 1e9)}")
    print(f"2. Price in 1e-9 dollars: {DATABENTO_API_RESPONSE['levels'][0]['bid_px']}")
    print(f"   → Decimal dollars: ${Decimal(DATABENTO_API_RESPONSE['levels'][0]['bid_px']) / Decimal('1e9')}")
