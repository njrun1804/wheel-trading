...
class MetaActiveImprovementEngine:
    ...
    ) -> tuple[str, list[ActiveImprovement]]:
    ...
    def suggest_improvements(
        ...
        if not self._has_proper_error_handling(code):
            suggestions.append(
                "Add try-except blocks for better error handling (learned from Claude)"
            )

        if not self._has_documentation(code):
            suggestions.append(
                "Add docstrings and comments (Claude always documents code)"
            )
        ...
        if self._has_magic_numbers(code):
            suggestions.append(
                "Replace magic numbers with named constants (Claude best practice)"
            )
    ...
    def _detect_improvement_opportunities(
        self, code: str, context: dict[str, Any]
    ) -> list[ActiveImprovement]:
    ...
    def _apply_improvements(
        self, code: str, improvements: list[ActiveImprovement]
    ) -> str:
        ...
        for improvement in improvements:
            ...
            if self._validate_improvement_safety(improvement):
                ...
                print(
                    f"  ⚠️ Skipped unsafe improvement: {improvement.improvement_type}"
                )
    ...
    def _needs_validation(self, code: str) -> bool:
        ...
        has_user_input = any(
            pattern in code for pattern in ["input(", "sys.argv", "request."]
        )
        has_validation = any(
            word in code.lower() for word in ["validate", "check", "assert", "if not"]
        )
            ...
            for _i, line in enumerate(lines):
                ...
                if line.strip().startswith("def ") and ":" in line:
                    ...
                    improved_lines.append(
                        " " * (indent + 4) + f'"""Execute {func_name} operation."""'
                    )
        ...
        if "input(" in code and "if not" not in code:
            ...
            for line in lines:
                ...
                if "input(" in line and "=" in line:
                    ...
                    improved_lines.append(
                        " " * (indent + 4) + f'print("Error: {var_name} is required")'
                    )
    ...
    def _calculate_improvement_confidence(self, improvement_type: str) -> float:
        ...
        priority_boost = {"high": 0.1, "medium": 0.05, "low": 0.0}
    ...
    def get_improvement_stats(self) -> dict[str, Any]:
        ...
        stats = {
            ...
            "avg_confidence": sum(imp.confidence for imp in self.improvements_applied)
