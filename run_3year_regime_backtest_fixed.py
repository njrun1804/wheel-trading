...
async def run_3year_regime_aware_backtest():
    ...
    for regime_id, regime_info in sorted(regime_detector.regime_info.items()):
        print(
            f"  {regime_info.regime_id:>2} | {regime_info.name:<12} | {regime_info.volatility:>9.1%} | "
    ...
    print(
        f"  Parameters: Delta={params.target_delta}, DTE={params.target_dte}, Max Position={params.max_position_size:.0%}"
    )
    ...
    print(
        f"  Assignments: {full_results.assignments} ({full_results.assignments/max(1,full_results.total_trades)*100:.1f}%)"
    )
    ...
    for regime, params_set in regime_params.items():
        ...
        if recent_period:
            try:
                ...
                print(
                    f"  {regime:<7} | {period_str:<25} | {result.total_return:>5.1%} | "
    ...
    for year in [2022, 2023, 2024, 2025]:
        ...
        year_end = (
            datetime(year, 12, 31) if year < 2025 else datetime.strptime(str(end_date), "%Y-%m-%d")
        )
        ...
        try:
            ...
            year_result = await backtester.backtest_strategy(
                ...
                parameters=WheelParameters(
                    target_delta=0.30, target_dte=45, max_position_size=0.20
                ),
            )
    ...
    if yearly_results:
        ...
        for r in yearly_results:
            print(
                f"  {r['year']} | {r['return']:>6.1%} | {r['sharpe']:>6.2f} | {r['trades']:>6} | "
    ...
    print(
        f"- Current market regime: {current_regime.name if 'current_regime' in locals() else 'Unknown'}"
    )
