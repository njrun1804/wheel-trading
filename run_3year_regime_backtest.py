...
async def run_3year_regime_aware_backtest():
        ...
        for period_start, period_end in regime_periods[:5]:  # Test up to 5 periods
            try:
                ...
                if regime_id == 0:  # Low vol regime
                    params = WheelParameters(target_delta=0.35, target_dte=60, max_position_size=0.25)
                elif regime_id == 3:  # Crisis regime
                    params = WheelParameters(target_delta=0.15, target_dte=30, max_position_size=0.10)
                else:  # Normal/High vol
                    params = WheelParameters(target_delta=0.25, target_dte=45, max_position_size=0.20)
    ...
    for year, days, ret, avg_vol, min_vol, max_vol in yearly_returns:
        print(f"  {int(year)} | {days:>4} | {ret:>10.1%} | {avg_vol:>6.1%} | {min_vol:>5.1%} - {max_vol:>5.1%}")
    ...
    if regime_results:
        ...
        for regime_id, stats in sorted(regime_results.items()):
            print(f"  {regime_id:>6} | {stats['periods']:>7} | {stats['total_trades']:>6} | "
