...
class CompressedLRUCache(Generic[T]):
    ...
    def __init__(self, max_size: int, compress_threshold: int = 1024) -> None:
        ...
        with self._lock:
            ...
            if compressed:
                try:
                    ...
                    logger.warning(
                        f"Decompression failed for key {key}: {e}",
                        extra={
                            ...
                            "cache_hit_rate": round(
                                self.hits / (self.hits + self.misses) * 100, 1
                            )
        ...
        with self._lock:
            ...
            if self._should_compress(value):
                try:
                    ...
                    logger.warning(
                        f"Compression failed for key {key}: {e}",
                        extra={
    ...
    def _should_compress(self, value: T) -> bool:
        ...
        try:
            ...
            logger.debug(
                f"Failed to serialize value for compression check: {e}",
                extra={
...
class MemoryMappedIndex:
        ...
        try:
            ...
            if file_size > 8192:
                ...
                try:
                    ...
                    logger.debug(
                        f"Failed to decompress/deserialize memory-mapped index from {self.file_path}: {e}",
                        extra={
            ...
            logger.error(
                f"Failed to load memory-mapped index {self.file_path}: {e}",
                exc_info=True,
                extra={
                    ...
                    "file_readable": self.file_path.is_file()
    ...
    def get(self, key: str) -> bytes | None:
        ...
        try:
            ...
            logger.error(
                f"Failed to read from memory-mapped index: {e}",
                exc_info=True,
                extra={
...
class MemoryPool:
    ...
    def return_buffer(self, buffer: bytes):
        ...
        Args:
            buffer: Buffer to return to the pool. Pool is organized by size
                   to enable efficient reuse of similarly-sized allocations.
        """
        size = len(buffer)
...
class MemoryOptimizer:
    """Main memory optimization manager - M4 Pro Optimized."""

    def __init__(self, target_usage_mb: float = 800):  # Reduced from 2048MB to 800MB
        ...
        # Reduced cache sizes for M4 Pro optimization
        reduced_search_cache = min(
            50, self.config.cache.search_cache_size // 4
        )  # 75% reduction
        reduced_file_cache = min(
            100, self.config.cache.file_cache_size // 3
        )  # 66% reduction

        # Memory management components
        self.search_cache = CompressedLRUCache[list[dict[str, Any]]](
            max_size=reduced_search_cache, compress_threshold=256
        )
        self.file_cache = CompressedLRUCache[str](
            max_size=reduced_file_cache, compress_threshold=512
        )
        self.weak_cache = WeakValueCache()
        self.memory_pool = MemoryPool()

        # Memory-mapped indexes with lazy loading
        self.mmap_indexes: dict[str, MemoryMappedIndex] = {}
        self._index_usage_tracker: dict[str, float] = {}  # Track last access time

        # More aggressive GC optimization for M4 Pro
        self._gc_threshold = (200, 5, 5)  # Much more aggressive collection
        self._last_gc_time = time.time()
        self._gc_interval = min(
            15, self.config.monitoring.gc_interval_s
        )  # More frequent GC

        # Memory monitoring with smaller history
        self._memory_history: list[tuple[float, float]] = []  # (timestamp, usage_mb)
        self._max_history_size = 50  # Reduced from default
        self._monitoring_active = False

        # M4 Pro specific optimizations
        self._enable_memory_pressure_response = True
        self._last_pressure_check = time.time()
        self._pressure_check_interval = 5  # Check every 5 seconds
    ...
    async def optimize_memory_usage(self):
        """Optimize current memory usage - M4 Pro Enhanced."""
        logger.info("🧹 Optimizing Einstein memory usage...")

        # Get current memory usage
        current_usage = self.get_memory_usage_mb()

        # M4 Pro: Always optimize if over 600MB (75% of 800MB target)
        if current_usage > self.target_usage_mb * 0.75:
            # Apply aggressive optimization
            freed_mb = self._aggressive_cleanup()
            logger.info(f"Freed {freed_mb:.1f}MB through aggressive cleanup")

        # M4 Pro: Clean up unused memory-mapped indexes
        self._cleanup_unused_indexes()

        # Run garbage collection
        self._run_gc()

        # Update memory history (with reduced size)
        self._memory_history.append((time.time(), current_usage))

        # Keep only last N measurements (reduced for M4 Pro)
        ...
        logger.info(
            f"Einstein memory optimization complete: {current_usage:.1f}MB -> {new_usage:.1f}MB (freed {freed_total:.1f}MB)"
        )
    ...
    def get_memory_profile(self) -> MemoryProfile:
        ...
        try:
            ...
            logger.debug(
                f"Failed to calculate cache memory usage: {e}",
                extra={
        ...
        return MemoryProfile(
            ...
            cache_mb=cache_mb,
            indexes_mb=sum(
                mmap.file_path.stat().st_size / 1024 / 1024
    ...
    def register_memory_mapped_index(self, name: str, file_path: Path):
        """Register a memory-mapped index for efficient large data access.

        Args:
            name: Unique name for the index
            file_path: Path to the memory-mapped index file

        Note:
            Memory-mapped indexes allow accessing large data structures
            without loading them entirely into RAM, optimized for M4 Pro.
        """
        mmap_index = MemoryMappedIndex(file_path)
        if mmap_index.load():
            self.mmap_indexes[name] = mmap_index
            self._index_usage_tracker[name] = time.time()
            logger.debug(f"Registered memory-mapped index: {name}")

    def _cleanup_unused_indexes(self):
        """Clean up unused memory-mapped indexes (M4 Pro optimization)."""
        current_time = time.time()
        unused_threshold = 300  # 5 minutes

        unused_indexes = []
        for name, last_used in self._index_usage_tracker.items():
            if current_time - last_used > unused_threshold:
                unused_indexes.append(name)

        for name in unused_indexes:
            if name in self.mmap_indexes:
                self.mmap_indexes[name].close()
                del self.mmap_indexes[name]
                del self._index_usage_tracker[name]
                logger.debug(f"Closed unused memory-mapped index: {name}")

    def access_memory_mapped_index(self, name: str, key: str) -> bytes | None:
        """Access memory-mapped index with usage tracking."""
        if name in self.mmap_indexes:
            self._index_usage_tracker[name] = time.time()
            return self.mmap_indexes[name].get(key)
        return None
    ...
    def get_optimization_recommendations(self) -> list[str]:
        ...
        if profile.total_mb > profile.target_usage_mb:
            recommendations.append(
                f"Memory usage {profile.total_mb:.1f}MB exceeds target {profile.target_usage_mb:.1f}MB"
            )

        if profile.cache_mb > 500:
            recommendations.append(
                f"Cache usage {profile.cache_mb:.1f}MB is high - consider reducing cache sizes"
            )

        if profile.system_available_mb < 1024:
            recommendations.append(
                f"System memory low ({profile.system_available_mb:.1f}MB available)"
            )
        ...
        if len(self._memory_history) >= 10:
            ...
            if len(recent_usage) >= 2:
                ...
                if (
                    growth_rate > self.config.cache.cache_memory_limit_mb / 50
                ):  # Growing by more than 2% of cache limit per measurement
                    recommendations.append(
                        f"Memory usage growing rapidly ({growth_rate:.1f}MB per interval)"
                    )
    ...
    async def start_memory_monitoring(self, interval_seconds: int = None):
        ...
        Args:
            interval_seconds: Monitoring check interval. Uses config default if None.
        """
        ...
        async def monitor_loop():
            while self._monitoring_active:
                try:
                    ...
                    if (
                        profile.total_mb > profile.target_usage_mb * 0.9
                    ):  # 90% of target
                        logger.warning(
                            f"High memory usage detected: {profile.total_mb:.1f}MB"
                        )
...
async def benchmark_memory_optimization():
    ...
    for i in range(1000):
        key = f"query_{i}"
        results = [
            {"content": f"result_{j}" * 100, "file": f"file_{j}.py", "line": j}
            for j in range(10)
        ]
    ...
    print(
        f"   After loading: {loaded_profile.total_mb:.1f}MB (+{loaded_profile.total_mb - initial_profile.total_mb:.1f}MB)"
    )
    ...
    memory_efficiency = (
        (initial_profile.target_usage_mb - final_profile.total_mb)
        / initial_profile.target_usage_mb
        * 100
    )
    ...
    if final_profile.total_mb <= initial_profile.target_usage_mb:
        ...
        print(
            f"   Overage: {final_profile.total_mb - initial_profile.target_usage_mb:.1f}MB"
        )
