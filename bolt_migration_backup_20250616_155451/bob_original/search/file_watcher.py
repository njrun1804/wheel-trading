...
class EinsteinFileWatcher(FileSystemEventHandler):
    ...
    def __init__(self, update_callback: Callable[[FileChangeEvent], None]):
        ...
        self.debounce_delay = (
            config.monitoring.debounce_delay_ms / 1000.0
        )  # Convert to seconds
    ...
    def _schedule_update(self, file_path: str, event_type: str):
        ...
        change_event = FileChangeEvent(
            file_path=Path(file_path), event_type=event_type, timestamp=time.time()
        )
    ...
    async def _debounced_update(self, file_path: str):
        ...
        try:
            ...
            if file_path in self.pending_updates:
                ...
                if self.update_callback:
                    if asyncio.iscoroutinefunction(self.update_callback):
                        ...
                        self.update_callback(change_event)
            ...
            logger.error(
                f"Error processing file update {file_path}: {e}",
                exc_info=True,
                extra={
...
class EinsteinRealtimeIndexer:
        ...
        for watch_path in self.watch_paths:
            if watch_path.exists():
                self.observer.schedule(
                    self.file_handler, str(watch_path), recursive=True
                )
    ...
    async def _handle_file_change(self, change_event: FileChangeEvent):
        ...
        try:
            ...
            logger.debug(
                f"Processing {change_event.event_type}: {change_event.file_path}"
            )
            ...
            if change_event.event_type in ["created", "modified"]:
                ...
                if hasattr(self.einstein_hub, "embedding_pipeline"):
                    try:
                        ...
                        logger.error(
                            f"Embedding update failed for {change_event.file_path}: {e}",
                            exc_info=True,
                            extra={
                                ...
                                "event_type": change_event.event_type,
                                "has_embedding_pipeline": hasattr(
                                    self.einstein_hub, "embedding_pipeline"
                                ),
                                "stats": self.stats,
                                "file_exists": change_event.file_path.exists(),
                                "file_size": change_event.file_path.stat().st_size
                                if change_event.file_path.exists()
                                else 0,
                            },
                        )
                ...
                if hasattr(self.einstein_hub, "dependency_graph"):
                    try:
                        ...
                        logger.error(
                            f"Dependency graph update failed for {change_event.file_path}: {e}",
                            exc_info=True,
                            extra={
                                ...
                                "file_path": str(change_event.file_path),
                                "event_type": change_event.event_type,
                                "has_dependency_graph": hasattr(
                                    self.einstein_hub, "dependency_graph"
                                ),
                                "stats": self.stats,
                                "file_exists": change_event.file_path.exists(),
            ...
            logger.error(
                f"Failed to process file change {change_event.file_path}: {e}",
                exc_info=True,
                extra={
    ...
    async def _remove_from_indexes(self, file_path: Path):
        ...
        try:
            ...
            await self.einstein_hub.duckdb.execute(
                "DELETE FROM file_analytics WHERE file_path = ?", (str(file_path),)
            )
            ...
            logger.error(
                f"Failed to remove {file_path} from indexes: {e}",
                exc_info=True,
                extra={
                    ...
                    "file_existed": file_path in str(e)
