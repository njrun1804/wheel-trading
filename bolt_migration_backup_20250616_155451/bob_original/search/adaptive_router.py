...
from einstein.query_router import QueryPlan, QueryRouter
...
class AdaptiveQueryRouter(QueryRouter):
    ...
    def __init__(self, model_path: Path = None):
        ...
        self.query_history = deque(
            maxlen=config.cache.max_cache_entries
        )  # Keep last queries
    ...
    def analyze_query_adaptive(
        ...
        adaptive_plan = QueryPlan(
            ...
            * 0.9,  # Slightly lower confidence for exploration
    ...
    def record_outcome(
        ...
        success_score = (
            0.4 * user_satisfaction + 0.3 * latency_score + 0.3 * result_score
        )
    ...
    def _calculate_reward(self, outcome: QueryOutcome) -> float:
        ...
        # Combined reward
        total_reward = (
            0.4 * satisfaction_reward + 0.4 * latency_reward + 0.2 * result_reward
        )
    ...
    def _update_stats(self, outcome: QueryOutcome):
        ...
        self.stats["average_latency"] = (
            alpha * outcome.latency_ms + (1 - alpha) * self.stats["average_latency"]
        )
    ...
    def _save_model(self):
        ...
        try:
            ...
            logger.error(
                f"Failed to save adaptive model: {e}",
                exc_info=True,
                extra={
                    ...
                    "parent_writable": os.access(self.model_path.parent, os.W_OK)
                    if self.model_path.parent.exists()
                    else False,
                    "stats": self.stats,
                    "bandit_weights": self.bandit.arm_weights.tolist()
                    if hasattr(self.bandit, "arm_weights")
                    else None,
                },
            )
    ...
    def _load_model(self):
        ...
        try:
            if self.model_path.exists():
                ...
                self.bandit.arm_weights = model_data.get(
                    "bandit_weights", self.bandit.arm_weights
                )
                self.bandit.arm_counts = model_data.get(
                    "bandit_counts", self.bandit.arm_counts
                )
                self.bandit.exploration_rate = model_data.get(
                    "exploration_rate", self.bandit.exploration_rate
                )
            ...
            logger.error(
                f"Failed to load adaptive model: {e}",
                exc_info=True,
                extra={
                    ...
                    "file_readable": os.access(self.model_path, os.R_OK)
                    if self.model_path.exists()
                    else False,
                    "bandit_stats": self.bandit.get_stats()
                    if hasattr(self.bandit, "get_stats")
                    else None,
                },
            )
