...
def disable_server(config, server_name):
    ...
    if server_name in active and server_name not in [
        "filesystem",
        "github",
        "python_analysis",
    ]:
        ...
        print("✗ Cannot disable core server or server not found")
        return False
...
def main():
    ...
    parser.add_argument(
        "action", choices=["list", "enable", "disable"], help="Action to perform"
    )
