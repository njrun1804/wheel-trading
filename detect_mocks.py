        ...
        for alias in node.names:
            if "mock" in alias.name.lower():
                self.mock_imports.append(
                    {"line": node.lineno, "import": alias.name, "alias": alias.asname}
                )
    ...
    def visit_Call(self, node):
        ...
        if isinstance(node.func, ast.Name):
            if node.func.id in ["Mock", "MagicMock", "AsyncMock", "PropertyMock"]:
                self.mock_calls.append({"line": node.lineno, "type": node.func.id})
        self.generic_visit(node)
    ...
    def visit_Assign(self, node):
        ...
        if hasattr(node.value, "value"):
            ...
            for target in node.targets:
                if isinstance(target, ast.Name) and "api_key" in target.id.lower():
        ...
        for dir_name, files in sorted(by_dir.items()):
            ...
            for filepath in files:
                ...
                if mock_count or patch_count:
                    report_lines.append(
                        f"    Imports: {mock_count}, Patches: {patch_count}"
                    )
...
def _add_file_details(lines: list[str], analysis: dict):
    ...
    if analysis.get("mock_imports"):
        ...
        for imp in analysis["mock_imports"]:
            lines.append(
                f"    Line {imp['line']}: {imp.get('module', '')}.{imp['import']}"
            )
    ...
    if analysis.get("fake_data"):
        ...
        for fake in analysis["fake_data"]:
            lines.append(
                f"    Line {fake['line']}: {fake['variable']} = '{fake['value']}'"
            )
