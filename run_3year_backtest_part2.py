...
def optimize_parameters_for_regime(
    returns: np.ndarray,
    volatility: float,
    skewness: float
) -> dict:
...
async def backtest_with_regime_parameters():
    ...
    for regime in regime_order:
        ...
        print(f"    Position Size: {params['position_size']:.1%} (Kelly: {params['kelly_fraction']:.1%})")
    ...
    for regime in regime_order:
        ...
        if perfs:
            ...
            print(f"  {regime_names[regime]:<6} | {len(perfs):>7} | {avg_return:>10.1%} | "
    ...
    full_params = WheelParameters(
        target_delta=weighted_delta,
        target_dte=45,
        max_position_size=weighted_position
    )
    ...
    calmar = abs(full_result.annualized_return / full_result.max_drawdown) if full_result.max_drawdown != 0 else 0
