...
def assess_hardware():
    ...
    cpu_info = {
        ...
        "performance_cores": int(
            run_command("sysctl -n hw.perflevel0.physicalcpu") or "0"
        ),
        "efficiency_cores": int(
            run_command("sysctl -n hw.perflevel1.physicalcpu") or "0"
        ),
        "cpu_frequency_max": int(run_command("sysctl -n hw.cpufrequency_max") or "0")
        / 1e9,
        "cache_sizes": {
    ...
    print(
        f"  Cache sizes: L1i={cpu_info['cache_sizes']['l1_icache']}, "
    ...
    gpu_info = {
        ...
        "metal_version": run_command(
            "system_profiler SPDisplaysDataType | grep 'Metal' | head -1"
        ),
        "gpu_family": run_command(
    ...
    try:
        ...
        print(
            f"  TensorFlow Metal: {'✅' if gpu_libs['tensorflow_metal']['available'] else '❌'}"
        )
    except ImportError:
    ...
    print(
        f"  Available: {storage_info['available_gb']:.1f}GB ({100-disk.percent:.1f}%)"
    )
    ...
    features = {
        ...
        "accelerate_framework": os.path.exists(
            "/System/Library/Frameworks/Accelerate.framework"
        ),
        "metal_performance_shaders": os.path.exists(
...
def generate_optimization_config(assessment: dict) -> dict:
    ...
    config = {
        ...
        "memory": {
            "cache_size_mb": int(
                assessment["memory"]["total_gb"] * 1024 * 0.2
            ),  # 20% for cache
            "buffer_size_mb": int(
                assessment["memory"]["total_gb"] * 1024 * 0.3
            ),  # 30% for buffers
            "max_allocation_gb": int(assessment["memory"]["total_gb"] * 0.8),  # 80% max
        },
        "gpu": {
            "preferred_backend": "mlx"
            if assessment["gpu"]["libraries"]["mlx"]["available"]
            else "cpu",
            "batch_size": 4096
            if assessment["gpu"]["libraries"]["mlx"]["available"]
            else 1024,
        ...
        "io": {
            ...
            "prefetch_size_mb": 64
            if assessment["storage"]["ssd_type"] == "NVMe"
            else 32,
            "use_mmap": True,
        },
    }
