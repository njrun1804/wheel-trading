...
from meta_daemon_config import LearningRulesConfig, get_daemon_config
...
class QualityReport:
    ...
    def is_compliant(self) -> bool:
        ...
        return (
            self.compliance_score >= config.quality_gate.minimum_compliance_percentage
        )
...
class LearningRuleEnforcer:
    ...
    def __init__(self, config: LearningRulesConfig):
        ...
        self.executor = ThreadPoolExecutor(
            max_workers=get_daemon_config().daemon.worker_threads
        )
        ...
        print("   Anti-pattern checks: Active")
    ...
    async def enforce_quality_rules(self, file_path: Path) -> QualityReport:
        ...
        if total_checks == 0:
            ...
            compliance_score = max(
                0.0, 100.0 - (error_violations / total_checks * 100.0)
            )
    ...
    async def _check_implementation_depth(
        self, file_path: Path, content: str
    ) -> list[QualityViolation]:
                ...
                if func_end:
                    ...
                    for line in func_lines:
                        ...
                        if (
                            ...
                            and not stripped.startswith('"""')
                            and not stripped.startswith("'''")
                            and stripped not in ["pass", "return", "return None"]
                        ):
                            meaningful_lines += 1
    ...
    async def _check_dependency_usage(
        self, file_path: Path, content: str
    ) -> list[QualityViolation]:
    ...
    async def _check_async_completeness(
        self, file_path: Path, content: str
    ) -> list[QualityViolation]:
                ...
                if not has_await and self.config.require_await_in_async:
                    violations.append(
                        QualityViolation(
                            ...
                            severity="warning",
                            suggestion="Add await calls or make function synchronous",
                            learning_rule_id=6,
                        )
                    )
    ...
    def format_violation_report(self, report: QualityReport) -> str:
        ...
        for rule_name, violations in violations_by_rule.items():
            ...
            for violation in violations[:3]:  # Show first 3
                ...
                output.append(
                    f"     {severity_icon} Line {violation.line_number}: {violation.violation_text}"
                )
