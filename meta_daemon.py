...
from meta_daemon_config import get_daemon_config
...
class MetaDaemon:
    ...
    def __init__(self):
        ...
        self.logger.info(
            f"   Background threads: {self.config.daemon.background_threads}"
        )
        self.logger.info(
            f"   Real-time processing: {self.config.file_watch.real_time_processing}"
        )
        self.logger.info(
            f"   Quality enforcement: {self.config.quality_gate.enforcement_level}"
        )
            ...
            while self.is_running:
                ...
                for i, task in enumerate(background_tasks):
                    if task.done():
                        ...
                        if i == 0:
                            background_tasks[i] = asyncio.create_task(
    ...
    async def _setup_file_watching(self):
        ...
        self.observer.schedule(self.event_handler, path=str(watch_path), recursive=True)
    ...
    def schedule_file_change(self, file_path: Path, change_type: str):
        ...
        if (
            ...
            asyncio.run_coroutine_threadsafe(
                self.handle_file_change(file_path, change_type), self._event_loop
            )
    ...
    async def handle_file_change(self, file_path: Path, change_type: str):
        ...
        try:
            ...
            quality_report = await self.quality_enforcer.enforce_quality_rules(
                file_path
            )
            ...
            if len(self.stats["compliance_scores"]) > retention_limit:
                self.stats["compliance_scores"] = self.stats["compliance_scores"][
                    -retention_limit:
                ]
            ...
            if not quality_report.is_compliant:
                ...
                self.logger.info(
                    f"✅ {file_path.name}: {quality_report.compliance_score:.1f}% compliant"
                )
    ...
    async def _handle_quality_violations(
        self, file_path: Path, quality_report: QualityReport
    ):
        ...
        if enforcement_level == "strict":
            ...
            violation_summary = self.quality_enforcer.format_violation_report(
                quality_report
            )
            self.logger.error("🚨 QUALITY GATE VIOLATION BLOCKED:")
            ...
            self.logger.warning("⚠️ QUALITY WARNING:")
            self.logger.warning(violation_summary)

        else:  # log
            # Just log
            self.logger.info(
                f"ℹ️ Quality issues detected in {file_path}: {len(quality_report.violations)} violations"
            )

    async def _alert_quality_violation(
        self, file_path: Path, quality_report: QualityReport
    ):
...
"""
        ...
        try:
            ...
            self.logger.warning(
                f"Quality violation in {file_path}: {quality_report.compliance_score:.1f}% compliance"
            )
    ...
    async def _health_monitor(self):
        ...
        while self.is_running:
            try:
                ...
                self.logger.debug(
                    f"💚 Health check: OK (uptime: {uptime:.0f}s, memory: {memory_usage_mb:.1f}MB)"
                )
        ...
        while self.is_running:
            try:
                if self.stats["compliance_scores"]:
                    current_avg = sum(self.stats["compliance_scores"]) / len(
                        self.stats["compliance_scores"]
                    )

                    if (
                        current_avg
                        < self.config.quality_gate.minimum_compliance_percentage
                    ):
                        self.logger.warning(
                            f"📉 Compliance dropping: {current_avg:.1f}%"
                        )
...
"""
    ...
    def get_status(self) -> DaemonStatus:
        ...
        if self.stats["compliance_scores"]:
            current_compliance = sum(self.stats["compliance_scores"]) / len(
                self.stats["compliance_scores"]
            )
