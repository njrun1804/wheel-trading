...
class MetaWatcher:
    ...
    def handle_file_change(self, file_path: Path):
        ...
        self._record_observation(
            "file_modified",
            {"file_path": str(file_path), "timestamp": time.time(), **change_analysis},
        )
    ...
    def detect_change_patterns(self, file_path: Path, change_analysis: dict[str, Any]):
        ...
        if len(recent_changes) >= 3:
            ...
            timestamps = [
                json.loads(change[0])["timestamp"] for change in recent_changes[:3]
            ]
            intervals = [
...
async def start_meta_watching():
    ...
    watcher._record_observation(
        "coordinated_watching_started",
        {
