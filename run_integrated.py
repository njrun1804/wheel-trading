...
async def run_integrated_system():
    ...
    print(
        f"   Performance: {'Arrow/Polars' if config.performance.use_arrow else 'Standard'}"
    )
    print(
        f"   MCP Servers: {sum([config.mcp.use_duckdb_mcp, config.mcp.use_mlflow_mcp, config.mcp.use_statsource_mcp])}/3 enabled"
    )
    ...
    if hasattr(registry.storage, "get_options_polars"):
        options_df = await registry.storage.get_options_polars(
            config.trading.symbol, lookback_hours=24
        )
    ...
    analysis = await registry.risk_analyzer.analyze_portfolio(
        positions=positions, portfolio_value=100000  # This would come from account
    )
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Unity Wheel Trading - Integrated System"
    )
